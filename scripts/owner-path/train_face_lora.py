"""**이 스크립트는 더 이상 쓰지 않는다. `face_lora_aitoolkit.yaml`을 써라.**

2026-08-22에 두 길을 다 재봤고 결론이 났다.

    ComfyUI 내장 노드   한 단계 149초 -> 1000단계에 65시간   (다섯 번 실패)
    ai-toolkit          한 단계 1.4초 -> 1000단계에 **26분**  (성공)

**100배 차이다.** 이 파일은 왜 안 되는지를 숫자로 남기려고 둔다 -- 다음 사람이
"ComfyUI에 학습 노드가 있으니 그걸 쓰자"고 다시 가지 않도록. 아래 주석의 실측이
그 근거다.

돌리는 법은 `scripts/owner-path/face_lora_aitoolkit.yaml` 머리말에 있다.
