...
createServer(async (request, response) => {
  ...
  if (/^\/api\/projects\/local-draft\/review-approvals\/timelines\/[^/]+$/.test(url.pathname) && request.method === "GET") return sendJson(response, 200, { project_id: "local-draft", timeline_id: latestBundle?.timeline_id ?? "timeline_e2e", review_status: "draft", approved_at: null, updated_at: "now", source_session_id: latestBundle?.session_id ?? atomicSession.session_id, source_session_revision: 1, is_current: true, invalidated_at: null, invalidated_reason: null });
  ...
  if (url.pathname.startsWith("/api/projects/local-draft/subtitles/") && request.method === "GET") {
    ...
    return sendJson(response, 200, { job_id: jobId, status: "succeeded", subtitle: { subtitle_id: `subtitle-export-${jobId}`, project_id: "local-draft", timeline_id: latestBundle?.timeline_id, format: "srt", file_uri: `local://${jobId}.srt`, status: "succeeded", notes: [], source_session_id: latestBundle?.session_id ?? atomicSession.session_id, source_session_revision: isCurrentJob ? outputArtifactRevision : 1, is_current: isCurrentJob } });
  }
  if (url.pathname.startsWith("/api/projects/local-draft/final-renders/") && request.method === "GET") {
    ...
    return sendJson(response, latestBundle?.output_blocked ? 400 : 200, latestBundle?.output_blocked ? { detail: "gap_blocks_final_output" } : { job_id: jobId, status: "succeeded", render: { export_id: `final-export-${jobId}`, timeline_id: latestBundle?.timeline_id, export_type: "final_render", file_uri: `local://${jobId}.mp4`, status: "succeeded", source_session_id: latestBundle?.session_id ?? atomicSession.session_id, source_session_revision: isCurrentJob ? outputArtifactRevision : 1, is_current: isCurrentJob } });
  }
  if (url.pathname.startsWith("/api/projects/local-draft/capcut-draft-exports/") && request.method === "GET") {
    ...
    return sendJson(response, 200, { job_id: jobId, status: "succeeded", export: { export_id: `capcut-export-${jobId}`, timeline_id: latestBundle?.timeline_id, export_type: "capcut_draft", file_uri: `local://${jobId}`, status: "succeeded", notes: [], source_session_id: latestBundle?.session_id ?? atomicSession.session_id, source_session_revision: isCurrentJob ? outputArtifactRevision : 1, is_current: isCurrentJob, handoff: { status: isCurrentJob ? capcutHandoffStatus : "not_started", source_file_uri: `local://${jobId}`, source_session_id: latestBundle?.session_id ?? atomicSession.session_id, source_session_revision: isCurrentJob ? outputArtifactRevision : 1, registered_project_path: isCurrentJob && capcutHandoffStatus === "ready" ? "C:/mock/draft" : null, error_message: null, registered_at: isCurrentJob && capcutHandoffStatus === "ready" ? "now" : null, reused: false } } });
  }
