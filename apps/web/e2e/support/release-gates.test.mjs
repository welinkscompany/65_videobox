...
test("performance report has a fixed five-sample protocol and fails only structural or 20 percent regression", () => {
  ...
  assert.deepEqual(report, {
    ...
    browser: { engine: "chromium", version: "123.0.0.0", ci_profile: "chromium-headless-workers-1-1920x1080" },
    interaction: "right_dock_drag",
    warmup_count: 1,
    measurement_count: 5,
    baseline_median_ms: 250,
    baseline_capture_profile: "chromium-headless-workers-1-1920x1080",
    baseline_capture_intent: "initial absolute guard pending a calibrated reference capture",
  ...
  assert.equal(assessWorkbenchPerformance({ browserVersion: "123", ciProfile: "ci", warmupMs: 1, measurementsMs: [301, 301, 301, 301, 301] }).regression, true);
