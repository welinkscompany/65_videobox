...
async function reset(page) {
  expect((await page.request.post("http://127.0.0.1:8000/__e2e/reset-draft")).status()).toBe(200);
...
test("gap-only approval preserves returned gap IDs and blocks final render and CapCut", async ({ page }) => {
  ...
  const state = await (await page.request.get("http://127.0.0.1:8000/__e2e/draft-state")).json();
  ...
  expect((await page.request.post("http://127.0.0.1:8000/api/projects/local-draft/jobs/final-render")).status()).toBe(400);
  expect((await page.request.post("http://127.0.0.1:8000/api/projects/local-draft/jobs/capcut-draft-export")).status()).toBe(400);
});

test("ready-assets approval uses returned IDs and provides current-revision playback and CapCut smoke", async ({ page }) => {
  await reset(page);
  expect((await page.request.post("http://127.0.0.1:8000/api/projects/local-draft/draft-readiness/broll/upload")).status()).toBe(201);
  expect((await page.request.post("http://127.0.0.1:8000/api/projects/local-draft/draft-readiness/readiness_e2e/complete")).status()).toBe(200);
  const requested = [];
  ...
  await expect(page).toHaveURL(new RegExp(`/editor\\?session_id=${state.latest_bundle.session_id}$`));
  const video = page.getByLabel("현재 편집본 재생");
  ...
  const handoff = await page.request.post("http://127.0.0.1:8000/api/projects/local-draft/capcut-draft-exports/capcut-e2e/handoff");
