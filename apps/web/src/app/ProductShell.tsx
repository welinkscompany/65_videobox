...
export function ProductShell({ projectId, projects, archive, section, onNavigate, onOpenSettings, onArchiveProject, onDeleteProjectPermanently, onRenameProject, children, forceCollapsed = false }: { projectId: string; projects: Project[]; archive?: ProjectArchiveControls; section: ShellSection; onNavigate: (projectId: string, section: WorkspaceSection) => void; onOpenSettings: () => void; onArchiveProject?: (projectId: string) => void | Promise<void>; onDeleteProjectPermanently?: (projectId: string) => void | Promise<void>; onRenameProject?: (projectId: string, name: string) => void | Promise<void>; children: ReactNode; forceCollapsed?: boolean }) {
  ...
  return <SidebarProvider open={!collapsed} onOpenChange={(open) => setCollapsed(!open)}>
    ...
    <Sidebar collapsible="icon" className="vb-product-sidebar" aria-label="프로젝트와 화면">
      ...
      </SidebarContent><SidebarFooter><div className="vb-sidebar-footer"><small className="group-data-[collapsible=icon]:hidden">{localDeploymentCapabilities.aiExecution === "local" ? "이 기기에서 작업" : "AI 기능 끔"}</small></div></SidebarFooter>{/* 가져온 `SidebarRail`은 폭 16px 띠에 `tabIndex={-1}`이다 -- 마우스로 정확히
          겨눠야 하고 키보드로는 아예 닿지 않는다. owner가 "클릭하기가 너무 어렵다"고
          한 자리다. 폭을 손가락이 닿는 크기로 넓히고 탭 순서에 돌려놓는다. */}
      <SidebarRail tabIndex={0} className="vb-sidebar-rail" aria-label={collapsed ? "작업실 펼치기" : "작업실 접기"} title={collapsed ? "작업실 펼치기" : "작업실 접기"} />
    </Sidebar>
