...
describe("responsive Director workspace", () => {
  it("640px sheet preserves the draft and returns focus after Escape", async () => {
    ...
    const openButton = screen.getByRole("button", { name: "유진 열기" });
    fireEvent.click(openButton);
    const textbox = screen.getByRole("textbox", { name: "유진에게 요청하기" });
    fireEvent.change(textbox, { target: { value: "사람 없는 영상" } });
    fireEvent.keyDown(textbox, { key: "Escape" });
    await waitFor(() => expect(openButton).toHaveFocus());
    fireEvent.click(openButton);
    expect(screen.getByRole("textbox", { name: "유진에게 요청하기" })).toHaveValue("사람 없는 영상");
  });

  it("640px sheet is an aria-modal dialog and traps keyboard focus", async () => {
    renderResponsiveDirector(640);
    fireEvent.click(screen.getByRole("button", { name: "유진 열기" }));
    const dialog = screen.getByRole("dialog", { name: "유진 영상 도우미" });
  ...
  it("back and close both return to the trigger while preserving the draft", async () => {
    ...
    const openButton = screen.getByRole("button", { name: "유진 열기" });
    fireEvent.click(openButton);
    fireEvent.change(screen.getByRole("textbox", { name: "유진에게 요청하기" }), { target: { value: "유지할 초안" } });
    fireEvent.click(screen.getByRole("button", { name: "뒤로" }));
    await waitFor(() => expect(openButton).toHaveFocus());
    fireEvent.click(openButton);
    expect(screen.getByRole("textbox", { name: "유진에게 요청하기" })).toHaveValue("유지할 초안");
  ...
  it("uses a bounded desktop aside and an accessible collapse control", async () => {
    ...
    const aside = screen.getByRole("complementary", { name: "유진 영상 도우미" });
    expect(aside).toHaveClass("director-aside");
    fireEvent.click(screen.getByRole("button", { name: "유진 접기" }));
    expect(screen.getByRole("button", { name: "유진 펼치기" })).toHaveAttribute("aria-expanded", "false");
    expect(screen.queryByRole("textbox", { name: "유진에게 요청하기" })).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "유진 펼치기" }));
    expect(screen.getByRole("textbox", { name: "유진에게 요청하기" })).toBeVisible();
  });
  ...
  it("marks the narrow candidate tray as a carousel and exposes reduced-motion preference", async () => {
    ...
    expect(screen.getByRole("dialog", { name: "유진 영상 도우미" })).toHaveAttribute("data-reduced-motion", "true");
  });
  ...
  it("blocks Task16 manual library background focus and apply while the sheet is open", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "유진 열기" }));
    await screen.findByRole("dialog", { name: "유진 영상 도우미" });
  ...
  it("returns focus to the mounted desktop control when the narrow sheet exits at the breakpoint", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("button", { name: "유진 접기" })).toHaveFocus());
  });
  ...
  it("does not close the sheet while an IME composition handles Escape", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "유진 열기" }));
    await screen.findByText("추천을 적용하기 전에 변경된 내용이 있는지 확인하고 있어요.");
    fireEvent.keyDown(screen.getByRole("textbox", { name: "유진에게 요청하기" }), { key: "Escape", isComposing: true });
    expect(screen.getByRole("dialog", { name: "유진 영상 도우미" })).toBeVisible();
  });
});
