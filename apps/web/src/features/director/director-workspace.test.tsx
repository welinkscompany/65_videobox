...
describe("DirectorWorkspace", () => {
  it("유진의 시작과 요청 제어를 제작자 언어로 표시한다", () => {
    ...
    expect(screen.getByRole("heading", { name: "유진" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "유진에게 추천받기" })).toBeInTheDocument();
    expect(screen.getByRole("textbox", { name: "유진에게 요청하기" })).toBeInTheDocument();
  });
  ...
  it("추천 항목을 다시 고를 때 P12와 접두사가 붙은 원시 참조 코드를 노출하지 않는다", async () => {
    ...
    fireEvent.change(screen.getByRole("textbox", { name: "유진에게 요청하기" }), { target: { value: "3번 영상 교체" } });
    fireEvent.click(screen.getByRole("button", { name: "요청 보내기" }));
    await screen.findByText("유진 추천 12의 비롤 3번");
    expect(screen.getByText("유진 추천 1의 배경음악 2번")).toBeVisible();
  ...
  it("명시적 적용은 preflight 뒤 선택 후보를 materialize하고 candidate_ids만 한번 적용한다", async () => {
    ...
    fireEvent.change(screen.getByRole("textbox", { name: "유진에게 요청하기" }), { target: { value: "적용" } });
    fireEvent.click(screen.getByRole("button", { name: "요청 보내기" }));
  ...
  it("apply 직전 재검증이 stale이면 materialize나 apply를 호출하지 않는다", async () => {
    ...
    fireEvent.change(screen.getByRole("textbox", { name: "유진에게 요청하기" }), { target: { value: "적용" } }); fireEvent.click(screen.getByRole("button", { name: "요청 보내기" })); await screen.findByText("준비");
    await waitFor(() => expect(screen.getByRole("button", { name: "이 추천 적용" })).toBeEnabled());
    fireEvent.click(screen.getByRole("button", { name: "이 추천 적용" }));
  ...
  it("후보 선택은 batch transaction으로 적용된다는 상태를 표시한다", () => {
    render(<DirectorWorkspace state="proposal_ready" projectId="project-1" sessionId="session-1" sessionRevision={4} proposal={proposal} preflightProposal={vi.fn().mockResolvedValue({ status: "ready", diff: {} })} materializeCandidate={vi.fn()} applyProposal={vi.fn()} sendMessage={vi.fn()} onManualMode={vi.fn()} />);
    expect(screen.getByRole("checkbox", { name: "유진 추천 12의 비롤 3번 고르기" })).toBeChecked();
    expect(screen.getByRole("checkbox", { name: "유진 추천 12의 비롤 4번 고르기" })).not.toBeChecked();
  ...
  it("명시적 all scope는 client materialize loop 없이 batch apply endpoint를 한 번만 호출한다", async () => {
    ...
    fireEvent.change(screen.getByRole("textbox", { name: "유진에게 요청하기" }), { target: { value: "모두 적용" } }); fireEvent.click(screen.getByRole("button", { name: "요청 보내기" })); await screen.findByText("일괄 적용 준비");
    fireEvent.click(screen.getByRole("checkbox", { name: "유진 추천 12의 비롤 4번 고르기" }));
    await waitFor(() => expect(screen.getByRole("checkbox", { name: "유진 추천 12의 비롤 4번 고르기" })).toBeChecked());
    fireEvent.click(screen.getByRole("button", { name: "이 추천 적용" }));
  ...
  it("여러 후보가 선택됐는데 batch provider가 없으면 첫 후보만 적용하지 않고 오류를 표시한다", async () => {
    ...
    fireEvent.change(screen.getByRole("textbox", { name: "유진에게 요청하기" }), { target: { value: "두 후보 적용" } }); fireEvent.click(screen.getByRole("button", { name: "요청 보내기" })); await screen.findByText("일괄 적용 준비");
    fireEvent.click(screen.getByRole("checkbox", { name: "유진 추천 12의 비롤 4번 고르기" }));
    fireEvent.click(screen.getByRole("button", { name: "이 추천 적용" }));
  ...
  it("apply scope는 selected/broll/all 명시 radio로 고르고 그 결과만 batch endpoint에 보낸다", async () => {
    ...
    fireEvent.change(screen.getByRole("textbox", { name: "유진에게 요청하기" }), { target: { value: "B-roll 모두 적용" } }); fireEvent.click(screen.getByRole("button", { name: "요청 보내기" })); await screen.findByText("범위 적용 준비");
  ...
  it("전체 범위 radio는 선택 상태와 무관하게 proposal 순서 전체를 batch endpoint에 보낸다", async () => {
    ...
    render(<DirectorWorkspace state="proposal_ready" projectId="project-1" sessionId="session-1" sessionRevision={4} proposal={mixed} preflightProposal={vi.fn().mockResolvedValue({ status: "ready", diff: {} })} materializeCandidate={vi.fn()} applyProposal={vi.fn()} batchApplyProposal={batchApplyProposal} sendMessage={sendMessage} onManualMode={vi.fn()} />);
    fireEvent.change(screen.getByRole("textbox", { name: "유진에게 요청하기" }), { target: { value: "전체 적용" } }); fireEvent.click(screen.getByRole("button", { name: "요청 보내기" })); await screen.findByText("전체 적용 준비");
  ...
  it("proposal 교체 시 이전 후보 id를 버리고 새 proposal의 후보로 선택을 재설정한다", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("checkbox", { name: "유진 추천 13의 비롤 1번 고르기" })).toBeChecked());
    expect(screen.queryByRole("checkbox", { name: "유진 추천 12의 비롤 3번 고르기" })).not.toBeInTheDocument();
  });
