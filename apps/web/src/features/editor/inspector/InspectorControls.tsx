            ...
            <>
              ...
              {showMediaField("speed") ? (
                <>
                  ...
                  {/* **여기에 `장면 길이`를 붙이면 안 된다(2026-09-04 코드리뷰로
                      걷어냄).** 한 번 붙였다가 **틀린 숫자**를 보여 줬다.

                      이 칸은 `media_controls.speed`다. 엔진에서 이 값이 하는 일은
                      **원본을 얼마나 먹는가**이지 장면 슬롯이 아니다 --
                      `composition_plan.py:838`이 `source_out = source_in +
                      (end - start) * speed`로 계산하고 `end - start`는 그대로 둔다.
                      2배속을 걸면 같은 4초 자리에 원본 8초를 밀어 넣는 것이지
                      장면이 2초가 되는 게 아니다.
      ...
      {target?.kind === "overlay" ? (
        <fieldset>
          ...
          {/* 사진을 장면 위에 얹는 자리(owner 요청 2026-09-06 "사진을 우리 영상
              ...
              한 번 골랐던 자리를 "안 고름"으로 되돌리는 저장이 먹힌다.

              (최종 검토 발견 2026-09-11: 아래 문장은 틀렸었다 -- "자산 목록의
              `화면에 얹기`로 갓 얹은 사진은 이 단추를 거치지 않으므로 칸
              자체가 안 실려 아무것도 안 골랐는데 움직이는 일은 없다"는
              틀렸다. `화면에 얹기`도, 유진이 프리셋 없이 얹는 것도 칸을 아예
              안 보내고, 백엔드의 세 상태 계약에서 "안 보냄"은 "지금 저장된
              값을 그대로 둔다"는 뜻이다(`update_segment_image_overlay`
              머리말). 그래서 프리셋이 이미 있는 장면 위에 새 자산을 얹으면
              그 자산이 **앞 자산의 자리·크기·움직임을 그대로 물려받는다.**
              이건 결함이 아니라 의도된 동작이다 -- 엔진은 세그먼트 하나당
              이미지 오버레이를 최대 하나만 두므로, "이미 오버레이가 있는
              장면에 B를 얹는다"는 실제로는 "이미 놓인 액자 속 그림만
              바꾸는" 일에 가깝다. 그림창(PIP)에서 화면만 바꿔도 액자 위치는
              그대로인 것과 같은 기대다. 자리를 통째로 다시 잡고 싶으면
              오버레이를 뺐다가 새로 얹으면 된다. 고정 시험:
