...
export function CreationInterview({ projectId }: { projectId: string }) {
  ...
  async function skipCandidate(assetId: string) {
    // 자동 채우기가 도는 동안 수동 편집을 잠근다(owner 결정 2026-08-29) --
    // 둘 다 readiness.revision을 건드려서 자동 채우기 중 수동 편집이 끼어들면
    // draft_readiness_revision_conflict로 부딪혔다. 단추 disabled로도 막지만
    // 이 함수를 직접 호출하는 경로가 생겨도 막히도록 여기서도 확인한다.
