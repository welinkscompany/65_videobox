...
export function CreationInterview({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    ...
    setCandidateRanges(Object.fromEntries(candidates.map((item) => [item.asset_id, { start: String(item.target_range.start_sec), end: String(item.target_range.end_sec) }] )));
  }, [readiness]);
