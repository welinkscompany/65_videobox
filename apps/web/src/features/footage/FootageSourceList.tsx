  ...
  return <aside className="vb-footage-pane vb-footage-sources" data-testid="footage-source-list">
    ...
    {assets.length === 0 ? <p className="vb-footage-empty">준비된 촬영본이 없어요.</p> : <div className="vb-footage-source-scroll" data-bounded="true">
      {assets.map((asset) => {
        ...
        return <Button key={asset.library_asset_id} type="button" variant="ghost" className="vb-footage-source" aria-pressed={selectedIds.includes(asset.library_asset_id)} onClick={(event) => onSelect(asset, event.shiftKey)}>
