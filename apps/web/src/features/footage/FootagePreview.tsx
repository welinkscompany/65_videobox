...
export function FootagePreview({ asset, previewUrl, previewRanges = [], previewUnavailable = false, currentTime, duration, frameStep, onTimeChange, onFrameStep, onPreviewError }: Props) {
  ...
  return <section className="vb-footage-pane vb-footage-preview" data-testid="footage-preview">
    <div className="vb-footage-pane__heading"><div><p className="vb-eyebrow">PREVIEW</p><h2>{filename}</h2>{asset ? <a className="vb-action-link" href={`/library?library_asset_id=${encodeURIComponent(asset.library_asset_id)}`}>라이브러리에서 보기</a> : null}</div><span>{formatTime(currentTime)} / {formatTime(duration)}</span></div>
