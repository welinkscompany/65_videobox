...
type Props = { asset: LibraryAsset | null; previewUrl?: string | null; currentTime: number; duration: number; frameStep: number; onTimeChange: (time: number) => void; onFrameStep: (delta: number) => void };

export function FootagePreview({ asset, previewUrl, currentTime, duration, frameStep, onTimeChange, onFrameStep }: Props) {
  const videoRef = useRef<HTMLVideoElement | null>(null);
  const filename = asset ? String(asset.user_metadata?.filename ?? asset.library_asset_id) : "촬영본을 선택하세요";
  useEffect(() => {
    const video = videoRef.current;
    if (video && Math.abs(video.currentTime - currentTime) > frameStep) video.currentTime = currentTime;
  ...
  return <section className="vb-footage-pane vb-footage-preview" data-testid="footage-preview">
    ...
    <div className="vb-footage-player">{asset ? <video ref={videoRef} controls preload="metadata" src={previewUrl ?? asset.preview_url ?? undefined} aria-label={`${filename} 미리보기`} data-testid="footage-video" onTimeUpdate={(event) => onTimeChange(event.currentTarget.currentTime)} onSeeked={(event) => onTimeChange(event.currentTarget.currentTime)} /> : <p>선택한 촬영본의 미리보기가 여기에 표시돼요.</p>}</div>
