...
import { api, type FootageProposal, type FootageProposalPreview, type FootageSegment, type FootageSequence, type LibraryAsset } from "../../api";
...
export function FootageOrganizerPage() {
  ...
  const sequenceItems = useMemo(() => {
    const selected = proposal?.segments.find((segment) => segment.segment_id === selectedSegmentId);
    return selected ? [{ source_segment_id: selected.source_segment_id, item_order: 1, start_sec: selected.start_sec, end_sec: selected.end_sec }] : [];
  }, [proposal, selectedSegmentId]);

  function selectAsset(asset: LibraryAsset) { setSelectedAsset(asset); setProposal(null); setProposalPreview(null); setSequence(null); setSelectedSequenceItemId(null); setSelectedSegmentId(null); setPlayhead(0); setNotice(null); }
  async function startProposal() {
    ...
    try {
      ...
      setProposal(result); setProposalPreview(null); setSelectedSegmentId(result.segments[0]?.segment_id ?? null); setNotice("제안이 준비됐어요. 타임라인을 확인하세요.");
  ...
  async function previewProposal() { if (!proposal) return; setBusy("미리보기"); try { const result = await api.previewFootageProposal(proposal.proposal_id, { expected_revision: proposal.revision }); setProposalPreview(result); setNotice("제안 미리보기를 준비했어요. 원본은 그대로예요."); } catch { setNotice("미리보기를 준비하지 못했습니다."); } finally { setBusy(null); } }
  ...
  return <main className="vb-footage-page" data-testid="footage-workspace" data-layout="four-pane">
    ...
    {loading ? <div className="vb-footage-state" role="status">촬영본을 불러오는 중…</div> : error ? <div className="vb-footage-state" role="alert"><p>{error}</p><Button type="button" variant="outline" onClick={() => void loadAssets()}>다시 시도</Button></div> : <div className="vb-footage-grid">
      ...
      <div className="vb-footage-center"><FootagePreview asset={selectedAsset} previewUrl={proposalPreview?.preview_url} currentTime={playhead} duration={duration} frameStep={FRAME_STEP} onTimeChange={setPlayhead} onFrameStep={(delta) => setPlayhead((time) => Math.max(0, Math.min(duration || Infinity, time + delta * FRAME_STEP)))} /><SceneTimeline proposal={proposal} playhead={playhead} selectedSegmentId={selectedSegmentId} onSelectSegment={(segment) => { setSelectedSegmentId(segment.segment_id); setPlayhead(segment.start_sec); }} onSplit={split} onMerge={merge} onExclude={exclude} onBoundary={moveBoundary} /></div>
