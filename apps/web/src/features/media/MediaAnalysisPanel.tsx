...
export function MediaAnalysisPanel({ projectId, onSelectAsset }: { projectId: string; onSelectAsset?: (assetId: string) => void }) {
  ...
  return <section className="panel" aria-labelledby="media-analysis-heading">
    ...
    {items.length === 0 ? <p className="empty-state">분석 대기 항목 없음</p> : items.map((item, index) => <article className="confirmation-card" key={item.analysis_id}>
      <strong>{item.asset_id}</strong><p>{item.status} · {item.progress_percent}% · 대기 순서 {index + 1}</p>
      {item.error_message ? <p className="error-banner">{item.error_message}</p> : null}
      <button className="action-button subtle" type="button" onClick={() => void action(async () => { const response = await api.mediaAnalysisPreview(projectId, item.asset_id); const raw = response.preview as { duration_sec?: unknown } | null; setPreview({ assetId: item.asset_id, durationSec: typeof raw?.duration_sec === "number" ? raw.duration_sec : undefined }); onSelectAsset?.(item.asset_id); })}>미리보기</button>
