...
export function MediaAnalysisPanel({ projectId, onSelectAsset }: { projectId: string; onSelectAsset?: (assetId: string) => void }) {
  ...
  return <section className="panel" aria-labelledby="media-analysis-heading">
    ...
    {items.length === 0 ? <p className="empty-state">확인할 미디어가 없어요.</p> : items.map((item) => <article className="confirmation-card" key={item.analysis_id}>
      <strong>{item.asset_id}</strong><p>{statusCopy[item.status] ?? "미디어를 확인하고 있어요"} · {item.progress_percent}%</p>
