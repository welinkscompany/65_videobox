...
**Scope:** 루미 대화형 영상 제작, 전용 Hermes 실행 경계, mem0 보조 기억, 로컬 Qwen/BGE 역할 분리, 향후 SaaS 전환 경계

## 1. 결정

VideoBox의 사용자 대화와 영상 기획을 담당하는 에이전트는 `루미`다. 루미의 주 창작·대화 추론은 VideoBox 전용 Hermes profile인 `lumi-video-director`가 맡는다. 현재 로컬 Qwen은 제거하지 않고, 원본 미디어를 외부로 보내지 않는 저비용 배경 분석과 제한된 구조화 보조 작업에 사용한다.

로컬 배포는 VideoBox와 Hermes를 하나의 container process에 섞지 않고, 하나의 `videobox-local` Docker Compose 제품 스택 안에서 독립 서비스로 운영한다. release-local profile은 VideoBox web/API/render worker, Hermes, mem0와 보안 gateway를 함께 시작한다. 개발 중 hot reload profile은 web/API를 호스트에서 실행할 수 있지만 최종 로컬 사용자 실행 단위는 Compose 스택이다.
...
1. 사용자가 VideoBox 안에서 루미와 대화하며 영상 목적과 스타일을 인터뷰한다.
2. 루미는 실제 프로젝트·자산·편집 상태를 typed tool로 조회하고 적용 가능한 초안을 제안한다.
...
- 사용자의 승인 없이 루미가 timeline 또는 output을 변경하는 것
...
release Compose project 이름은 `videobox-local`로 고정한다. dev/test는 `videobox-local-dev-<worktree-hash>`와 `videobox-local-test-<run-id>`처럼 충돌 없는 ephemeral suffix를 사용하고 release external volume을 연결하지 않으며 test 종료 때 자체 volume/network를 제거한다.
...
| `videobox-hermes-agent` | 루미 agent runtime과 OAuth-backed creative reasoning | internal only; debug profile만 loopback | `videobox-hermes-state`, read-only pinned skill bundle |
...
- 사용자 표시명: `루미`
- 내부 profile ID: `lumi-video-director`
- compose project: `videobox-local`
...
초기 embedded-Lumi tool allowlist는 다음으로 제한한다. 이 surface는 network MCP server가 아니라 Agent Gateway 내부 handler registry다.
...
Qwen은 Hermes OAuth 장애 시 사용자에게 알리지 않고 루미인 것처럼 답하지 않는다. 반대로 Qwen이 꺼져도 Hermes 대화, lexical search와 수동 편집은 계속 가능해야 한다.
...
| Lumi skill/prompt/policy source | repository의 versioned bundle + manifest |

mem0 장애는 편집과 대화를 막지 않는다. 해당 요청에서는 project context만 사용하고 `장기 기억 사용 불가` 상태를 audit에 남긴다.

## 13. Skill 학습 구조

루미가 매 작업마다 자신의 실행 code를 자동 변경하는 구조는 사용하지 않는다. 학습은 세 층으로 나눈다.
...
|---|---|---|
| Hermes/OAuth unavailable | `루미 온라인 대화 사용 불가`와 재연결 안내 | 수동 편집, local analysis/search, render/output |
...
2. **Local product stack:** `videobox-local` web/API/render worker/Hermes/memory gateway/mem0/분리 egress compose, host bridge, data root, identity와 aggregate health; 편집 mutation 없음
3. **Read-only Lumi chat:** project context, conversation persistence, read-only asset/session tools
...
7. **Editor integration:** OSS editor Task 20이 실제 Gateway의 우측 루미 패널과 inline candidate interaction을 소비
...
1. 현재 미커밋 Lumi copy를 OSS 계획 Task 1로 먼저 closeout한다.
...
- `docs/llm-provider-strategy.ko.md`의 `Qwen -> Gemini -> OpenAI` 자동 fallback 전략은 현재 local-only 구현 및 이 설계와 충돌하므로 historical 문서로 표시하거나 새 provider-capability 정책으로 교체한다.
- 같은 문서의 external/Gemini 0 계약은 normal test/local runtime에는 유지하되, explicit Hermes OAuth creative route를 별도 capability로 추가한다.
- `docs/videobox-mcp-scope.ko.md`의 외부 orchestration용 coarse-grained create/render/export tool 목록은 루미의 in-product allowlist로 재사용하지 않는다. 독립 VideoBox 엔진이라는 원칙은 유지하되, 루미 surface는 read/proposal-only와 별도 사용자 승인 endpoint로 좁힌다.
- `docs/superpowers/specs/2026-07-17-videobox-oss-dashboard-editor-adoption-design.md`와 22개 Task의 `Hermes agent/container 제외`는 해당 22개 Task 범위에만 유지한다. Hermes 작업은 새 계획으로 추적한다.
...
- 루미/Hermes/Qwen/BGE/deterministic code의 역할이 겹치지 않는다.
- 하나의 `videobox-local` 제품 스택 안에서 VideoBox와 Hermes가 별도 container로 분리되고, volume, identity, port와 credential 경계가 정의돼 있다.
