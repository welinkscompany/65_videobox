...
- [x] Step 4 — Release verify. 2026-07-14: CLI/service verifier accepted `starter-v1@1.0.0`; full Python 3.12 backend, frontend test/build, and 600-second Korean output smoke are green. Closeout gap/reverse gate `VIDEOBOX_RUN_REAL_STARTER_PACK_E2E=1 .venv\Scripts\python.exe -m pytest -q tests/test_real_starter_media_pack_e2e.py` passed against the actual ignored pack: install → real ID list selection/favorite/materialize → timeline → actual SRT content → FFmpeg MP4 audio/duration → PyCapCut draft, with source archive exclusion asserted throughout.
