...
### Task 36: 검토 화면이 실제 초안을 못 읽는다 — **미착수 · 최우선. Task 31을 실제로 막는다**
...
- [ ] **Step 1: 실패 테스트** — 번들이 만든 타임라인을 조회 엔드포인트로 읽는 API 테스트.
      **이 테스트가 없어서 지금까지 안 잡혔다**
- [ ] **Step 2~: 계약 정리 → 구현 → 실서버 재검증 → 커밋**

**이것이 닫히기 전에는 "내보내기가 뚫렸다"고 말할 수 없다.** Task 31은 화면과 API를
연결했을 뿐이고, 그 화면에 도달하는 경로가 막혀 있다.
