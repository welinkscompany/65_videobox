...
### Task 23: 긴 촬영본에서 쓸 구간 추천 — **중단 (2026-08-07). 상류가 막혀 있다**

**결론부터: 이 Task는 지금 구조로는 목표를 달성할 수 없다. Task 27이 선행돼야 한다.**

읽는 쪽(구간 선택 로직 + 배선)은 구현·검증까지 끝냈지만, **실측 결과 읽을 데이터가
실제로는 없었다.** 아래 "실측" 항목이 근거다.
...
- [ ] **Step 5: 전체 회귀 + 커밋** — 읽는 쪽만 커밋한다. Task 목표는 미달성
...
- [ ] **Step 0: 실제 영상으로 소요 시간 실측 (선행)** — owner의 520MB 파일로 위 명령을
      직접 돌려 초 단위를 잰다. **이 숫자 없이 방식을 정하지 않는다**
- [ ] **Step 1~: 실측 결과에 따라 Step 확정**

**경계 주의.** 장면 전환 감지는 "화면이 바뀌는 지점"이지 "좋은 장면"이 아니다.
산책처럼 완만한 영상은 전환이 적게 잡힐 수 있고, 그때는 분석의 태그·요약을 함께 쓰는
보완이 필요하다. Step 0 실측 뒤에 판단한다.
