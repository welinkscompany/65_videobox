...
**owner 결정이 필요한 지점.** 앞으로 올리실 영상이 주로 **긴 산책 영상**인지, 지금
라이브러리처럼 **짧은 클립**인지에 따라 만들 것이 달라진다. 긴 영상 위주라면 장면 전환
감지만으로 충분하고, 짧은 클립 위주라면 위 후보 중 하나를 더 정해야 한다.

- [ ] **Step 1: owner 확인 후 범위 확정** — 위 두 종류 중 무엇을 우선할지
- [ ] **Step 2~: 실패 테스트 → 구현 → 실측 → 커밋**
