...
### Task 30: `exact_preview` 회귀 테스트의 시계 경쟁 조건 — **미착수 · 기존 결함**
...
- [ ] **Step 1: 실패 재현 고정** — 시계를 고정한 상태에서 이 단언이 어떻게 깨지는지
- [ ] **Step 2~: 주입된 clock으로 바꾸거나 임계값을 시계 해상도보다 크게 잡는다**
...
### Task 35: 홈 화면이 실제 상태를 보여주지 않는다 — **미착수 · 중간**
...
- [ ] **Step 1: 실패 테스트** — 완성본이 있으면 홈이 그것을 반영하는지
- [ ] **Step 2~: RED → 구현 → 브라우저 실측 → 커밋**
