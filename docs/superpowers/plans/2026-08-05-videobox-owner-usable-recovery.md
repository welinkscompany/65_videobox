...
### Task 27: 실제 장면 경계 감지 — **완료 (2026-08-07, 커밋 `645d0bbee`)**

(2026-08-08 확인: 모든 Step이 완료인데 제목만 `미착수`로 남아 있었다. 제목을 실제
상태에 맞췄다. 아래 본문은 착수 시점의 기록이라 그대로 둔다.)
...
### Task 28: 장면 구간을 비전 모델과 분리해 저장 — **완료 (2026-08-07)**
...
- [x] **Step 1: 실패 테스트** — 비전 provider가 죽어도 장면 구간이 저장되는지,
      취소된 분석에는 여전히 안 써지는지
- [x] **Step 2: RED 확인** — `assert [] == [(0.0, 6.0), (6.0, 12.0)]`
- [x] **Step 3: 구현 — 원인이 두 군데였다.**
      1. `media_analysis.py`: 저장 호출을 `probe()` 직후, 비전 호출 **앞으로** 옮겼다
      2. **그것만으로는 부족했다.** `local_project_store._set_media_analysis_error()`가
         오류 상태 전환 시 `DELETE FROM media_scene_windows`를 실행해 방금 저장한 값을
         다시 지우고 있었다. 임베딩은 비전 요약과 짝이라 계속 지우고, **장면 구간만
         보존**하도록 고쳤다
- [x] **Step 4: 실제 서버 역방향 검증 + 회귀 + 커밋** — 실제 촬영본
      (`20260323_152848.mp4`, 18.4초)을 가져와 분석이 `LM_STUDIO_BLOCKED`로 막히는
      조건을 재현했고, **그 상태에서 장면 구간이 살아남는 것**을 확인했다:
      `[{'start_sec': 0.0, 'end_sec': 18.400967}]`. 이전 코드에서는 같은 조건에서 `[]`였다

**검증 함정 하나.** 첫 검증 시도는 이전 세션의 API 서버가 8000번 포트를 잡고 있어
**새 서버가 뜨지 못했고, 요청이 옛 코드로 갔다.** 결과를 신뢰하지 않고 포트를 비운 뒤
다시 검증했다. 포트 충돌은 조용히 옛 코드를 검증하게 만든다.

### Task 30: `exact_preview` 회귀 테스트의 시계 경쟁 조건 — **완료 (2026-08-07)**

Task 28 회귀에서 `tests/test_exact_preview_artifact.py`가 실패했다. **내 변경 탓인지
반복 측정으로 확인했고, 아니었다.**

| 조건 | 결과 |
|---|---|
| 변경 있음, 파일 쌍 실행 3회 | 3회 모두 1건 실패 |
| **변경 없음(stash), 파일 쌍 실행 2회** | **1회 통과 / 1회 실패** |
| 변경 있음, 단독 실행 | 통과 |

변경 없이도 실패하므로 **기존부터 불안정한 테스트**다. 다만 실패 빈도가 실행 순서에
따라 달라지므로 단정은 여기까지만 한다.

**원인.** `test_exact_preview_retry_creates_a_new_generation_after_failure`가
`recover_stale_exact_preview_claims(older_than_seconds=0) == 1`을 단언한다.
**실제 시계로 "0초보다 오래됐는가"를 묻는 것이라 시계 해상도에 따라 참·거짓이 갈린다.**
같은 파일의 다른 테스트가 실패로 보고되기도 했는데, 같은 경쟁 조건이 실행 순서에 따라
다른 지점에서 터지는 것으로 보인다.

**왜 고쳐야 하나.** 불안정한 회귀는 **회귀 자체를 못 믿게 만든다.** 이번에도 내 변경을
의심하느라 다섯 번을 다시 돌렸다. 이 저장소는 회귀 통과를 완료의 근거로 쓰므로 방치할
수 없다.

- [x] **Step 1~2: 주입된 clock으로 교체** — 세 곳 모두
      `older_than_seconds=0` → 시계를 60초 전진시킨 뒤 `older_than_seconds=30`.
      `_SteppingClock`을 테스트에 넣어 `LocalProjectStore(now=clock)`로 주입했다.
      **프로덕션 코드는 건드리지 않았다** — 결함은 테스트의 가정이었다

**틀렸던 가정.** 테스트에 *"A zero age threshold is deterministic here because the
claim timestamp is strictly earlier than a later clock read"*라는 주석이 달려 있었다.
Windows 시계 해상도(~15ms)에서는 두 읽기가 같은 값이 될 수 있어 `claimed_at < cutoff`가
거짓이 된다. 주석이 단언한 결정성이 성립하지 않았다.

**검증.** 수정 전 파일 쌍 실행 3회 중 3회 실패 → 수정 후 **5회 연속 통과**.
한 번 통과로 단정하지 않고 반복 측정으로 확인했다.

### Task 29: 분석이 `LM_STUDIO_BLOCKED`로 실패하는 조건 규명 — **완료 (2026-08-07)**
...
- [x] **Step 1: 실패 테스트** — 전송 계층이 타임아웃을 `blocked`가 아닌 코드로 분류하는지,
      진짜 연결 실패는 여전히 `blocked`인지, 그리고 타임아웃이 재시도 예약으로 가는지
- [x] **Step 2: RED 확인** — `assert exc.code == "timeout"`에서 `"blocked"`
- [x] **Step 3: 구현 — 전송 계층 분류만 고치면 됐다.**
      `_request_json_endpoint`가 `(HTTPError, URLError, TimeoutError, OSError)`를
      전부 `blocked`로 뭉갰고, 분석 워커는 `blocked`를 **재시도 없는 종료**로 처리한다.
      타임아웃만 `"timeout"` 코드로 분리하니 기존 재시도 백오프(5초 → 30초) 경로를 타게
      됐다. **워커 쪽은 손대지 않았다** — 이미 `blocked`가 아닌 오류는 재시도하고 있었다.
      urllib이 타임아웃을 직접 던지기도 하고 `URLError`로 감싸기도 해서 두 형태를 모두 인식한다
- [x] **Step 4: 회귀 + 커밋** — 백엔드 3106 passed(기존 불안정 1건은 Task 30)

**남은 한계.** 이 수정은 **연쇄를 끊지 못하고 완화만 한다.** 버려진 요청이 LM Studio를
계속 점유하는 것 자체는 그대로이므로, 재시도가 여전히 대기열에 걸릴 수 있다. 다만
5초·30초 뒤 두 번 더 시도하므로 그 사이 대기열이 비면 성공한다. 실제로 이번 세션의
모든 실패가 재시도 한 번으로 성공했다. **점유 자체를 없애려면 요청 취소나 동시성 제어가
필요하고, 그건 별도 판단이 필요하다.**
...
### Task 24: 편집 화면에서 B-roll 구간 손보기 — **완료 (2026-08-07)**
...
- [x] **Step 1: 실패 테스트** — 레지스트리·화면·전송 세 층 모두
- [x] **Step 2: RED 확인** — 3건 실패
- [x] **Step 3: 구현 — 세 곳이었다.**
      1. `inspectorRegistry`: B-roll에 `["inSec","outSec"]` 필드를 주고 `clearOnly`를 뗐다.
         페이드는 주지 않는다 — B-roll은 기본 무음이라 의미가 없다
      2. `InspectorControls`: 필드 목록에 따라 렌더하도록 바꿨다(예전엔 `clearOnly`
         하나로 갈렸다). 문구는 `쓸 구간 시작` / `쓸 구간 끝` — §10.13에 따라 `in_sec`
         같은 내부 용어를 화면에 쓰지 않는다. 끝이 시작보다 앞이면 저장을 막는다
      3. **`editorCommandPort.mediaControls()`가 `in_sec`/`out_sec`를 매핑하지 않아
         전송에서 값이 버려지고 있었다.** 화면만 만들었으면 조용히 무시됐을 것이다
- [x] **Step 4: GREEN + 검증 + 커밋** — 프론트 764 passed, tsc 통과.
      백엔드에도 `update_segment_broll_override`가 구간 값을 보존하고 무음 기본값
      (`preserve_source_audio=False`)이 유지되는지 확인하는 테스트를 더했다

**브라우저 실측은 하지 않았다 (사유를 남긴다).** 이 화면을 브라우저로 띄우려면 대본 →
기획 승인 → 초안 준비 → 초안 번들 생성까지 거쳐 실제 편집 세션을 만들어야 한다.
대신 **UI 렌더·동작·전송 본문·백엔드 보존을 각각 테스트로 고정**해 체인 전체를 닫았다
(전송 본문 테스트가 `in_sec`/`out_sec`가 실제 요청에 실리는 것을 단언한다).
owner가 실제 초안을 한 번 만들 때 눈으로 확인하는 것이 남은 몫이다.
...
## 2026-08-07 QA 결과 — 서브에이전트 3각도 조사 (Task 31~33)

owner 요청으로 서브에이전트를 세 각도(미연결 API 표면 / 저장만 하고 안 읽는 데이터 /
화면 흐름 막다른 길)로 돌렸다. **아래는 내가 직접 코드로 재확인한 것만 적는다.**
서브에이전트가 보고했으나 내가 확인하지 않은 항목은 마지막 절에 따로 분리했다.

### Task 31: 내보내기가 화면에서 불가능하다 — **완료 (2026-08-08, 커밋 `fd5bec945`)**

(2026-08-08 확인: 제목이 `미착수 · 최우선`으로 남아 있었으나 실제로는 닫혀 있었다.
`TimelineReviewPage.tsx:140`이 `api.approveTimeline`을 부르고 `검토 승인` 버튼이 있다.
아래 본문은 문제를 발견한 시점의 기록이므로 그대로 둔다.)

**계획서의 최종 인수 기준이 여기서 깨진다.** "owner가 ... 유진과 대화하며 고친 뒤
**내보낼 수 있다**"가 성립하지 않는다.
...
| 확인 | 결과 |
|---|---|
| `api.approveTimeline` (`api.ts:1637`) 호출처 | **프로덕션 코드 0건.** `scripts/verify_owner_path.py:190`과 테스트뿐 |
| 검토 화면 문구 (`TimelineReviewPage.tsx:140`) | "안전한 승인 계약이 준비될 때까지 승인과 추천 변경은 사용할 수 없어요." |
| 승인 버튼 부재를 고정하는 테스트 | `TimelineReviewPage.test.tsx:110-113` — 버튼이 **없어야** 통과 |
| 내보내기 게이트 (`OutputsPage.tsx:440-450`) | `review.review_status === "approved"` **및** `approval.review_status === "approved"` 요구 |

승인을 만들 UI가 없으므로 **자막·완성본·CapCut 버튼 3개가 브라우저에서 영구 비활성**이다.

**왜 여태 안 드러났나 — 이게 핵심이다.** backlog에 "9단계 전부 통과"로 기록된 검증은
`scripts/verify_owner_path.py`가 **파이썬에서 승인 함수를 직접 호출**해 통과시킨 것이다.
UI 경로를 검증한 근거가 아니다. **스크립트로는 되는데 사람 손으로는 안 되는 상태**를
아무도 구분하지 않았다.

**기다리는 대상은 실재한다.** `approval_workflow_contract.py`(17KB)가 이미 있고
테스트도 있는데(`tests/test_approval_workflow_contract.py`), **어느 라우터도 import하지
않는다.** 유일한 사용처는 `yujin_agent_package_contract.py:18`이다.

- [x] **Step 0: owner 확인 (선행)** — 확인 결과 의도적 보류가 아니었다. 승인 UI를
      붙이는 것으로 확정했다
- [x] **Step 1~: 승인 UI 연결** — `TimelineReviewPage`에 `검토 승인` 버튼을 붙이고
      `api.approveTimeline`을 화면에서 부르게 했다(커밋 `fd5bec945`). 브라우저에서
      `완성본 만들기` → `output.mp4 h264 1920x1080 10초`까지 확인했다

### Task 32: 보관한 프로젝트를 되돌릴 수 없다 — **완료 (2026-08-07)**

직접 확인한 근거:

- 보관 버튼은 화면에 있다: `ProductShell.tsx:68,70` (`보관` → `보관 확인`)
- `api.listProjects` (`api.ts:1563`)는 `/api/projects`를 **쿼리 없이** 호출한다
- 백엔드 기본값은 제외다: `routers/projects.py:32` `include_archived: bool = False`
- `api.restoreProject` (`api.ts:1569`)는 있으나 **호출처가 프로덕션에 0건**
- 백엔드 복원 엔드포인트는 살아 있다: `routers/projects.py:59`

**즉 보관 버튼을 한 번 누르면 그 프로젝트는 화면에서 영구히 사라진다.** 데이터는 남아
있으므로 손실은 아니지만, owner가 되돌릴 수단이 UI에 없다. Task 8에서 "보관 + 완전 삭제
이중 확인"을 완료로 닫았는데, **보관의 반대 동작이 빠져 있었다.**

- [x] **Step 1: 실패 테스트** — 보관함을 열면 목록이 뜨고 되돌리기가 동작하는지,
      비어 있으면 그렇게 말하는지
- [x] **Step 2: RED 확인**
- [x] **Step 3: 구현 — 세 곳이었다.**
      1. `api.listProjects(includeArchived)`가 `?include_archived=true`를 붙인다.
         **이게 없으면 화면을 만들어도 목록이 늘 비어 있다**
      2. 사이드바에 `보관함 보기` → 목록 → `되돌리기`. 열 때만 불러오므로 평소 경로의
         요청 수는 그대로다
      3. `AppRouter`의 **두 곳**(작업 화면·설정 화면)에 배선했다. 한 곳만 하면 반쪽이 된다
- [x] **Step 4: 회귀 + 커밋** — 프론트 768 passed, tsc 통과,
      백엔드 3107 passed (기존 불안정 1건은 Task 30)

**중간에 걸린 것 2건.** props 3개를 인라인으로 넘겼더니 `task22-parity-owners.test.ts`의
소스 패턴 검사(문자 거리 360자)가 깨졌다 → 관련 props를 `archive` 하나로 묶어 해결했다
(코드도 더 읽기 쉬워졌다). 그리고 `ProductShell.tsx` 헤더 규정대로
`docs/oss/editor-ui-source-map.json`의 `normalized_sha256` 2곳을 갱신했다 —
**이건 프론트 테스트로는 안 잡히고 백엔드 회귀에서만 잡힌다.**

**브라우저 실측은 하지 않았다.** 보관 → 보관함 열기 → 되돌리기 흐름은 테스트로 고정했고
`listProjects` 쿼리와 `restoreProject` 호출까지 단언한다. 실제 화면 확인은 Task 31의
승인→내보내기 실측과 함께 하는 것이 효율적이라 그때 같이 한다.

### Task 33: 숏폼(세로) 출력을 화면에서 고를 수 없다 — **완료 (2026-08-07)**

owner가 2026-08-07에 "롱폼 숏폼 다 만든다"고 확인했으므로 실사용을 막는 항목이다.

- 엔진·API·프론트 타입까지는 전부 있다(Task 17에서 완료)
- 그런데 `api.buildTimeline`(orientation을 받는 유일한 경로) **호출처가 0건**이고,
  실제 초안 생성 경로인 `createAtomicDraftBundle`의 요청 타입(`api.ts:46`)에는
  **orientation 필드 자체가 없다**
- `orientation="vertical"`로 실제 렌더를 돌려본 적도 없다 (Task 17이 스스로 기록)

**Task 17을 "완료"로 표기한 것이 부정확했다.** 백엔드 기준으로는 맞지만 owner가 쓸 수
있는가 기준으로는 미완이다. 앞으로 완료 기준을 **"owner가 화면에서 그 기능을 쓸 수
있는가"**로 잡는다.

- [x] **Step 1: 실패 테스트** — 초안이 기본 가로로 나오는지, 세로를 고르면 세로가 되는지
- [x] **Step 2: RED 확인 — 여기서 훨씬 큰 결함이 드러났다.**
      `assert (1080, 1920) == (1920, 1080)` — **초안이 세로로 나오고 있었다.**
- [x] **Step 3: 구현** — 저장소 → API 모델 → 프론트 타입 → 기획 화면까지 전 구간 연결.
      기획 화면에 `숏폼(세로)으로 만들기` 체크박스를 넣었다
- [x] **Step 4: 회귀 + 커밋** — 프론트 769 passed, tsc 통과,
      백엔드 3108 passed (기존 불안정 1건은 Task 30)

**F-9가 "해결됨"으로 닫혔는데 실제로는 안 고쳐져 있었다.**

F-9는 owner 결정("가로가 기본이어야 한다")에 따라 `build_timeline`이 orientation
미지정 시 landscape를 채우도록 고쳤다. **그런데 owner의 초안은 `build_timeline`을
거치지 않는다** — `materialize_atomic_draft_bundle`이 timeline dict를 직접 만들고,
거기에 `output` 키가 없어서 `CompositionPlan`의 기본값(`DEFAULT_OUTPUT_WIDTH=1080`,
`DEFAULT_OUTPUT_HEIGHT=1920` — 세로)으로 떨어졌다.

즉 **F-9 수정 이후에도 롱폼 포함 모든 초안이 세로로 렌더되고 있었다.** 고친 경로와
실제로 쓰이는 경로가 달랐던 것이다. 이 세션에서 찾은 "만들어놓고 연결 안 함" 패턴의
일곱 번째이고, **결과물에 직접 영향을 준다는 점에서 가장 나쁘다.**

**실제 세로 렌더 1회는 하지 않았다.** 각 층(저장소·API·프론트)을 테스트로 고정했고
`CompositionPlan.from_timeline`이 실제로 1920×1080 / 1080×1920을 돌려주는 것까지
단언했다. ffmpeg 실렌더 확인은 owner가 첫 영상을 만들 때 함께 본다.

### 서브에이전트가 보고했으나 내가 확인하지 않은 것

**그대로 믿지 말 것.** 착수 전에 각각 재확인이 필요하다.

- 유진 스트리밍 대화 4개 API와 `hermesSseClient.ts`(320여 줄)가 UI에 연결 안 됨 →
  응답 중단·재시도 버튼이 존재할 수 없음
- 에디터 라이브러리 프리셋·즐겨찾기·최근 5건 미연결. `PUT .../presets/{id}`는
  `api.ts`에 메서드조차 없음
- 미디어 라이브러리 설치 상태(`not_installed`/`degraded`)를 안 읽어서, 라이브러리가
  깨져도 사용자는 빈 목록만 봄
- 유진 추천 제외 설정(크리에이터·태그 제외) 미연결
- stale proposal에서 `refreshDirectorProposal` 복구 경로가 있는데 `blocked`로만 막음
- 자막 스타일 광범위 적용 전 영향 범위 미리보기 미연결
- `importBrollBatch`(디렉터리 일괄 반입) 미연결
- 백엔드 엔드포인트 약 28개가 `api.ts`에서 미호출
...
- [x] **Step 1: 각 항목이 정말 미사용인지 재확인 — 완료 (2026-08-08)**

      | 항목 | 재확인 결과 |
      |---|---|
      | `voice_samples` 테이블 | **죽었다.** 스키마 밖 참조는 `inputs/voice_samples` **폴더 경로**뿐이고 테이블 사용처는 0건이다 |
      | `list_media_analysis_cache` / `prune_stale_media_analysis_cache` | **죽었다.** 정의만 있고 프로덕션 호출처 0건 |
      | `hermes_capability_audit` | 화면 참조 0건 |
      | `provider_trace_failed_runs` | 화면 참조 0건 |
      | `director_proposal_lifecycle_events` | 화면 참조 0건 |
      | asset metadata `source_pack_id`·`in_app_only`·`provenance` 등 | 화면 참조 0건. `provenance`는 grep에 2건이 걸리지만 **다른 것**이다(`warning_provenance` 필드와 주석) |

- [ ] **Step 2~: 제거하거나 연결한다. 판단 근거를 남긴다** — 착수 시 `media_analysis_cache`
      부터 본다. stale 플래그를 **계산·저장하는데 읽지 않는다**는 것은 캐시 무효화가
      실제로 동작하지 않는다는 뜻일 수 있어, 지우기 전에 그 영향부터 확인해야 한다

### Task 35: 홈 화면이 실제 상태를 보여주지 않는다 — **완료 (2026-08-08)**

owner가 2026-08-07에 "대시보드 다 만든 거냐"고 물어 확인한 결과다.

`ProductShell.tsx:95-97`의 `HomePage`는 **props로 데이터를 받지 않고 API도 호출하지
않는다.** 카드 3개의 설명 문구가 전부 하드코딩이다.

| 카드 | 하드코딩된 문구 | 문제 |
|---|---|---|
| 최근 완성본 | "완성된 영상이 아직 없어요." | **완성본이 있어도 똑같이 뜬다** |
| 자산 준비가 필요한 프로젝트 | "대본에 맞는 사진·영상·소리를 추가해 주세요." | 자산이 충분해도 똑같이 뜬다 |
| 작업 중인 초안 계속하기 | "이어 할 작업을 선택해 편집을 계속하세요." | 초안이 없어도 똑같이 뜬다 |

즉 홈은 **이동 버튼 3개짜리 메뉴**이고 상태 표시는 장식이다. 사용자에게 **사실이 아닌
정보를 보여준다**는 점이 특히 나쁘다 — 완성본을 만들어도 홈은 없다고 말한다.

(설정 화면 `SettingsPage`는 실제로 동작한다. 로컬 설정과 `VoiceTtsSettings` 연결됨.)

- [x] **Step 1: 실패 테스트** — 완성본이 있어도 "없어요"라고 말하지 않는지
- [x] **Step 2: 거짓 표시 제거** — `최근 완성본` 카드가 상태를 단정하지 않고
      `출력 화면에서 완성한 영상을 확인할 수 있어요.`로 바뀌었다
- [x] **Step 3: 실제 개수 표시 — 완료 (2026-08-08)**

      owner가 "대시보드가 전부 완성돼야 테스트할 수 있다"고 해서 진행했다.
      **아래에 적힌 제약은 뒤집지 않고 우회했다.** 홈이 `listJobs`를 부르는 대신
      `GET /api/projects/{id}/home-summary` 하나만 부른다. 세는 일은 서버가 하고
      홈은 방문당 요청 한 번을 유지한다 — `ProductShell.test.tsx`가 고정한
      "작업 목록은 `작업 상태`를 열 때만"은 그대로 성립한다.

      | 카드 | 이제 말하는 것 |
      |---|---|
      | 작업 중인 초안 | `이어서 편집할 작업이 있어요.` / `아직 시작한 작업이 없어요.` |
      | 최근 완성본 | `완성한 영상이 N개 있어요.` / `아직 완성한 영상이 없어요.` |
      | 자산 준비 | `채울 자리가 N곳 남았어요.` / `필요한 자산이 모두 준비됐어요.` |

      요약을 못 읽으면 **상태를 단정하지 않는 원래 문구로 돌아가고** 카드는 계속
      눌린다. 세지 못하는 홈이 이동까지 막아서는 안 된다.

      브라우저 실측: `b-roll-smoke-test`는 `완성한 영상이 1개`,
      `progress-bar-live-test`는 `완성한 영상이 2개 / 아직 시작한 작업이 없어요`.
      프로젝트마다 다른 값이 나오는 것으로 하드코딩이 아님을 확인했다.

**실제 데이터를 넣으려다 설계 제약에 부딪혔다.** 처음에는 `api.listJobs`로 완성본 개수를
읽어 표시하도록 만들었는데, `ProductShell.test.tsx`가
**"사용자가 `작업 상태`를 열기 전에는 작업 목록을 부르지 않는다"**를 의도적으로 고정하고
있어서 기존 테스트 2개가 깨졌다(`expect(listJobs).not.toHaveBeenCalled()`).

이건 우연이 아니라 **의도된 속성**으로 보인다(로컬 우선/불필요한 폴링 회피). 그래서
**그 속성을 내 판단으로 뒤집지 않고**, 대신 해로운 부분 — 사용자에게 거짓을 말하는 것 —
만 제거했다. 나머지 두 카드(`작업 중인 초안`, `자산 준비`)도 여전히 고정 문구다.

**owner 결정이 필요한 지점.** 홈에서 실제 상태(완성본 개수, 초안 유무, 자산 부족 여부)를
보여주려면 **홈 진입 시 데이터를 불러오는 것을 허용**해야 한다. 지금 설계는 그것을
막고 있다. 대시보드다운 홈을 원하시면 그 제약을 푸는 결정이 먼저다.


### Task 36: 검토 화면이 실제 초안을 못 읽는다 — **완료 (2026-08-07)**

2026-08-07 승인→내보내기 실서버 검증에서 찾았다. **Task 31에서 승인 버튼을 달았지만,
그 화면이 실제 초안에서는 아예 로드되지 않는다.**

**근거 (실제 서버).** 실제 촬영본으로 기획 승인 → 초안 준비(ready) → 초안 번들
생성(`output_blocked=False`)까지 정상 통과한 뒤,
`GET /api/projects/{id}/timelines/{job_id}`가 **HTTP 500**을 냈다.
`TimelinePayloadResponse` 검증 오류 5건이다.

| 누락 필드 | 위치 | 이유 |
|---|---|---|
| `output_mode` | 타임라인 본문 | 번들은 이 값을 DB 컬럼에만 넣고 JSON에는 안 넣는다 |
| `asset_uri` | 자막 클립 2개 | 자막 클립에는 자산 파일이 없다 |
| `clip_type` | 자막 클립 2개 | 번들이 채우지 않는다 |

`TimelineReviewPage.tsx:65`가 `api.getTimeline`을 `Promise.all`로 부르므로, 이 500이
나면 화면은 그대로 오류 상태가 된다. **즉 owner는 승인 버튼을 볼 수조차 없다.**

**내 변경 탓이 아니다.** 검증 오류 목록에 Task 33이 추가한 `output`은 없다. 번들이
만드는 타임라인 payload와 조회 엔드포인트의 응답 계약이 **원래부터 어긋나 있었다.**
`test_atomic_draft_bundle.py`는 store를 직접 읽고, API 테스트는 번들이 만든 타임라인을
이 엔드포인트로 읽지 않아서 아무도 몰랐다.

**고칠 방향 (택일이 아니라 판단 필요).**

1. 번들이 `output_mode`를 payload에도 넣고, 자막 클립에 `clip_type`을 채운다.
   `asset_uri`는 자막에 없는 게 정상이므로 응답 모델에서 선택 항목이어야 한다
2. 또는 응답 모델을 실제 payload에 맞게 완화한다

**1번이 맞아 보이지만 자막 클립에 `asset_uri`를 억지로 만들어 넣는 것은 틀렸다.**
어느 쪽이든 계약을 한쪽으로 정리하는 결정이 먼저다.

- [x] **Step 1: 실패 테스트** — `test_api_atomic_draft_bundle.py`에 번들이 만든
      타임라인을 조회 엔드포인트로 읽고 승인까지 가는 테스트를 추가했다.
      **이 테스트가 없어서 지금까지 안 잡혔다**
- [x] **Step 2: RED 확인** — `3 validation errors for TimelinePayloadResponse`
- [x] **Step 3: 계약 정리 — 빠진 데이터와 과한 모델을 구분했다.**
      1. **데이터가 빠진 것**: 번들의 자막 클립에 `clip_type: "caption"`을 채웠다.
         B-roll 클립은 이미 `clip_type`을 선언하고 있었으므로 같은 관례를 따른 것이다
      2. **데이터가 빠진 것**: 번들 payload에 `"output_mode": "review"`를 넣었다.
         `save_timeline_run`이 payload에 이 값을 주입하는데(기본값도 `"review"`),
         번들은 그 함수를 거치지 않고 JSON을 직접 써서 빠져 있었다
      3. **모델이 과했던 것**: `TimelineClipResponse.asset_uri`를 선택으로 바꿨다.
         **자막은 텍스트로 그리므로 원본 파일이 없는 것이 정상이다.**
         여기에 억지로 값을 만들어 넣는 것은 틀린 방향이라고 판단했다
      4. 응답 모델에 `output`을 선언했다 — 없으면 Task 33이 채운 값이 응답에서 잘린다
      5. 프론트 타입도 맞췄다(`TimelineClip.asset_uri`, `TimelinePayload.output`)
- [x] **Step 4: 실서버 재검증 + 회귀 + 커밋** — 아래 참조

**실서버 역방향 검증 (2026-08-07, owner 실제 촬영본).**

| 단계 | 결과 |
|---|---|
| B-roll 반입 (Task 22) | 실제 촬영본 3개 전부 HTTP 201 |
| 기획 승인 → 초안 준비 | `ready` |
| 초안 번들 | `output_blocked=False` |
| **출력 규격 (Task 33)** | **`{'width': 1920, 'height': 1080}` — 가로** |
| **검토 승인 (Task 31)** | **HTTP 202, `review_status=approved`** |
| **자막 만들기** | **HTTP 202, `succeeded`** |

**내보내기 경로가 실제로 뚫렸다.** 이전에는 승인 화면조차 열리지 않았다.

**갭 검증에서 확인한 것.** `output_mode`를 `"review"`로 하드코딩한 것이 임의값이 아닌지
확인했다. `save_timeline_run`이 같은 기본값을 쓰고(`local_project_store.py:3799`),
DB INSERT도 `"review"`를 넣는다(`:2627`). 관례와 일치한다.

**남은 것.** 브라우저에서 실제로 승인 버튼을 눌러보는 것은 아직이다. API 계층은 전부
확인했고 화면 동작은 테스트로 고정돼 있다. owner가 첫 영상을 만들 때 함께 본다.

### 2026-08-07 실서버 검증에서 함께 확인한 것

- **Task 22 재확인**: 실제 촬영본 3개를 media-inbox에서 프로젝트로 반입, 전부 HTTP 201
- **Task 33 부분 확인**: 초안 번들이 `output_blocked=False`로 정상 생성됐다.
  출력 규격 값 자체는 위 500 때문에 API로 확인하지 못했다(단위 테스트로는 확인됨)
- **오진하지 않은 것**: 처음에 번들 생성이 `staging/...json` 없음으로 실패했는데,
  **데이터 경로가 150자로 길어서 생긴 Windows 경로 길이 문제**였다. 37자 경로로 옮기니
  바로 성공했다. 제품 결함으로 보고하지 않았다


### Task 37: 검토 스냅샷도 초안을 못 읽었다 — **완료 (2026-08-07)**

**브라우저 QA가 잡았다.** Task 36에서 타임라인 조회를 고친 뒤 API로는 승인까지
통과했는데, **실제 브라우저에서 검토 화면을 여니 여전히 "검토 내용을 불러오지
못했어요"가 떴다.**

검토 화면은 세 호출을 `Promise.all`로 함께 부른다. Task 36이 그중 하나만 고쳤다.

| 호출 | Task 36 이후 |
|---|---|
| `GET /timelines/{job_id}` | 200 (Task 36에서 고침) |
| **`GET /review-snapshots/{job_id}`** | **500** |
| `GET /review-approvals/timelines/{id}` | 200 |

**원인.** `SegmentAnalysisRecord`가 `confidence: float`와 `cleanup_decision: str`을
필수로 받는데, **무음 초안은 전사를 하지 않으므로 두 값이 모두 `None`이다.**
데이터가 빠진 게 아니라 **없는 것이 정상**인 값이라, `asset_uri`와 같은 판단으로
모델을 선택 항목으로 바꿨다. 프론트 `SegmentRecord` 타입도 맞췄다.

- [x] **Step 1~2: 실패 테스트 + RED** — Task 36 테스트를 확장해 검토 화면이 부르는
      세 호출을 모두 검증한다. `2 validation errors for SegmentAnalysisRecord`
- [x] **Step 3: 구현** — `confidence`/`cleanup_decision`을 선택으로
- [x] **Step 4: 브라우저 실측 + 회귀 + 커밋**

**교훈.** Task 36에서 "API로 승인이 202로 통과했으니 뚫렸다"고 판단했는데 **틀렸다.**
화면은 여러 호출을 함께 하고, 그중 하나만 실패해도 사용자에게는 아무것도 안 보인다.
**API 단건 확인은 화면 확인을 대체하지 못한다.**

---

## 2026-08-07 브라우저 QA — 전 구간 실측

owner가 다음 날 직접 테스트할 수 있도록 **실제 브라우저에서 클릭해 확인**했다.
API 계층만 보던 이전 검증과 달리, 이번에 Task 37을 잡아냈다.

| 화면 | 결과 |
|---|---|
| 홈 | 정상. 카드 3개는 여전히 고정 문구(Task 35) |
| 편집 작업판 | **정상.** 트랙·타임라인·자막·B-roll 클립이 실제 내용으로 표시됨 |
| 검토 | Task 37 수정 후 **정상.** 승인/다시열기 버튼 동작 |
| 출력 | **정상.** 자막·완성본·CapCut 세 버튼 모두 활성 |

**실물 확인 (제일 중요).** 출력 화면에서 `완성본 만들기`를 실제로 클릭했다.

```
final_render succeeded
→ exports/final_render/export_001/output.mp4
→ ffprobe: h264 1920x1080, 10.0초
```

**브라우저 클릭으로 실제 mp4가 가로(1920×1080)로 만들어졌다.** Task 31(승인),
Task 33(가로 기본값), Task 36·37(화면 로딩)이 전부 실물로 증명됐다.

### Task 38: 편집 화면의 내부 식별자 노출 — **완료 (2026-08-07)**

브라우저 QA에서 두 곳을 찾았다. `F-3`(내부 식별자 노출)이 해결됨으로 닫혀 있었는데
**이 두 경로에는 남아 있었다.** §10.13 위반이다.

| 위치 | 노출된 값 | 고친 방식 |
|---|---|---|
| 편집 작업판 `소스 확인` 라벨 | `NARRATION · segment_draft_0faacc4a45` | `내레이션 · 1번째 장면` |
| 타임라인 스냅 안내 | `스냅: 항목 시작 (caption:segment_draft_...:start, 0초)` | `스냅: 항목 시작 (0초)` |

- **역할 이름**: `track.role.toUpperCase()`로 영어 원문을 그대로 대문자화하고 있었다.
  한국어 라벨 표를 만들어 교체했다
- **장면 번호**: 세그먼트 내부 ID 대신 **클립 시작 시각 순서로 계산한 장면 번호**를
  쓴다. 검토 화면이 장면을 세는 방식과 같다
- **스냅 대상 ID**: `caption:<segment>:start`는 내부 키이고 사용자에게 아무 의미가 없다.
  종류와 시각만 남겼다

기존 테스트 9곳이 이 잘못된 문구를 고정하고 있어 함께 고쳤다.

**검증.** 네 화면(홈·편집·검토·출력)을 브라우저에서 열어 내부 식별자 패턴
(`segment_draft_*`, `editing_session_*`, `timeline_*`, `asset_*`, `clip_*`,
`NARRATION`, `OVERLAY` 등)을 훑었다. **네 화면 모두 0건.**

**남은 화면 문제 (owner 테스트를 막지 않는다).**

- 홈 카드 3개는 여전히 고정 문구다 (Task 35 — owner 결정 대기)
