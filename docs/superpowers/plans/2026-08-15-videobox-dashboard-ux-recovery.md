...
- [x] RED: "초안 있음"류 상태 문구가 화면에 **1회**, "편집 계속하기" CTA가 **1회**만 나옴을 고정한다. 다음 할 일 1개 + 상태 요약 1벌 + 유진 입력만 남는다. 카드 3장(자산/편집/완성본)은 상태 요약과 통합한다 — 이동 기능은 유지하되 같은 문구를 반복하지 않는다.
- [x] copy audit: 남는 문구 전부 §10.13 어휘 점검. 새 문구 없음 — 기존 승인 어휘 재조합만.
- [x] 실제 브라우저: 홈에서 편집·자산·출력으로 각각 이동 가능, 중복 문구 0.
- [x] 커밋 `fix: say each home status once` → 배포 → 확인.
...
- [x] RED: 라이브러리에서 선택한 영상 자산에 "구간 정리" 진입이 있고 그 자산이 선택된 채 `/footage`가 열림을 고정한다. 문구는 §10.13 audit.
- [x] 실제 브라우저: 왕복 경로.
- [x] 커밋 → 배포 → 확인.
...
### Task 4 완료 (2026-08-15 야간, `c29f4f064`)

실제 컨테이너 배포본, `/projects/my-project/home`에서 재측정(초안 있음·자산 준비
완료·완성본 0개 상태).
...
### Task 6 완료 (2026-08-15 야간, `d185aa166`)

실제 컨테이너 배포본 1920×1080에서 왕복 경로 전체 확인.

1. `/library`에서 영상 자산 카드 선택 → 미리보기 헤더에 "구간 정리하기" 링크
   등장(href `/footage?library_asset_id=<id>`)
2. 클릭 → `/footage`가 **그 자산이 선택된 채** 열림(미리보기 헤더에 파일명 표시)
3. 같은 화면에 "라이브러리에서 보기" 링크 등장(href `/library?library_asset_id=<id>`,
   같은 id)
4. 클릭 → `/library`가 **같은 자산이 다시 선택된 채** 열림
