...
env VIDEOBOX_RUN_LM_STUDIO_MEDIA_SMOKE가 없으면 skip한다. Slice 1 release 실행에서는 env를 켜고 현재 PC capability가 확인됐으므로 PASS가 필요하다. 이 gate에서 SKIP은 성공으로 세지 않는다.
...
구현 완료 HEAD, test totals, live profile key/variant, 분석 sample SHA, external call 0, artifact path를 기록한다. 모델명은 현재 PC smoke evidence로만 기록하고 배포 default로 고정하지 않는다.
...
- live LM Studio smoke는 현재 PC에서 실제 PASS하며 Gemini/external call은 0이다.
