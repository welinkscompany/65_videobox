...
### 4. 렌더("내보내줘") — **동작은 끝냄(2026-09-13), 답변 문구는 §6 참고**
...
### 6. 실물 채팅 확인 — **만들기·다시 만들기·펼치기는 끝냄, 렌더는 아직**
...
못박고 있는데, 이제 둘 다 사실이 아니다(2026-09-01 결정으로 직접 편집
의도는 이미 자동 적용되고, 이번에 렌더도 실제로 실행된다) — 근본 원인을
찾아 큐에 올렸다(`task_935fadb7`), 이번 세션에서는 안 고쳤다.

**이 문제는 숏폼만의 것이 아닐 수 있다** — 같은 시스템 프롬프트가 기존
16개 편집 의도의 대화 답변에도 쓰인다. 다음에 열 때 이것부터 볼 것.
