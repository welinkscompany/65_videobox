...
> **2026-07-21 publish-fence remediation 기록 (official checkbox/누적 동결):** `b781540ca`→`e27049fba`와 후속 bounded fence는 base/override B-roll·BGM·SFX, export overlay 및 virtual narration segment가 읽는 `narration_source_uri`의 실제 project asset을 snapshot해 final render와 exact preview publish 전에 다시 검증한다. `/segments/`는 narration track에서만 허용되고, direct legacy URI는 등록 asset에 역매핑하며 그 외는 fail-closed다. 큰 source의 full SHA와 completed MP4 copy는 writer transaction 바깥에서 한 번만 수행하고, transaction 안에서는 precomputed result·session CAS·size/mtime 재확인·atomic rename·DB pointer만 하므로 느린 rehash/copy가 편집 session write를 막지 않는다. affected runtime `172 passed`, fresh local fixture cold `387.5ms`(≤20초), warm `83.2ms`(≤500ms)를 확인했다. 이번 최종 verification에서는 frontend production build와 exact-preview E2E `5 passed`를 다시 실행했지만, 전체 Python regression은 **미검증**이며, Task 9/Task 11 사람 gate와 공식 누적 **9/22 (40.9%)**는 변하지 않는다.
