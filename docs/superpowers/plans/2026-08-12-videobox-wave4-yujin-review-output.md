...
- [x] Real browser: starter fills input; send proposal; preview then cancel proves zero mutation; resend, preview, apply exactly once; editor-side undo/reapply also verified against the live session revision and undo/redo counts.
- [x] Review both current default variants, edit master and prove stale, rebuild/approve. Real runtime rendered both variants independently after a sibling renderer failure; retrying one preserved the other failed card, both retries completed, both videos played, and both explicit result confirmations were recorded. The deterministic controlled sibling-failure/retry path remains covered by isolated Playwright.
- [x] Run the focused backend command (`252 passed, 1 warning` before the final output-lineage fixes; the affected backend regression set was rerun at `35 passed, 1 warning`), focused frontend command (`127 passed`), production build, full isolated Playwright E2E (`40 passed`) and targeted variant playback E2E (`2 passed`).
- [x] Fix the provider boundary for the actual LM Studio runtime: pass a flat grammar-compatible Yujin footage schema with `minItems: 1`, keep strict Pydantic/adapter validation authoritative, and keep the footage analysis request separate from the Yujin conversation request. Commits: `92616ea21`, `998f59683`, `228fe76aa`, `ccb02f5a`.
- [x] Close reverse-flow output gaps found in real runtime review: propagate an approved master review to new and reused variant materializations, allow derived variant timelines through final-render publish CAS while retaining source/review fences, and preserve non-requested sibling cards during per-variant retry. Commits: `be37f09f8`, `80bb95d6c`, `4cc258d23`.
- [x] Real browser at 1280x800 completed the Yujin footage flow on a duration-bearing source: starter fill → `candidate_only` → preview → cancel with unchanged draft revision → resend → preview → explicit apply. Final proposal GET was `approved`, revision `2`; browser requests were `201/200/200/200` for create/interpret/preview/approve and console had `0` errors / `0` warnings.
- [x] Regression after the final fixes: backend focused `252 passed`, frontend Wave4-focused `124 passed`, production build succeeded with the existing Vite chunk-size warning, and isolated Playwright E2E `40 passed` with snapshot manifest verified.
...
- [x] Real runtime browser smoke at 1280x800 covered root-to-`/projects` redirect, `/library`, `/footage`, project shell and output shell. Starter click filled the real request textbox without sending; console errors were `0`, output API calls were `200` with media `206`, and measured document width did not exceed the viewport.
- [x] Remaining Wave4 runtime gate closed: editor-side undo/reapply, master stale→rebuild→approve, independent horizontal/vertical render, sibling failure retention, per-variant retry, actual playback, and explicit confirmation were verified in the real 1280×800 runtime browser. The footage send/preview/cancel/resend/preview/apply path is verified above. Owner-ready Check is a runtime preflight only; it is not owner acceptance or human sign-off.
