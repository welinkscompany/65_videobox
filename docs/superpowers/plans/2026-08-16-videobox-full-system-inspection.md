...
1. **owner acceptance (최우선).** owner가 실제 긴 원본으로 만든 완성본을 처음부터 끝까지 직접 시청·청취하고,
   ...
   대신하지 못한다.
2. **Hermes 실제 provider 활성 상태에서의 확인.** 화면이 실제로 쓰는 유진 채팅 경로(로컬 qwen 모델)는
   2026-08-16 점검에서 **실제 메시지를 보내 라이브 응답으로 확인했다** — "이 영상 전체 분위기를 한 문장으로
   요약해줘"에 유진이 실제 생성 응답(구체 정보를 되물음)으로 답했고, `director/.../messages` POST가 200이었다.
   **다만 이것은 Hermes egress provider(9119, `openai-codex` OAuth)가 아니라 로컬 모델 경로다.** Hermes
   provider 자체의 로그인은 CLAUDE.md §6 승인 필요 항목이라 시도하지 않았고, 그 경로가 실제로 필요한 것은
   Mem0 기억 저장/검색뿐이다(§10.14). 화면 채팅이 막혀 있다는 뜻이 아니라, 이 특정 provider의 실사용 확인만
   여전히 owner 몫이다.
3. **UX 개편 보류 2건 (owner가 Task 1~6을 써본 뒤 결정).** 재료 전면 통합(라이브러리+촬영본+프로젝트 자산),
   5단계 단일 작업판 — 계획서 "하지 않을 것" 절에 근거와 함께 기록돼 있다.
4. **옛 자산 4개의 길이 데이터 backfill 여부.** `wave2-long-qa.mp4`, `wave2-short-a/b/c.mp4`는 여전히 duration
   데이터가 없다. 화면 문구는 "길이 정보 없음"으로 정직해졌다(owner 판단이 필요했던 원래 항목은 이것으로 이미
   해소됨). 조사 결과, **개인 라이브러리 자산에는 프로젝트 b-roll(`_backfill_broll_media_facts`)과 달리 자동
   재probe 경로가 아예 없다** — `library_ingest.py:142`의 `probe_metadata`는 최초 ingest 1회만 호출된다.
   4개 파일만 데이터를 채우려면 일회성 수동 재probe나 새 backfill 루프를 만들어야 하는데, 후자는 코드 추가이고
   전자도 4개뿐인 오래된 QA 자산에 실제 재생·편집에는 영향 없는 표시 문제라 우선순위 판단이 필요하다. **진행
   여부는 owner 결정.**
5. **(경미, 이번 세션에서 해결) 문서 낡음 2건** — UX 계획서 Task 7 체크박스와 핸드오프 시작점 HEAD 표기.
   `a630a7b2`로 고쳤다.

## 이 점검에서 실제로 한 것과 하지 않은 것

**이번 후속 턴에서 한 것** (`a630a7b2`, 코드 변경 없음, 전부 문서 또는 읽기 전용 확인):

- 위 발견 5의 문서 낡음 2건 수정
- Hermes/유진 라이브 채팅 실제 확인(로컬 경로, 위 발견 2)
- 옛 자산 4개 backfill의 기술적 실현 가능성 조사(위 발견 4) — 코드는 추가하지 않았다

**여전히 하지 않은 것, 이유와 함께:**

- **owner acceptance 대행 없음.** 시청·청취·취향 판단은 사람만 할 수 있다.
- **Hermes egress provider(9119) 로그인 없음** (CLAUDE.md §6 승인 필요 항목 — Claude가 임의로 진행하지 않는다).
- **오탐 테스트 수선 없음.** `test_maintenance_loop_cadence.py`의 시간 민감성은 알려진 패턴으로 기록만.
- **UX 보류 2건 구현 없음.** owner가 아직 Task 1~6을 써보지 않았고, 계획서가 명시적으로 "owner 별도 결정"으로
  남긴 항목이라 임의로 착수하지 않았다.
- **옛 자산 4개 duration backfill 코드 없음.** 위 발견 4처럼 실제 코드 추가가 필요한 작업이라 우선순위 확인
  없이 진행하지 않았다.
