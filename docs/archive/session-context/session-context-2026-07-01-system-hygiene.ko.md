...
- 현재 worktree 기준 backend full regression은 `314 passed`다
...
- backend full regression은 현재 direct 실행 기준으로 `314 passed`까지 다시 확인됐다
...
- 이후 fresh `pytest -q` 전체 회귀를 다시 캡처해 `314 passed in 765.44s`를 확인했고, 최신 full-backend baseline을 그 수치로 갱신했다
...
- approved timeline의 stale non-bool `segment.review_required` shape는 synthetic output blocker로 오판하지 않고 explicit review-required bool/string 값만 blocker로 인정하는 backend regression `1 passed`를 추가로 확인했다
...
- unknown dict-shaped `review_flag.code`는 approved timeline output gating blocker로 오판하지 않고 canonical review flag code만 blocker로 유지하는 backend regression `1 passed`를 추가로 확인했다
...
- current-focused helper 재검증 기준으로 backend output-gating slice는 현재 `18 passed`다
