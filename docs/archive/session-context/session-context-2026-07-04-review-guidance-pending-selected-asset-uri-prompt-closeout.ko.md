...
- review guidance pending selected asset uri prompt closeout

## 1. 이번 turn에서 실제로 끝낸 것

- `review guidance` prompt의 `pending_recommendations.payload.selected_asset_uri` surface가 whitespace stale 값을 raw 문자열 그대로 노출하던 작은 경계 1개를 닫았습니다
- `packages/core-engine/src/videobox_core_engine/review_guidance.py`에서 prompt 전용 pending recommendation row를 만들 때 nested `payload.selected_asset_uri`도 `strip()` 기준으로 정리하도록 맞췄습니다
...
- 그중 이번 slice는 이미 진행 중인 `review guidance` prompt canonicalization family와 직접 이어지고, TTS approval/output truth와 더 가깝게 맞닿는 nested `payload.selected_asset_uri` surface를 선택했습니다
- 단순 metadata trim보다 승인된 TTS asset uri truth와 직접 연결되는 점에서 우선순위가 더 높다고 판단했습니다

## 3. 이번 turn의 변경 범위

- `packages/core-engine/src/videobox_core_engine/review_guidance.py`
  - `_prompt_pending_recommendations(...)`에서 nested `payload.selected_asset_uri` trim 추가
- `tests/test_api.py`
  - `test_review_guidance_builder_trims_pending_recommendation_selected_asset_uri_in_prompt` exact regression 추가
- SSOT 문서
  - `docs/implementation-plan.ko.md`
  - `docs/development-status-2026-06-29.ko.md`
- closeout 문서 추가
  - `docs/session-context-2026-07-04-review-guidance-pending-selected-asset-uri-prompt-closeout.ko.md`
