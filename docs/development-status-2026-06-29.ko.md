...
- `A3 완료`: 고정 Hermes v0.18.2 source에서 `POST /auth/password-login`의 `basic` provider, session cookie, fresh `POST /api/auth/ws-ticket`, `/api/ws?ticket=...`, `session.create`→`prompt.submit`와 allowlisted `gateway.ready`/`message.delta`/`message.complete` envelope를 확인해 injected fake transport로 고정했다. workspace→gateway service credential, strict internal URL, no redirect/proxy, narrow NDJSON gateway와 public SSE를 추가했다. 동일 `LocalProjectStore`의 `director_hermes_runs`가 user-before-dispatch, atomic Postgres-compatible claim, stale CAS reclaim, owner-token-fenced terminal/assistant persistence를 소유한다.
- `A3 검증·실행 경계`: 신규 A3 5파일과 기존 Director focused gate는 **29 passed**, Compose verifier/start 회귀는 **71 passed**였다. real gateway auth env가 없으므로 live service start, live HTTP/WebSocket, OAuth/provider/chat/Mem0는 실행하지 않았고 통과로 주장하지 않는다. Phase 0은 **2/2 완료**, Phase A는 **3/4 완료**, 새 initiative는 **5/20 (25.0%)**, 잔여 **75.0%**이며 runtime/chat child는 **5/6 (83.3%)**, 잔여 **16.7%**다. 다른 child와 기존 공식 누적 **9/22 (40.9%)**, 잔여 **59.1%**는 바꾸지 않는다. 다음 task는 **`A4`만**이다.
