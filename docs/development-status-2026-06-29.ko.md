...
- `A2 검증`: package·mount·verifier·installer·start ordering 부재 RED는 **17 failed**였다. 대문자 executable 우회와 install 뒤 profile 미선택을 별도 RED 2건으로 재현해 case-insensitive 차단과 명시적 profile 실행으로 닫았다. 후속 scanner review는 delimiter 의존 user path·GitHub token·PSM1/MJS/shebang·일반 보안 문장 오탐을 **9 failed**, 확장자를 위장한 PE/ELF magic을 **2 failed**로 재현했다. 파일 확장자·선두 shebang/binary magic과 실제 token/credential assignment를 분리한 뒤 profile test는 **27 passed, 1 skipped**, A2와 기존 Yujin contract는 **109 passed, 1 skipped**다. 기존 handoff의 재현 불가능한 **193** 표기는 같은 고유 8파일 baseline의 **192 passed, 1 skipped**로 교정했고, scanner 보완 뒤 같은 exact 8파일 command는 **196 passed, 1 skipped**다. skip 1건은 현재 Windows 계정의 symbolic-link 생성 권한 부재이며 verifier의 reparse-point fail-closed 검사 자체는 정적·source test로 유지된다. profile/runtime/plan-state verifier와 `git diff --check`도 통과했다.
