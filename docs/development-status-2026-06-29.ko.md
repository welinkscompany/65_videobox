...
- `A3 리뷰 수정 완료`: 고정 Hermes v0.18.2 image source에서 TUI의 빈 toolset이 `None`(전체 허용)으로 바뀌고 bundled `kanban` plugin이 자동 활성화되는 fail-open을 확인했다. runtime은 `HERMES_TUI_TOOLSETS=context_engine`으로 비어 있지 않은 allowlist를 고정하고, compressor가 base `ContextEngine.get_tool_schemas()`를 그대로 상속하며 `get_tool_definitions(["context_engine"]) == []`임을 `--network none` one-shot verifier로 실행 증명했다. profile의 `cli: []`, compressor, no MCP, known/disabled `kanban`은 defense-in-depth다. prompt-only 방어에 의존하지 않는다.
- `A3 transport/output 경계`: Hermes와 gateway URL은 exact internal HTTP host/port만 허용하고 redirect/proxy/userinfo/path/query를 거부한다. 모든 prompt event는 exact session을 요구하며 tool/approval/file 계열 event, 비정상 completion status, empty final을 redacted blocked로 닫는다. gateway는 전체 upstream 출력을 quarantine한 뒤 credential/provider/path 패턴과 split-event 조합을 검사하고, 422와 NDJSON 오류는 입력·provider payload를 반사하지 않는다. service token은 양쪽에서 32 bytes 이상이며 placeholder를 거부하고, workspace는 계속 Hermes credential을 받지 않는다.
- `A3 durable/SSE 경계`: durable admission은 bounded registry/active/queue 예약 뒤에만 실행하고 sync store I/O는 `asyncio.to_thread`로 global async lock 밖에서 수행한다. CAS false/exception도 단 하나의 blocked terminal을 발행해 subscriber가 멈추지 않으며 terminal event는 event cap 때문에 재귀/교착하지 않는다. subscriber는 response header 전에 단일 예약되고 condition lock 밖에서 yield한다. active slot은 task별로 정확히 한 번 회수되고 lifespan shutdown이 task를 취소·회수한다. restart는 stale pending을 자동 재실행하지 않고 completed text를 durable message ID로 replay하며, reverse completion도 인접 행이 아닌 run의 user/assistant ID linkage를 사용한다. Postgres-compatible static UPSERT/CAS SQL contract를 보존했다.
- `A3 검증·실행 경계`: RED focused run은 다수 실패 뒤 CAS hang과 cancel-during-terminal-store hang을 재현했고, 수정 후 관련 gateway/run/SSE/Director/Compose/start gate는 **133 passed**다. `scripts/verify-hermes-yujin-zero-tools.ps1`, runtime/profile `-StaticOnly`, plan-state verifier가 모두 통과했다. 기존 CapCut lease test는 production 변경 없이 fixed sleep을 successful periodic renewal `Condition`으로 바꿨고 exact 10회, 파일 전체 **13 passed** 뒤 최종 full suite가 **1747 passed, 21 skipped**로 통과했다. 기존 Starlette multipart warning 1건은 실패가 아니다. 실제 service start, live HTTP/WebSocket, OAuth/provider/chat/Mem0, 실제 PostgreSQL 연결은 실행하지 않았다. Phase 0은 **2/2 완료**, Phase A는 **3/4 완료**, 새 initiative는 **5/20 (25.0%)**, 잔여 **75.0%**이며 runtime/chat child는 **5/6 (83.3%)**, 잔여 **16.7%**다. 다음 task는 **`A4`만**이다.
