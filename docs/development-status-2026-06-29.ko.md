...
- `A2 실행 경계`: real gateway auth env가 없으므로 live service start, live HTTP, OAuth/provider/chat/Mem0와 실제 profile install은 실행하지 않았고 통과로 주장하지 않는다. Phase 0은 **2/2 완료**, Phase A는 **2/4 완료**, 새 initiative는 **4/20 (20.0%)**, 잔여 **80.0%**이며 runtime/chat child는 **4/6 (66.7%)**, 잔여 **33.3%**다. 다른 child와 기존 공식 누적 **9/22 (40.9%)**, 잔여 **59.1%**는 바꾸지 않는다. 다음 task는 **`A3`만**이다.
