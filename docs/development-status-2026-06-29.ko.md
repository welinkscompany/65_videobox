...
> **2026-08-25까지의 기록이다.** (08-06 이후에도 08-08·08-13·08-17·08-18·08-25 항목이 더 붙었다.) **현재 상태는
> `CLAUDE.md` §2의 `최신 세션 인계` 줄이 가리키는 문서를 따른다** — 아래 두 문서 이름은 2026-08-06 당시 것이다(2026-09-07 점검).
> 옛 안내: 2026-08-06부로 이 파일에 새 turn별 closeout 항목을 추가하지 않는다. `CLAUDE.md`가
...
- `[x] Gate 1 코드 품질`: Task 23A–C와 Task 23D Task 1–3의 독립 리뷰·보완 이력을 통합 확인했다. 최종 spec/plan/gap과 reverse-runtime review는 각각 **C0/I0/M0**였다. 최종 quality review가 receipt 임시파일 동시 충돌, 짧은 commit provenance, Unicode C1 control을 Minor 3건으로 찾았고, 보완 중 write/flush 실패 후 partial temp cleanup Minor 1건도 추가로 찾았다. 네 항목을 RED→GREEN으로 수정한 뒤 최종 quality 재검토도 **C0/I0/M0 APPROVE**였다.
...
- `[x] Gate 4 전체 검증`: receipt 보완 뒤 owner-ready 전용 **112 passed, warning 1**, 전체 Python **2960 passed, 48 skipped, warning 1** (`1638.47s`)를 다시 통과했다. 직전 통합 감사의 전체 frontend **52 files / 733 passed**, production build **1850 modules**, full isolated Chromium E2E **35/35**, snapshot manifest, provenance/UI-system, external-runtime/network focused **2 files / 6 passed**, six static/non-live script 결과도 유지된다. 이번 후속 변경은 PowerShell 진단 도구·Python 테스트·문서에만 한정돼 frontend/build/E2E는 중복 실행하지 않았다.
...
- `진행률·경계`: Task 23 production slice는 **4/4 (100.0%)**, 잔여 **0.0%**다. Task 9 사람 acceptance와 live/provider/rights/publish/현재 CapCut Desktop edit-export는 계속 별도다. manual fallback은 유지된다. 최종 독립 review metadata와 hardening push까지 완료했다.
...
- `Git·보존`: 문서 closeout 시작 HEAD는 `5b1e0454f`이고 branch는 upstream보다 22 commit ahead였다. 구현·closeout 문서는 `bfe477aa3`까지 `origin/codex/videobox-container-compatibility`에 push했고 upstream `0/0`을 확인했다. `.tmp-final-fence-debug/`, `.tmp-real-video-dogfood/`, `apps/web/.tmp-real-video-dogfood/`, 첫 실패 시도 `artifacts/owner-sample-edit-20260803`(r1 별칭), 실패한 `artifacts/owner-sample-edit-20260803-r2`, `artifacts/owner-sample-edit-20260803-r3`, 성공 `artifacts/owner-sample-edit-20260803-r4`를 stage/remove/delete하지 않았고 `artifacts/` ignore를 확인했다.
...
- `[x] B5·Phase B 완료`: RightDock의 실제 conversation/typed proposal에서 사용자가 후보를 radio로 고르고 Apply를 누르기 전까지 편집 mutation은 0이다. Apply 뒤에는 route/director epoch와 current revision을 다시 확인해 기존 typed `EditorCommandPort` 명령을 정확히 한 번 실행하고 session/manifest를 새 revision으로 다시 읽는다.
- `one-player·상태 보존`: RightDock은 자체 player를 만들지 않고 기존 `PreviewCoordinator`와 sole `PreviewStage`만 사용한다. Inspector를 닫았다 열어도 conversation, draft, selected candidate, history scroll, player 상태가 route ownership 아래 보존된다.
- `실패·수동 fallback`: forged unsupported candidate, Hermes unavailable, stale completion과 output pending/running/failed/unavailable은 editor/output mutation 0으로 닫힌다. 기존 Director/editor의 수동 편집 fallback과 기존 output route는 계속 사용할 수 있다.
- `non-live·Live 경계`: fake typed Hermes와 real local VideoBox API/store를 쓰는 non-live smoke가 SSE 완료, proposal 저장, Apply 전 mutation 0, Apply 뒤 revision +1·caption 1개 변경, playback manifest, output readiness의 current→invalidated 전환, output job 0, external provider call 0을 확인한다. 별도 `-Live`는 이중 승인·credential·loopback·disposable project/sample copy를 요구한다. secret-gated nonce HMAC이 server canonical project root를 첫 POST 전에 묶고 raw path는 응답하지 않으므로, 같은 데이터의 다른 절대 root clone은 POST 0으로 fail-closed하며 source SHA를 보존한다.
- `코드리뷰·갭·역방향`: B4/B5 재검토에서 caption safe-area와 table bounds, segment caption-style playback 투영, image와 non-image candidate server attestation, TTS exact asset revision/file SHA, terminal CAS race, PostgreSQL 공통 lock order, candidate whitespace, Live root binding을 찾아 TDD로 수정했다. 실제 PostgreSQL 16에서도 변조 TTS terminal CAS가 전체 session write를 rollback했다. 최종 독립 spec 및 quality·gap·reverse review는 Critical/Important/Minor **0**, READY다.
- `검증`: focused frontend **7 files / 236 passed**, 전체 frontend **50 files / 657 passed**, editor Playwright E2E **9 passed**, production build, focused smoke **5 passed**, direct non-live smoke, 전체 Python **2139 passed, 21 skipped**(기존 Starlette multipart warning 1건), Hermes profile/runtime/20-ID plan-state/zero-tools verifier, Editor UI OSS provenance/UI-system verifier, `git diff --check`가 통과했다.
- `2026-07-30 재검증`: focused Python **83 passed**, focused frontend **3 files / 127 passed**, production build, non-live creator reverse smoke, 실제 PostgreSQL store **16 passed**와 targeted TTS terminal rollback **1 passed**, `git diff --check`, 독립 spec/quality/gap/reverse review가 통과했다.
- `미실행`: 실제 `-Live` happy path, live Hermes/provider, 다중 writer PostgreSQL 동시성 stress, 전체 Docker topology 실증, 사용자 원본 영상 재생·청취, 실제 CapCut Desktop 사람 검증은 실행하지 않았고 통과로 간주하지 않는다. frontend의 React `act(...)`, jsdom navigation, intentional ErrorBoundary stderr와 build의 500 kB chunk warning은 exit 0인 기존 비실패 출력이다.
- `진행률`: Phase B **5/5 (100.0%)**, creator-tools child **5/5 (100.0%)**, Hermes Yujin initiative **11/20 (55.0%), 잔여 45.0%**다. 기존 VideoBox 공식 누적은 Task 9 사람/환경 acceptance 전까지 **9/22 (40.9%)**, 잔여 **59.1%**로 유지한다.
- `다음 작업`: **C1만** 진행한다. durable run/event cursor와 final/interrupted conversation 복구를 TDD로 추가하되 current route/session/revision fence, duplicate suppression 준비, manual fallback, external provider call 0을 유지한다. reconnect/cancel/retry 전체(C2), capability lifecycle(C3), dashboard operations(C4), Mem0(D1–D4)는 선행 구현하지 않는다.
...
- `[x] B3 완료`: fresh creator context와 terminal transaction 안에서 session revision, asset-index revision, 실제 asset kind, media revision, source 존재와 SHA-256을 다시 확인한 `raw_video/broll_video` B-roll, BGM, SFX만 `ready/actionable`로 저장한다. 이미지와 B4 후보는 `candidate_only`/수동 적용으로 남고, 섞인 proposal에서도 actionable 후보만 freshness 대상이 된다.
- `명시적 단일 적용`: RightDock은 기준/현재 편집본, 실제 미디어 종류, 대상 장면, 지원 control과 적용 가능 상태를 보여 주지만 첫 후보를 자동 선택하지 않는다. 사용자가 radio로 후보 하나를 선택한 뒤에만 Route가 preflight와 materialize를 실행하고, await 뒤 route/director epoch와 current revision을 다시 확인해 `EditorCommandPort.applyMedia`를 한 번 호출한다. B-roll `contain/cover`는 `fit/crop`, BGM은 volume/fade, SFX는 volume만 전달한다.
- `우회·실패 경계`: Yujin direct REST `/apply`·`/batch-apply`는 proposal 단위로 mutation 전에 거부하고 legacy Director apply/batch는 유지한다. preview/materialize도 persisted metadata가 아니라 현재 DB asset kind/revision/source/SHA를 다시 확인한다. terminal CAS가 stale이면 모든 write를 rollback하고 proposal 없이 human reply와 manual fallback을 원자적으로 저장한다. display, stale completion, route 이동, materialize 실패, double-click에는 편집 mutation이 0 또는 명시적 한 번뿐이며 RightDock은 별도 player를 만들지 않는다.
- `범위 제한`: source copy, OpenCut runtime, provider/API 확대, Hermes-owned Mem0, SaaS, 자동 apply, 이미지 B-roll과 backend 미지원 effect는 추가하지 않았다. Task 9 사람/환경 acceptance와 실제 CapCut Desktop 실증도 별도다.
- `검증`: TDD RED→GREEN 뒤 controller fresh focused Python **323 passed, 1 skipped**(기존 Starlette multipart pending deprecation warning 1건), focused frontend **5 files / 126 passed**, production build, Hermes profile/20-ID plan-state/zero-tools verifier, Editor UI OSS provenance verifier, `git diff --check`가 통과했다. 독립 spec review와 독립 quality·gap·reverse review의 최종 판정은 Critical/Important/Minor **0** PASS다.
- `미실행`: 전체 Python regression, 전체 frontend suite, live provider canary, 실제 Hermes service-stop/manual environment proof, 실제 PostgreSQL/Docker 동시성, 사용자 원본 미디어 재생·청취와 CapCut 사람 검증은 실행하지 않았고 통과로 간주하지 않는다. Python의 1 skip은 실제 PostgreSQL 환경 미설정 경로이며 production build의 500 kB chunk warning은 기존 비실패 출력이다.
- `진행률`: Phase B **3/5 (60.0%)**, creator-tools child **3/5 (60.0%), 잔여 40.0%**, Hermes Yujin initiative **9/20 (45.0%), 잔여 55.0%**다. 기존 VideoBox 공식 누적은 Task 9 사람/환경 acceptance 전까지 **9/22 (40.9%)**, 잔여 **59.1%**로 유지한다.
- `다음 작업`: **B4만** 진행한다. backend가 실제 지원하는 caption, voice/TTS, overlay, output-check control만 typed recommendation과 명시적 current-revision apply/read-only 경계에 연결한다. 미지원 OpenCut effect, 자동 apply, provider 확대, Mem0, SaaS는 시작하지 않는다.
...
- `[x] A1 production 구현 시작`: 공식 digest의 `hermes serve --host 0.0.0.0 --port 9120`, health-only 비root FastAPI gateway, secret placeholder와 비파괴 targeted start/static verifier를 추가했다. 기본 `compose.yaml`은 Yujin 서비스·인증 보간 없이 기존 동작을 유지하고, `compose.hermes-yujin.yaml`과 `hermes-yujin` profile을 명시한 경우에만 A1 topology를 합친다. 하나의 flat network로는 workspace의 Hermes 직접 접근을 막을 수 없어 workspace↔gateway API internal network와 gateway↔Hermes internal network를 분리했고, Hermes만 provider egress를 가진다. Docker forwarding은 켜지 않으며 gateway process만 두 구간을 application level에서 잇는다. Gateway build context는 Dockerfile 전용 deny-all allowlist로 Dockerfile·requirements·gateway source만 전송 대상으로 다시 제한했다. 시작 검증은 raw `.env`를 재해석하지 않고 Compose가 해석한 모델의 정확한 환경 집합·값을 검사하며, 고정 Hermes 이미지의 공식 `_verify_password`를 `--network none`으로 실행해 plaintext/hash 관계까지 fail-closed 한다.
- `[x] A2 versioned profile package`: `videobox-yujin` 1.0.0 distribution에 한국어 우선·자동 적용 금지·VideoBox 근거 없는 preview/export 성공 주장 금지·미지원 effect의 실행 가능 제안 금지·수동 대체 절차를 고정했다. 첫 `videobox-editor` skill은 대화·확인 질문·수동 fallback만 소유하고 creator proposal과 도구 실행은 Phase B까지 열지 않는다.
- `[x] A2 container-only install·ownership gate`: package source는 opt-in overlay에서 `/opt/videobox-yujin-profile`로 read-only bind되고 OAuth state `/opt/data`와 분리된다. 정적 verifier는 exact manifest, 실제 owned file, undeclared executable, secret/API/OAuth/password/Mem0/local absolute user path, traversal·reparse point를 fail-closed 한다. 시작 스크립트는 Compose config와 password/hash 검증 뒤 profile static verifier를 실행하며, `ValidateOnly`는 이 세 검증 뒤 설치·서비스 mutation 0회로 끝난다. 실제 경로는 기존 Hermes running 상태 조회 → 고유 one-off installer container의 idempotent `hermes profile install ... --force -y` → 필요할 때만 명시적 `-p videobox-yujin` Hermes runtime → gateway 순서다. gateway 실패 시 이 실행이 새로 시작한 Hermes만 `compose stop`으로 회수하고 기존 running Hermes는 건드리지 않는다. host Hermes profile은 설치 대상이 아니다.
- `A2 검증`: package·mount·verifier·installer·start ordering 부재 RED는 **17 failed**였다. 대문자 executable 우회와 install 뒤 profile 미선택을 별도 RED 2건으로 재현해 case-insensitive 차단과 명시적 profile 실행으로 닫았다. 후속 scanner review는 delimiter 의존 user path·GitHub token·PSM1/MJS/shebang·일반 보안 문장 오탐을 **9 failed**, 확장자를 위장한 PE/ELF magic을 **2 failed**로 재현했다. 다시 denylist의 구조적 한계를 `.hta/.rb/.pl/.php/.txt/.json`과 허용 확장자 선두 shebang **7 failed**로 재현하고, Phase A package content type을 `.md/.yaml/.yml`로만 allowlist했다. 모든 허용 파일도 선두 shebang·PE·ELF magic을 검사하며 Markdown fenced shebang은 문서 내용으로 허용한다. 최종 profile test는 **34 passed, 1 skipped**, A2와 기존 Yujin contract는 **116 passed, 1 skipped**다. 기존 handoff의 재현 불가능한 **193** 표기는 같은 고유 8파일 baseline의 **192 passed, 1 skipped**로 교정했고, content-type 보완 뒤 같은 exact 8파일 command는 **203 passed, 1 skipped**다. skip 1건은 현재 Windows 계정의 symbolic-link 생성 권한 부재이며 verifier의 reparse-point fail-closed 검사 자체는 정적·source test로 유지된다. profile/runtime/plan-state verifier와 `git diff --check`도 통과했다.
- `A2 quality fail-closed 보완`: 신규 RED는 profile/content 쪽 **12 failed**, startup/state 쪽 **6 failed**였다. canonical `.venv`의 PyYAML SafeLoader helper가 모든 owned YAML의 parse·unsafe tag·duplicate key와 mapping/sequence/alias 내부 sensitive key를 구조적으로 검사하며, 허용 파일은 UTF-8(BOM 허용)을 strict decode하고 NUL·제어문자·ZIP 위장 payload를 거부한다. installer 기본 이름은 GUID one-off이고 실패 시 그 이름의 residue만 best-effort 제거한다. 최종 profile test는 **49 passed, 1 skipped**, 핵심 두 파일은 **99 passed, 1 skipped**, 같은 exact 8파일 command는 **222 passed, 1 skipped**다. profile/runtime/plan-state verifier, Python compile, PowerShell parse, `git diff --check`가 통과했다. Ruff는 canonical `.venv`에 설치되지 않아 실행하지 않았으며 전체 Python suite와 live start/install/HTTP/provider/OAuth/chat/Mem0는 실행하지 않았다.
- `A2 verifier runtime·persistent recovery 보완`: host-side static verifier dependency는 `requirements-dev.txt`의 exact `PyYAML==6.0.3` 한 곳에만 선언했고 현재 canonical `.venv` 설치 버전과 일치한다. installed distribution metadata 기준 license는 MIT, homepage는 `https://pyyaml.org/`, source는 `https://github.com/yaml/pyyaml`이며 외부 fetch나 runtime container dependency 추가는 없었다. missing import는 traceback·절대경로·입력값 없이 generic failure로 끝난다. Persistence contract: profile install persists in the videobox_hermes_oauth_state named volume at /opt/data; rerun uses --force idempotently; service cleanup only; the named volume is not deleted. gateway 실패 cleanup은 새로 시작한 service의 `compose stop`까지만 수행하므로 full rollback이나 profile/volume 삭제로 표현하지 않는다. 신규 focused GREEN은 **3 passed + 3 passed**, profile test는 **52 passed, 1 skipped**, 핵심 두 파일은 **102 passed, 1 skipped**, 같은 exact 8파일 command는 **225 passed, 1 skipped**다.
- `A2 post-install 상태 보완`: 신규 RED **3건**으로 installer command 실패, successful install 뒤 첫 Hermes `up` 실패, recovery 정의 순서를 고정했다. installer 실패는 `/opt/data` named volume에 partial profile이 남았을 수 있다고만 말하고 exact one-off container residue만 정리하며, successful install 뒤 runtime/gateway 실패는 profile이 persist한다고 말한다. 둘 다 `--force` idempotent rerun과 service-only/no-volume-delete를 안내하고 secret·입력 경로를 출력하지 않는다. focused GREEN은 **1 passed + 2 passed**, 핵심 두 파일은 **103 passed, 1 skipped**, 같은 exact 8파일 command는 **226 passed, 1 skipped**다.
- `A1 검증`: 최초 focused RED는 서비스·파일 부재로 **10 failed, 6 passed**, GREEN은 **16 passed**였다. build-context 리뷰 보완 RED는 전용 ignore 파일 부재로 **1 failed, 16 passed**, GREEN은 **17 passed**였다. opt-in topology 리뷰 RED는 **8 failed, 10 passed**, GREEN은 **18 passed**였고, capability authority 동기화 RED는 **2 failed, 3 passed**, GREEN은 **5 passed**였다. credential follow-up RED는 malformed/resolved env 계약에서 **3 failed, 8 passed**였으며, 별도 characterization RED에서 기존 raw parser가 미해결 `${MISSING}` 값을 fake Docker까지 넘겨 exit 0을 반환하는 것을 재현했다. Compose rendered model과 고정 Hermes verifier 적용 뒤 credential suite는 **12 passed**, A1 focused suite는 **23 passed**, Compose/plan-state 회귀를 합친 관련 gate는 **45 passed**다. child-process dummy environment의 Compose config, `scripts/verify-hermes-yujin-runtime.ps1 -StaticOnly`, plan-state verifier, `git diff --check`를 통과했고 전체 Python 회귀도 **1597 passed, 20 skipped**였다. 기존 Starlette multipart deprecation warning 1건은 실패가 아니다. 실제 `.env.container`와 usable credential은 만들지 않았다.
- `A3 lifecycle correction 완료`: 반복 lifecycle race 검토에서 admission/dispatch/terminal/notification task 소유권과 active-slot 회수가 여러 coroutine에 분산된 근본 원인을 확인했다. run은 `queued → starting → active → terminalizing → terminal`의 명시적 상태와 별도 `active_slot`을 가지며 admission/dispatch/terminal/cleanup task는 모두 tracked done-callback과 shutdown fixed-point에 포함된다. 슬롯 회수·queue 승격은 global lock 아래 단일 idempotent 함수만 수행하고 closing 뒤에는 승격·gateway start가 불가능하다. durable begin과 terminal CAS는 global lock 밖의 owned task이며 caller 취소가 commit을 가장해 취소하지 않는다. terminalization은 공유된 shielded task 하나만 소유해 반복 취소에도 terminal event를 한 번만 발행한다. subscriber 알림은 untracked notify task 대신 lock 아래 동기 `Event.set()`을 사용한다.
- `A3 이전 검증 근거 (현재 완료 근거 아님)`: 고정 Hermes v0.18.2 image source에서 TUI의 빈 toolset이 `None`(전체 허용)으로 바뀌고 bundled `kanban` plugin이 자동 활성화되는 fail-open을 확인했다. runtime은 `HERMES_TUI_TOOLSETS=context_engine`으로 비어 있지 않은 allowlist를 고정하고, compressor가 base `ContextEngine.get_tool_schemas()`를 그대로 상속하며 `get_tool_definitions(["context_engine"]) == []`임을 `--network none` one-shot verifier로 실행 증명했다. profile의 `cli: []`, compressor, no MCP, known/disabled `kanban`은 defense-in-depth다. prompt-only 방어에 의존하지 않는다.
- `A3 transport/output 경계`: Hermes와 gateway URL은 exact internal HTTP host/port만 허용하고 redirect/proxy/userinfo/path/query를 거부한다. 모든 prompt event는 exact session을 요구하며 tool/approval/file 계열 event, 비정상 completion status, empty final을 redacted blocked로 닫는다. gateway는 평상시 최소 256자의 rolling quarantine tail을 유지하면서 안전한 prefix를 completion 전에 32,000-byte 이하 delta로 증분 발행한다. Authorization/Cookie/Bearer/token/password/API key/OAuth/Mem0/provider의 민감 label 또는 복합 prefix가 보이면 최초 시작점부터 공백·구분자 구문이 안전하게 해소될 때까지 상태형 quarantine을 고정한다. 따라서 256자를 넘는 임의 공백과 event split 뒤에도 assignment/header/value를 방출 전에 차단하고, 미해소 quarantine이 4,096 bytes를 넘으면 고정 길이를 늘려 허용하지 않고 fail-closed한다. `/opt/data`, `/videobox-data`, `/etc`, `/home`, Windows absolute path의 모든 fuzz-like split도 값 방출 전에 차단하며, 일반 장문과 민감 label을 단순 설명하는 안전한 문장은 completion 전 증분 발행을 유지한다. completion text는 이미 발행된 prefix와 일치하는 exact sanitized truth여야 한다. assembled text 200,000 bytes, public events 512, API-client line/event/delta 누적 상한을 각각 고정했다. service token은 양쪽 Python과 start verifier에서 32 bytes 이상·서로 다른 문자 8개 이상이며 placeholder를 거부한다.
- `A3 durable/SSE 경계`: durable admission은 bounded registry/active/queue 예약 뒤에만 실행하고 sync store I/O는 `asyncio.to_thread`로 global async lock 밖에서 수행한다. create-vs-shutdown, active+queued shutdown 50회, dispatch pre-entry cancel 50회, duplicate waiter cancel, terminal store 반복 취소를 고정해 closing 후 gateway 호출 0, active/waiting 0, durable pending orphan 0을 검증했다. subscriber는 response header 전에 단일 예약되고 global lock 밖에서 yield한다. restart는 stale pending을 자동 재실행하지 않고 completed text를 durable message ID로 replay한다. 다만 process-start에서 모든 orphan `pending/streaming`을 `interrupted`로 sweep하고 event cursor를 복구하는 기능은 Phase C C1 pending이며 A3 완료 범위가 아니다.
- `A3 correction 검증·실행 경계`: lifecycle·streaming·entropy·verifier 신규 RED는 각각 슬롯 누수/closing 후 2번째 gateway 호출/durable pending orphan, completion barrier timeout/split marker 노출, low-distinct token 허용, 실제 pinned image의 `NameError: context_engine is not defined` quoting loss를 재현했다. 수정 후 A3/A1/A2 교차 focused gate는 **217 tests, failures 0, errors 0, skipped 1**이고 runtime/profile `-StaticOnly`, plan-state, 실제 `--network none` zero-tool verifier가 모두 통과했다. 해당 lifecycle correction 당시 full suite는 **1775 passed, 21 skipped, warning 1건**으로 **757.05초**에 통과했다. warning은 기존 Starlette multipart pending deprecation이다. 실제 service start, live HTTP/WebSocket, OAuth/provider/chat/Mem0, 실제 PostgreSQL 연결은 실행하지 않았다. 현재 Phase 0은 **2/2 완료**, Phase A는 **3/4 완료**, 새 initiative는 **5/20 (25.0%)**, 잔여 **75.0%**이며 runtime/chat child는 **5/6 (83.3%)**, 잔여 **16.7%**다. 다음 task는 **`A4`만**이다.
- `A3 stateful quarantine Critical correction (2026-07-26)`: `token` + 300 spaces + `=PRIVATE` + 300 chars를 delta로 나누면 기존 256자 tail이 label을 먼저 방출하고 나중 delta에 `PRIVATE`까지 방출한 뒤에야 completion에서 blocked 되는 RED를 재현했다. 수정 후 Authorization/Cookie/Bearer/API key/password/OAuth/Mem0/provider와 경로 변형, 복합 label 내부·delimiter 앞 300자 초과 공백, 모든 문자 split point, 4,096-byte 미해소 상한에서 sentinel이 어떤 public delta에도 나타나지 않는다. 일반 장문과 안전한 설명 문장은 completion barrier 전 증분 발행을 유지한다. gateway focused는 **64 passed**, client/RPC/conversation/run-service/zero-tool contract 관련 묶음은 **118 passed**, 최신 full suite는 **1818 passed, 21 skipped, warning 1건**으로 **746.63초**에 통과했다. runtime/profile `-StaticOnly`, plan-state, 실제 pinned image `--network none` zero-tool verifier도 재통과했다. 실제 service start, live HTTP/WebSocket, OAuth/provider/chat/Mem0, 실제 PostgreSQL 연결과 push는 실행하지 않았다.
- `capability authority 분리`: A1의 optional gateway는 `health_only`, 두 internal network topology는 `deployed`이지만 capability issuance·private signing key·owner revocation writer·capability gateway route는 모두 계속 비활성/`not_deployed`다. 기존 preauth Hermes의 `network_mode: none`도 Yujin transport topology와 별도 상태로 유지한다.
- `A1 quality 재검토`: captured stdout을 먼저 끝까지 읽던 시작 helper가 대량 stderr에서 교착할 수 있었다. 1 MiB stderr의 captured validation·정상 start·실패 start 세 경로가 각각 8초 timeout에 걸리는 RED와 workspace alias가 username/plaintext/hash를 숨겨도 exit 0이던 RED 3건을 재현했다. capability 주석·static alias RED 2건도 별도로 확인했다. captured 검사는 stdout/stderr를 비동기로 동시에 drain하고 실제 `up`은 native argument array와 inherited streaming을 사용한다. workspace는 key뿐 아니라 정규화된 전체 resolved environment 값에도 credential 세 값이 0회인지 검사한다. static verifier도 dummy credential이 serialized workspace environment에 0회인지 확인한다. 신규 startup/alias GREEN은 **6 passed**, capability 주석/static alias GREEN은 **2 passed**, capability authority를 포함한 최종 관련 gate는 **58 passed**다. 이번 follow-up에서 전체 Python suite는 다시 실행하지 않았으며, 위 **1597 passed, 20 skipped**는 직전 credential commit의 검증 근거다.
- `A1 비차단 supply-chain 잔여`: agent-gateway의 `python:3.12-slim` base는 아직 digest pin이 아니고 A1 전용 Python SBOM도 생성하지 않았다. 이번 startup 안전성 fix에서 범위를 확장하지 않았으며 supply-chain closeout으로 주장하지 않는다.
- `A1 secret alias 오탐 수정`: 전체 workspace environment JSON의 substring을 찾으면 정상 username `videobox`가 property key나 `DATABASE_URL` 일부에 있는 경우까지 거부했다. start/static 각각 benign key·substring RED 1건을 재현했고, username/plaintext/hash exact value alias 3종은 기존대로 거부되는 것도 함께 고정했다. 두 스크립트는 공용 environment contract helper를 사용해 property key를 제외하고 scalar value만 ordinal-exact 비교한다. array/object/null 등 scalar map이 아닌 값은 fail-closed다. 최종 관련 gate는 **62 passed**다.
- `A1 array flatten 보완`: PowerShell pipeline이 property array를 평탄화해 빈 배열·1개 배열·2개 배열을 scalar처럼 통과시키던 `ValidateOnly` RED 3건을 재현했다. 공용 helper는 property의 raw `.Value`를 enumeration 전에 직접 검사해 string 외 `IEnumerable`, null, object를 password verify 전에 fail-closed 한다. 최종 관련 gate는 **72 passed**다.
- `A1 composite secret·password 길이 보완`: username은 ordinal-exact alias만 차단해 정상 `videobox` substring을 허용하지만 plaintext password와 hash는 scalar 값 안 어디에 포함돼도 차단한다. start composite RED 4건과 길이 1–11 RED 11건, static composite RED 4건을 재현했고 길이 0은 기존 empty 검사로 이미 차단됐다. PowerShell string `Length` 기준 최소 12자를 password/hash verifier 전에 강제한다. static verifier의 base/merged Compose capture도 stdout/stderr를 비동기로 동시에 drain한다. 최종 관련 gate는 **93 passed**다.
...
- fresh verification: affected runtime suite `172 passed`(기존 multipart PendingDeprecationWarning 1), exact local 10초·1280×720 fixture cold `387.5ms`(≤20초), warm cache lookup `83.2ms`(≤500ms). 이번 최종 verification에서는 frontend production build와 exact-preview E2E `5 passed`를 다시 실행했지만, 전체 Python regression은 다시 실행하지 않았다. 특히 사용자가 앞서 중단한 full Python regression은 **미검증**이며 focused pass를 full-pass로 해석하지 않는다.
...
## 2026-08-17 closeout — 캡컷 벤치마킹으로 편집기가 집이 됐다

인계 문서: `docs/handoffs/2026-08-18-videobox-next-session.ko.md`
승인 기록: `docs/decisions/2026-08-17-editor-capcut-layout-approval.ko.md`
커밋: `83261f4e5`(구현), `f7cb56939`·`83d8cc810`(문서·인계)

owner: *"그냥 캡컷하고 인터페이스를 벤치마킹해."* 그 앞의 진단은
`docs/handoffs/2026-08-17-videobox-first-use-walkthrough.ko.md`에 있다 —
**컷편집 기능이 없는 게 아니라 부를 수 있는 자리가 없었다.**

### 들어간 것

- **편집기가 집이 됐다.** 프로젝트를 열면 편집 화면이 먼저. 사이드바도
  `편집 · 이야기 · 재료 · 확인과 내보내기` 순. 첫 화면(`/projects`)도 앱 껍데기 안으로.
- **빈 편집판.** `POST .../editing-sessions/blank` 신설. 기획을 통과하지 않고 편집기를
  연다. 완전히 빈 세션이 아니라 비어 있는 5초 장면 하나로 열고 `review_required`로
  표시해 채우기 전에 완성본으로 나가지 않게 막는다.
- **컷 도구가 툴바에.** 나누기 · 앞과 붙이기 · 빼기. `Ctrl+B` / `Delete`도.
- **재생 위치가 보인다.** 눈금부터 트랙까지 관통하는 세로선(승인된 accent).
- **재료를 끌어다 놓는다.** 저장소에 `onDrop`이 하나도 없었다.
- **읽을 수 있는 표기.** 기계 시각 → `2시간 전`, 내부 단계 이름 → 사람 말.
- **자산 분류.** 종류 탭 안에 검색·정렬·즐겨찾기만 보기.

### 검증에서 잡은 결함 (전부 실제 화면에서)

단위 테스트가 **전부 초록인 채로** 컷편집이 막혀 있었다. 이번에도 같은 교훈이다.

- **재생 위치가 첫 클릭 자리에 붙박였다.** 원인 두 겹 — 타임라인과 소유자가 서로
  되돌리는 고리, 그리고 미리보기가 자기 구간 밖 위치를 도로 밀어 올리는 동작
  (테스트가 그걸 의도된 계약으로 못박고 있었다).
- **한 장면을 골랐는데 두 개가 골라졌다.** 고른 직후의 seek이 선택을 덮어썼다.
- **7초를 눌렀는데 5~7초 장면이 잡혔다.** 선택을 자막에서 유도하는데 나눈 뒤 자막과
  장면이 어긋난다. 단, 내레이션이 긴 통짜 하나일 때는 자막이 의미 단위다.
- **화면에 raw clip ID가 남아 있었다**(§10.13). 옆에 이미 사람이 읽는 이름이 있었다.
- **세션만 만들면 편집기가 안 열린다.** 짝이 되는 타임라인이 있어야 한다.
...
| 실제 컨테이너 | 콘솔 오류 0 · 가로 넘침 0 · 내부 ID 노출 0 |

### 남긴 것 — 셋 다 "값 하나 바꾸기"가 아니다

1. **왼쪽 재료 패널 기본 펴기** — 승인 있음. **세 번 시도해 세 번 되돌렸다.** 단위
   테스트 해법은 검증됐고(헬퍼 하나), 막힌 곳은 e2e다. 기본값이 바뀌면 layout mode가
   통째로 달라져 도크 폭·스냅샷이 함께 움직인다.
2. **미리보기/타임라인 비율** — `exact-preview.spec.mjs:94`와 2026-07-22 결정을 함께
   열어야 한다. owner 판단 필요.
3. **데스크톱 껍데기** — 웹이라서 생기는 진짜 불편은 이것뿐(도커·주소·파일 경로).

이 closeout은 자동 검증과 실제 컨테이너 확인을 정리한 것이며, owner acceptance
완료 보고가 아니다.

## 2026-08-18 closeout — 남겨 둔 셋 중 둘이 닫혔다

`2026-08-17`이 남긴 셋(A 왼쪽 재료 패널, B 비율, C 데스크톱 껍데기)을 이어서 했다.
자세한 것은 `docs/handoffs/2026-08-18-videobox-capcut-close-and-one-icon-launch.ko.md`.

### A — 왼쪽 재료 패널 기본 펴기 (닫힘, `551fe5123`)

네 번째 시도에서 들어갔다. 막고 있던 것은 기본값이 아니라 **그 기본값을 전제로
쓰인 테스트들**이었고, 값이 아니라 무엇을 지키려는 테스트인지를 보고 다시 쓰니
풀렸다.

세 번째 시도가 왜 실패를 늘렸는지도 나왔다 — **첫 프레임에는 작업판 폭이 아직 0이라
잠깐 `drawer`로 잡히고, 그때는 펴져 있는 도크도 DOM에 없다.** 그 순간에 재면 열린
것을 닫힌 것으로 읽고 눌러서 닫아 버린다.

### B — 비율 (절반, `83ddba9f3`)

키우는 게 문제가 아니라 **폭 경계가 깨져 있었다.** 같은 900px 높이에서 1600px
화면이 1440px 화면보다 미리보기를 107px 작게 그리고 있었다(407 → 300). 상한이
1499px를 경계로 두 벌이었고 서로 맞지 않았다.

막으라고 있던 가드는 1440x900과 1920x**1080**만 비교해서, 늘어난 화면 높이에 가려
못 봤다. **높이를 고정하고 폭을 비교**하도록 다시 썼다.

타임라인은 1920x1080에서 128px → 190px. 미리보기는 어느 폭에서도 줄지 않는다.
**아래 절반은 아직 아니다** — 그건 비율 정책이고 owner 판단이 필요하다.

### 재다가 잡은 결함 (전부 이번 변경 이전 것)

또 단위 테스트 1,033개가 초록인 채였다.

- **390x844에서 미리보기 영상이 0px.** 2026-08-17에 상한을 40vh로 올렸을 때
  타임라인이 364px를 먹고 밀어냈다. 승인된 스냅샷은 미리보기가 빈 상태라 무너질
  영상이 없었고, 그래서 아무도 못 봤다.
- **툴바 버튼 일곱 개 중 뒤 여섯 개가 화면 밖.** `overflow:hidden`이라 밀어 볼 수도
  없으니 폰에서는 재료 열도 유진이도 열 수 없었다.
- **작업판 전체가 390px보다 넓었다.** 툴바가 `min-width:auto`로 부풀어서.

### C — 데스크톱 껍데기 (둘 중 하나 닫힘)

`2026-08-17` 점검이 정리한 두 문제 중 **"파일 경로를 타이핑" 쪽은 이미 안 그러고
있었다** — 자산·내레이션·장면 영상·음성 전부 네이티브 파일 선택창을 쓰고 끌어다
놓기와 폴더째 고르기도 된다.

**"도커를 띄우고 주소를 쳐야 켜지는 것"을 풀었다.** `VideoBox.cmd` +
`scripts/Start-VideoBox.ps1` + `scripts/Install-VideoBoxShortcut.ps1`. 컨테이너는
직접 다루지 않고 `owner-ready.ps1`에 넘긴다(`CLAUDE.md` §3). 실제로 켜서 확인했다 --
주소창 없는 Edge 앱 창, 제목 `VideoBox`.

**진짜 네이티브 껍데기(Electron/Tauri)는 안 만들었다.** 위 확인 결과 그게 풀어 줄
문제가 남아 있지 않다. 하려면 범위를 owner와 먼저 맞춘다.
...
- **편집 작업판 스냅샷 5장.** 다시 만들어 보니 5장 모두 이미 2026-08-16 것으로
  낡아 있었다 -- 이번 배치 변경뿐 아니라 2026-08-17의 컷 도구 툴바·타임라인 높이도
  들어 있지 않았다. 즉 두 세션치 변경이 함께 들어 있다.
- **비율 정책.** 1920x1080에서 영상도 트랙도 아닌 것이 먼저 490px를 가져간다.

이 closeout은 자동 검증과 실제 실행 확인을 정리한 것이며, owner acceptance 완료
보고가 아니다.

### 추가 — 프레임 이동·장면 반복 (캡컷 목록 마지막 미착수 항목)

재생 줄에 `◀｜` `｜▶` `반복`. 프레임 폭은 타임라인이 실제로 쓰는 프레임률에서
뽑는다(30fps를 박아 두면 25fps 소재에서 어긋난다). 반복 구간은 화면이 이미
`적용 구간`으로 보여 주는 것을 그대로 쓴다 -- 같은 개념을 두 벌 만들지 않았고,
고른 장면이 없으면 단추 자체가 없다.

`npm test` 1,037 통과, `npm run test:e2e` 47 통과.

**이로써 캡컷 대비 남은 것은 `타임라인이 아래 절반` 하나뿐이고, 그건 코드가 아니라
비율 정책 결정이다.**

### 추가 — 전체 pytest에서 매번 다른 한 건이 깨지던 원인

여섯 번 돌렸다. 1회차는 **진짜 결함**(인계 지도 가드)이었고, 3~5회차는 전부 같은
병이었다 — 뒤에서 도는 정비 패스를 `time.sleep(0.4)`으로 기다린 뒤 바로 단정하는
테스트들. 3,600개를 함께 돌리면 그 안에 못 도는 때가 있고, 어느 것이 못 도느냐가
매번 달라서 "무작위로 깨진다"로 보였다.

`tests/conftest.py`의 `wait_for(predicate)`로 바꿨다. 저장소에 이미 두 곳에 따로
있던 헬퍼를 공용으로 올린 것이다. 고친 뒤 **3,610 통과 · 53 건너뜀 · 실패 0**.

**함께 배운 것: 실패 요약을 `tail`로 자르지 마라.** 트레이스백을 잘라 버려 원인을
못 보고 30분짜리 실행을 한 번 더 썼다.

### 추가 — 캡컷 남은 차이 셋 중 둘 (2026-08-18)

**타임라인 클립에 그림.** 영상은 썸네일, 소리는 파형. 엔진을 새로 만들지 않고
라이브러리 자산이 쓰던 ffmpeg `showwavespic`을 프로젝트 자산에도 열었다. 실제
화면에서 클립 12개가 자산 2개를 가리키는 것을 보고 동시 생성을 막았다.

**`다음 장면에도`.** 캡컷의 `복제`는 이 모델에 그대로 없다 -- 배치는 장면에서
파생되고 따로 만들 수 없다. 같은 화면을 한 번 더 쓰는 실제 방법(`save-media`)을
한 번에 갈 수 있게 했다.

**전환(디졸브)은 안 했다.** 렌더러가 하나의 `filter_complex` 그래프라 `xfade`를
넣으면 전체 길이가 줄고 그 뒤 모든 시각이 밀린다 -- 자막·소리·재생 위치·미리보기가
전부 그 시각을 쓴다. 화면만 만들면 "완성본에는 안 나오는" 실패가 된다. 별도 조각.

`npm test` 1,049 · e2e 48 · 새 pytest 3건.

### 정정 — 전환(디졸브)은 결국 들어갔다

앞 절에서 "렌더러가 하나의 `filter_complex` 그래프라 `xfade`를 넣으면 전체 길이가
줄고 그 뒤 모든 시각이 밀린다"고 적었는데 **틀렸다.** 렌더러를 concat 모델로
읽었기 때문이다.

실제로는 캔버스 위 **오버레이 모델**이고 겹침을 이미 지원한다. 그래서 디졸브는
길이를 안 건드린다 -- 겹치는 구간에서 위 클립의 투명도만 올리면 된다. 알파를
태우지 않으면(`alpha=1`, `format=yuva420p`) 아래가 비치는 대신 검은색으로 가라앉는다.

**완성본까지 가는 것을 실제 mp4 렌더로 확인했다.** 전체 pytest 3,618 통과.

이로써 캡컷 대비 남은 차이 셋(그림·복제·전환)이 전부 닫혔다. 남은 것은 마커·멀티캠·
키프레임·전문 색보정이고 전부 `implementation-plan` §4의 명시 제외 항목이다.

### 개발 기록 — 2026-08-25 전역 이동·E2E 계약 복구와 로컬 재배포

- 전역·프로젝트 화면에 현재 위치 경로와 작은 이전 버튼을 연결하고, 아이콘 조작의
  툴팁·조밀한 보조 버튼을 추가했다. 승인된 색 토큰과 큰 화면 배치는 바꾸지 않았다.
- 전체 Playwright에서 발견한 16개 실패는 대부분 제품 결함이 아니라 예전 도크 이름,
  항상 펼친 메뉴, 결과 영역 탭, 시작 화면 문구를 찾는 낡은 E2E 계약이었다. 현재
  계약으로 시험을 갱신했고, 1280px에서는 720px 미리보기 폭을 위해 단일 도크가
  정상이라는 경계를 명시했다.
- 갱신 과정에서 실제 결함 하나를 재현했다. 접힌 전체 메뉴가 `Escape`로 닫히지 않아,
  상단바에서 메뉴와 프로젝트 전환 목록을 Escape로 닫도록 고쳤다. 브라우저 시험은
  `aria-expanded` 변화와 트리거 초점 복귀도 확인한다.
- 최종 근거: Vitest 96 파일·1,282 통과, TypeScript 검사 통과, Playwright 47 통과.
  `68e3ddec3`까지 원격 `codex/videobox-container-compatibility`에 푸시했고, 공식
  `owner-ready.ps1 -Mode Start -Rebuild -WithYujinMemory` 재빌드 뒤 `/health` 200 및
  `/api/projects` 실제 15개 응답을 확인했다.
- 이는 자동·런타임 검증 기록이다. 대표님이 실제 프로젝트에서 색·배치·버튼 밀도와
  이전 이동의 사용감을 보는 화면 수용은 별도 게이트로 남는다.
