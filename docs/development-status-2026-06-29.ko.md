...
- `A3 transport/output 경계`: Hermes와 gateway URL은 exact internal HTTP host/port만 허용하고 redirect/proxy/userinfo/path/query를 거부한다. 모든 prompt event는 exact session을 요구하며 tool/approval/file 계열 event, 비정상 completion status, empty final을 redacted blocked로 닫는다. gateway는 최소 256자의 rolling quarantine tail을 유지하면서 안전한 prefix를 completion 전에 32,000-byte 이하 delta로 증분 발행한다. Authorization/Cookie/Bearer/token/provider와 `/opt/data`, `/videobox-data`, `/etc` 등 split marker는 tail을 벗어나기 전에 차단하며 completion text는 이미 발행된 prefix와 일치하는 exact sanitized truth여야 한다. assembled text 200,000 bytes, public events 512, API-client line/event/delta 누적 상한을 각각 고정했다. service token은 양쪽 Python과 start verifier에서 32 bytes 이상·서로 다른 문자 8개 이상이며 placeholder를 거부한다.
