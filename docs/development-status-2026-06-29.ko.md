...
- `[x] A2 container-only install·ownership gate`: package source는 opt-in overlay에서 `/opt/videobox-yujin-profile`로 read-only bind되고 OAuth state `/opt/data`와 분리된다. 정적 verifier는 exact manifest, 실제 owned file, undeclared executable, secret/API/OAuth/password/Mem0/local absolute user path, traversal·reparse point를 fail-closed 한다. 시작 스크립트의 `ValidateOnly`는 설치·서비스 mutation 전에 끝나며 실제 경로는 profile verify → named one-off installer container의 idempotent `hermes profile install ... --force -y` → 명시적 `-p videobox-yujin` Hermes runtime → gateway 순서다. host Hermes profile은 설치 대상이 아니다.
