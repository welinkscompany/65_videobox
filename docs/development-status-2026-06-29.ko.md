...
- `non-live·Live 경계`: fake typed Hermes와 real local VideoBox API/store를 쓰는 non-live smoke가 SSE 완료, proposal 저장, Apply 전 mutation 0, Apply 뒤 revision +1·caption 1개 변경, playback manifest, output readiness, output job 0, external provider call 0을 확인한다. 별도 `-Live`는 이중 승인·credential·loopback·disposable project/sample copy를 요구하고, supplied root의 strict session JSON과 API exact session을 첫 POST 전·Apply 전후 대조한다. mismatch면 POST 0으로 fail-closed이며 source SHA를 보존한다.
- `코드리뷰·갭·역방향`: B4/B5 재검토에서 caption safe-area와 table bounds, image candidate server attestation, terminal CAS race, PostgreSQL 공통 lock order, candidate whitespace, Live root/session binding을 찾아 TDD로 수정했다. 최종 독립 spec 및 quality·gap·reverse review는 Critical/Important/Minor **0**, READY다.
...
- `미실행`: 실제 `-Live` happy path, live Hermes/provider, 실제 PostgreSQL 동시성 integration, Docker 환경 실증, 사용자 원본 영상 재생·청취, 실제 CapCut Desktop 사람 검증은 실행하지 않았고 통과로 간주하지 않는다. frontend의 React `act(...)`, jsdom navigation, intentional ErrorBoundary stderr와 build의 500 kB chunk warning은 exit 0인 기존 비실패 출력이다.
