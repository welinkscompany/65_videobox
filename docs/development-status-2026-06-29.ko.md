...
- `A2 검증`: package·mount·verifier·installer·start ordering 부재 RED는 **17 failed**였다. 대문자 executable 우회와 install 뒤 profile 미선택을 별도 RED 2건으로 재현해 case-insensitive 차단과 명시적 profile 실행으로 닫았다. A2 exact 계획 범위와 기존 Yujin contract는 **99 passed, 1 skipped**, A1 topology/start/authority/plan 회귀를 합친 최종 관련 gate는 **193 passed, 1 skipped**였다. skip 1건은 현재 Windows 계정의 symbolic-link 생성 권한 부재이며 verifier의 reparse-point fail-closed 검사 자체는 정적·source test로 유지된다. profile/runtime/plan-state verifier와 `git diff --check`도 통과했다.
