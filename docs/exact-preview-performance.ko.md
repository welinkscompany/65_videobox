...
2026-07-21 bounded publish-fence 후 재측정 결과는 cold `0.5494s`(기준 ≤`20.0s`), warm `0.2436s`(기준 ≤`0.5s`)였다. 이 값은 10초·1280×720 local B-roll fixture에서 full SHA 재검증과 completed MP4 staging을 SQLite writer lock 밖으로 이동한 현재 구현의 측정값이다.
