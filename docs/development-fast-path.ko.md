...
4. 특별한 blocker가 없는 한 turn 종료 시에는 항상 커밋한다.
   - push는 브랜치 상태, 검증 상태, 협업 리스크를 보고 최적일 때만 진행한다.
...
8. 사용자에게 설명할 때는 기본적으로 존댓말을 유지한다.
9. 구현 slice를 추천하거나 수행한 뒤에는 아래 closeout 순서를 기본으로 유지한다.
   - 추천만 한 turn이어도 같은 closeout 형식으로 계획서 준수 점검, 다음 goal 프롬프트, 진행률/잔여율 추정치를 남긴다.
