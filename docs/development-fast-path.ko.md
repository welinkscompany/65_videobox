...
3. `artifacts/`, CapCut local export, 재현·QA 증거는 기본적으로 `preserve-evidence`다. Git 추적 제외는 허용하지만, 명시적 분류 근거 없이 삭제하지 않는다.
...
2. Hermes runtime·dashboard는 VideoBox data, media mount, PostgreSQL, `videobox-internal`, `videobox-edge`에 연결하지 않는다. Hermes 보조 기억은 전용 `/opt/data`와 `videobox-hermes-memory`의 local Ollama/Qdrant에만 둔다.
3. `videobox-hermes-model-seed`만 local Ollama model downloader egress를 가진다. 장기 Ollama service는 host port와 외부 egress를 갖지 않는다.
4. OAuth device code, account identity, credential contents, auth state와 memory contents는 source, `.env`, status document, verifier 출력에 기록하지 않는다. 검증은 mount/network/image/user/dependency 같은 경계 정보만 출력한다.
5. 이 local-MVP 경계는 VideoBox asset/file mutation, Telegram intake, egress gateway, host bridge, CapCut bridge의 활성화 근거가 아니다. 각각은 별도 구현·검증으로 닫는다.
