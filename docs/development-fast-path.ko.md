...
- gap 검증, 역방향 검증, 동작 검증, 배선 확인 넷은 **조각마다 고정**이다(§10.2 참조). spec review와 code-quality review는 리스크 대비 효율이 높을 때 붙인다
...
3. **갭 검증, 역방향 검증, 동작 검증, 배선 확인 넷은 조각마다 고정이다**(owner 상시 지시, 2026-09-11). 이전 규정은 "리스크 기반 선택"이었으나 owner가 기본값을 올렸다 — 근거는 이 넷이 각각 실제 결함을 잡았기 때문이다(아무도 안 읽는 값, 화면에서만 드러난 이름 충돌, 제품이 안 쓰는 분기). 묻는 것은 `CLAUDE.md` §3 표에 있다. code review는 여전히 리스크 기반이며, 논리적으로 닫힌 단위에만 붙인다.
...
### 10.5.1 완료의 정의 — 근거가 된 사고 기록 (`CLAUDE.md` §4에서 옮김, 2026-09-01)

`CLAUDE.md` §4가 8000자 상한을 넘겨 이 절의 구체적 사고 사례를 여기로 옮겼다
(요지·필수 원칙은 §4에 그대로 남아 있다).

- 이 저장소는 테스트 2,960개가 통과하는 동안 실제 음성 인식이 한 번도 실행되지
  않았다. 2026-08-08에는 계약 테스트가 전부 초록불인 채로 유진 컨테이너가
  **한 번도 뜬 적이 없었다** — 테스트 세 개가 오히려 동작하지 않는 설정을
  정답으로 고정하고 있었다.
- 2026-08-07에 완료의 정의로 다시 점검했더니, 백엔드는 완료였지만 **부를 화면이
  없어** 실제로 못 쓰던 기능이 하루에 여덟 건 나왔다(숏폼용 세로 고르기, 가로
  기본값 적용 경로, 검토 승인 화면 등).
- 이 격차는 재보면 계속 나온다. `api.ts` 메서드 중 화면에서 한 번도 안 불리는
  것이 2026-08-07에 144개 중 37개, 2026-08-09에 147개 중 31개였다. 새 백엔드를
  만들 때마다 부르는 화면을 같이 만들지 않으면 이 목록이 늘어난다.
- 2026-08-07에 승인 API가 202로 통과하는 것을 보고 "내보내기가 뚫렸다"고
  판단했으나, 브라우저에서 열어보니 같은 화면이 다른 호출의 500 때문에 여전히
  "불러오지 못했어요"만 띄우고 있었다. API 단건 확인이 화면 확인을 대체하지
  못하는 실제 사례다.
...
그 전까지 main은 **963커밋 뒤처졌고 842파일이 달랐다.** 브랜치가 아니라 포크였다.

**무엇이 문제였나.** 새로 시작하는 에이전트는 저장소 루트에 내린다. 거기가
낡아 있으면 낡은 트리를 보고 **자신 있게 틀린 보고**를 한다. 실제로 그 일이
있었다. 대응으로 루트 `CLAUDE.md`를 안내판으로 바꿔 뒀는데, 그건 고칠 수 있는
문제에 대한 우회로였다.

**같은 병을 그날 한 층 아래에서 또 봤다.** 6월에 쓰인 `videobox-mcp-scope.ko.md`가
