...
- 자산 탭 접근성 보강(2026-08-12): 비활성 패널이 DOM에 없는데도 모든 탭이 `aria-controls`를 가리키던 문제를 재현 테스트로 고정했다. 활성 탭만 실제 패널 ID를 가리키고, 비활성 탭은 `tabIndex=-1`로 두며, Arrow/Home/End 키로 탭을 이동할 수 있게 수정했다. 커밋 `5cf1dc3` 이후 공식 런타임을 재빌드해 실제 화면에서 패널 교체·포커스 이동을 확인했다.
