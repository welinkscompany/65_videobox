...
- 불필요한 로컬 모델 실행 artefact는 제거했다. 과거 custom local-runtime bootstrap 초안은 대시보드 실행 경로에 연결되지 않는다.
...
3. source에 남은 custom runtime bootstrap 초안을 Platform-only 결정을 기준으로 정리한다. 현재 요구에는 필요하지 않으므로 기본 권고는 삭제다.
