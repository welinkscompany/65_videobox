...
**owner 결정 (2026-08-06): 2번.** 지금 이미 호스트 네이티브 dev 서버
(`scripts/run_api.py`)에서 정상 동작하고, 코드 변경도 `base_url` 보안 검증 완화도
필요 없다. 1번(컨테이너→호스트 경계를 여는 것)은 architecture-plan.ko.md §11의
권고를 거스르는 선택이라 owner가 명시적으로 필요해지기 전까지는 보류하기로 했다.
컨테이너 스택으로 배포/패키징해야 하는 요구가 생기면 그때 다시 논의한다.
...
### F-9. 렌더 출력이 지금까지 전부 세로(1080×1920)였다 — **해결됨 (2026-08-06)**

owner 결정: "가로로 기본값으로 해야지. 다른 일반 캡컷이나 프리미어 프로도 모두 기본이
가로야." `build_timeline`이 이제 `orientation` 미지정 시 `landscape`(1920×1080)를
명시적으로 채운다. 아래는 발견 당시 기록.

- 근거 등급: **관측** — `artifacts/owner-sample-edit-20260803-r4/.../output.mp4`를
  `ffprobe`로 직접 열어 확인. `1080,1920` (세로)
- `composition_plan.py:18-19`의 `DEFAULT_OUTPUT_WIDTH = 1080`, `DEFAULT_OUTPUT_HEIGHT = 1920`이
  기본값이고, `build_timeline()`이 지금까지 `timeline["output"]`/`video_width`/`video_height`를
  채우는 코드가 전혀 없어서 **모든 프로젝트가 이 기본값으로 렌더돼 왔다.**
- 롱폼 설명형 영상이 주 제품이라면 이 기본값은 아마 의도가 아닐 것이다.
  다만 이번 세션이 만든 문제가 아니라 **이미 있던 상태**이고, 실측 전에는 아무도 몰랐다.
- Task 17에서 `orientation` 파라미터(`"landscape"`/`"vertical"`)를 추가해 명시적으로
  고를 수 있게는 했지만, **아무것도 명시하지 않았을 때의 기본값 자체는 바꾸지 않았다** —
  "기본값을 무엇으로 할지"는 owner 확인이 먼저 필요하다고 판단했다.
- 확인이 필요한 질문: 지금까지 만든 롱폼 프로젝트가 세로로 나온 걸 owner가 인지하고
  있었는지, 인지하지 못했다면 지금까지의 산출물에 대해 어떻게 할지, 그리고
  `orientation`을 명시하지 않은 프로젝트의 기본값을 `landscape`로 바꿀지.

### F-10. 코드리뷰로 잡은 낮은 우선순위 항목 (2026-08-06, 수정하지 않고 기록만)

- `local_project_store.py`의 `restore_project()`가 이전 상태(예: `ready`)를 기억하지 않고
  무조건 `draft`로 되돌린다. 지금은 `ready` 상태를 실제로 쓰는 코드가 없어 무해하지만,
  나중에 `ready`가 실제로 쓰이기 시작하면 복원 시 상태 정보를 잃는 함정이 된다
- `media_inbox.py`의 `run_inbox_cycle`이 매 주기마다 라이브러리 전체를 다시 해시하고,
  이동한 파일도 다시 한 번 해시한다(원본 검증 + 이동 후 검증). 정확성 문제는 아니고
  라이브러리가 매우 커지면 성능 이슈가 될 수 있다
