...
### 2026-08-16 밤 — 유진 추천 기능 라이브 검증 + 완성본 정밀 점검

owner 질문("유진이 영상·음악·효과음 추천 기능 다 들어갔나")에 대한 실측 답과,
완성본 정밀 점검에서 나온 **새 렌더러 결함 1건**.

**유진 추천 — 세 종류 모두 실제로 작동한다:**

- 편집기 "유진에게 추천받기": 의미검색("뜻으로 찾음") 기반 후보가 revision 일치
  (제안 기준 22 = 현재 22) 상태로 나옴. 라이브 확인.
- 의미검색 API(`POST /api/media-library/search`): broll 0.53 / **music 0.69** /
  sfx 0.53 점수로 세 종류 모두 응답. 2026-08-09 기억("음악은 아직 의미검색 아님")은
  이제 낡았다 — 음악도 의미검색이 붙어 있고 점수대가 §10.15의 우리말 범위와 일치.
  음악에 orientation을 주면 규정대로 422 거절도 확인.
- 유진 채팅(로컬 qwen): 라이브 응답 확인(앞 절). 촬영본 정리의 유진 장면 제안은
  E2E 2개가 상시 검증 중.
- UX 관찰: 추천 카드 라벨이 전부 "P01-B-01 · 미디어"로 나와 **어떤 자산인지 이름으로
  구분되지 않는다.** 파일명/제목 표기가 필요해 보임.

**완성본 정밀 점검(export_002):**

- 전체 디코드 무결성: 오류 0 (깨진 프레임 없음)
- 자막 타이밍: 5초·15초 경계 ±0.2초(6프레임) 이내 정확 전환, 장면별 문구 일치
- 영상: 정확히 600프레임/20.000초/1080p30
- **새 결함(4번째): 음악을 한 구간(0~5초)에만 넣자 완성본 오디오 트랙 자체가
  5.0초로 잘렸다.** 영상은 20초인데 오디오 샘플이 5초 이후 부재(무음이 아니라 없음).
  음악 없던 export_001은 20초 무음 베드가 정상적으로 깔려 있었다 — 음악 믹싱이
  베드를 대체하면서 길이를 음악 구간 길이로 줄이는 렌더러 결함. silencedetect의
  `time=00:00:05.01`과 스트림 duration 5.0으로 확정.
- 참고: 원본 촬영본에 이미 박혀 있던 자막(중앙, Vrew)과 VideoBox 자막(하단)이
  같은 화면에 공존할 수 있다. 결함은 아니지만 owner가 소재 고를 때 알아야 할 사항.

오픈 전 점검 계획서 프롬프트의 "미해결 3건"은 이 오디오 잘림을 포함해 **4건**으로
읽을 것.

## 오디오 잘림 수정 완료 (2026-08-16, `dcfeb76a`)

위 4번째 결함은 같은 날 해결했다. **미해결 목록에서 빼고 3건으로 읽는다.**

**원인은 믹싱 로직이 아니라 스레드 압박이었다.** 필터 그래프 자체는 옳았다 —
같은 그래프를 호스트와 컨테이너에서 단독 재현하면 20초 오디오가 정상적으로 나온다.
문제는 인코더·필터에만 `-threads 2`를 걸고 **입력 디코더는 안 묶은 것.** 입력 6개면
디코더가 호스트 CPU 수(16)씩 잡아 컨테이너 `pids_limit: 128`(스레드 포함)을 압박하고,
그 압박에서 ffmpeg가 오디오 브랜치만 일찍 끝낸 채 **성공(0)으로 종료**할 수 있다.
5.0초라는 숫자는 bgm atrim 경계였다. 같은 압박이 더 세면 아예 `-11`로 죽는다
(job 007/008이 그 경우).

수정 두 겹 (`ffmpeg_final_renderer.py`):

1. **모든 입력에 `-threads` 상한** — 디코더도 인코더와 같은 상한을 지킨다.
2. **fail-closed 사후 검사** — 렌더가 0으로 끝나도 출력물의 오디오 스트림 길이를
   ffprobe로 재서(`_probe_audio_stream_duration`), 타임라인보다 0.75초 이상 짧으면
   `FinalRenderError`. 같은 결함이 재발해도 조용히 완성본이 되지 못한다.

검증: 신규 테스트 3개(디코더 상한 / fail-closed / 음악 부분 구간 실렌더 회귀) +
전체 pytest 단독 3545 통과. 컨테이너 재빌드 후 실제 화면에서 "완성본 다시 만들기" →
job 009 성공, export_003 오디오 스트림 **20.000초**(영상 600프레임/20초), 0~5초에
음악 실림(max -9.6dB). 5초 이후 무음은 이 픽스처의 내레이션 소스가 원래 무음이라 정상.

## 실사용 발견 3건도 모두 닫았다 (2026-08-16)

위 "실사용 발견 3건"은 같은 날 전부 수정했다. **미해결 목록에서 뺀다.**

### 1. 자리표시 자산이 재료처럼 보이던 문제 (`75faaa4b`)

초안이 빈 자리를 표시하려고 넣는 자산(`asset_gap_placeholder_*`)이 편집기 재료
목록에 **"B-roll 1~4 · 0초"로 떠서 고를 수 있었다.** 골라 봐야 공백이 공백으로
바뀔 뿐이다.

저장소는 이미 이것을 `in_app_only: true`로 표시하고 있었고 합성 계획도 렌더 입력에서
빼고 있었다. **목록만 그 표시를 안 보고 있었다.** 이름 접두사가 아니라 그 표시를
기준으로 걸러낸다. 번호도 걸러낸 뒤에 매긴다 — 안 그러면 "B-roll 2"부터 시작해
owner가 하나를 잃어버렸다고 읽는다.

실제 컨테이너 확인: 편집기 재료가 자리표시 4개 없이 **실제 영상 2개(494.8초·4초)만**
남았다.

### 2. exports 엔드포인트가 디코딩 오류를 그대로 내보내던 문제 (`75faaa4b`)

`GET /api/projects/{id}/exports/{job_id}`는 CapCut 초안 결과용인데 **job 종류를
확인하지 않았다.** 완성본 job_id를 주면 그 mp4를 JSON 매니페스트로 읽으려다
`'utf-8' codec can't decode byte`가 그대로 사용자에게 나갔다.

두 겹으로 막았다 — 경로가 CapCut 초안이 아닌 job을 404로 거절하고, `get_export_run`도
매니페스트가 아닌 산출물이면 디코딩 오류 대신 분명한 메시지로 거절한다.
실제 컨테이너 확인: 같은 요청이 이제 `404 · CapCut draft export not found`.

### 3. 무음 완성본에 경고가 없던 문제

렌더가 끝난 파일의 **최대 음량을 실제로 재서**(-60dBFS 미만이면 소리 없음으로 본다)
그 결과를 완성본 기록에 남기고, 출력 화면이
`완성본에 소리가 들어 있지 않아요. 내레이션이나 음악을 넣고 다시 만들어 주세요.`
를 띄운다.

**재지 못한 경우와 소리가 없는 경우를 구분한다.** 측정 실패나 옛 완성본은 `null`이고,
그때 화면은 소리에 대해 아무 말도 하지 않는다 — 섞으면 멀쩡한 완성본을 의심하게 된다.
