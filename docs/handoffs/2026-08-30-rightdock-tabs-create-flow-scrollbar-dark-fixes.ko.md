...
**남긴 것**: "추천" 탭은 그대로 도크 안에 뒀다 — 이번 지시는 "대화창"에
관한 것이었고, 추천 후보 검토는 화면 공간이 더 필요해 작은 떠있는
패널에 안 맞는다. 캡컷은 실제로 제안 카드를 EditPilot 패널 **안**에
두지만, 이 경계는 이번 범위 밖으로 남겼다.

## 다음에 이어갈 사람에게

이 문서가 최신 인계다. `CLAUDE.md` §2의 "최신 세션 인계" 줄을 이 파일로
옮겨 두었다. 남은 우선순위는 캡컷 새 캡처·실제 로고 두 가지뿐이다
(Tailwind 결함·Docker 캐시 오진·유진 패널 독립화는 이번 세션에 전부
닫혔다).
