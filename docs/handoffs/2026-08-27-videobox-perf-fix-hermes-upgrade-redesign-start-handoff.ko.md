...
5. **2026-08-28, 새 갈래: 편집기 재생·컷 조작 UX 점검 시작, 화면 전수조사로 확장 중.**
   ...
   **전수조사 중 새로 찾았지만 아직 손대지 않은 것:** `/footage`의
   `FootagePreview.tsx`도 같은 패턴(기본 `controls` + 커스텀 재생바)이다.
   다만 여기는 커스텀 쪽에 재생/일시정지 대체가 없어서, 편집기와 똑같이
   기본 컨트롤을 끄면 음량 조절이 다시 통째로 사라지는 회귀가 재현된다.
   **owner 결정이 필요해서 이번엔 건드리지 않았다.**
