...
3. ~~**CPU 캡 조사는 보류 중**~~ — **2026-08-28 조사 완료. 결론이 뒤집혔다:
   ...
   **1.27초**만 돈다 — 나머지 ~19초는 Python 쪽이었다. 원인은 `sha256_file()`
   ...
   ~~**남은 병목(이번 범위 밖, 다음 과제 후보)**~~ — **2026-08-28 원인 특정·수정
   완료**(커밋 `7bde7b35`). `verify_output_sources()`가 `capture_output_source_snapshots()`로
   해시한 직후 경과 시간 0인 채로 `verify_output_source_snapshots()`가 같은
   파일을 또 해시하고 있었다 — 캡처가 이미 불일치를 잡아 예외를 던지므로
   그 즉시 재검증은 방어 효과가 전혀 없다(자기 자신과 항상 일치). 이중 해시를
   없앴다. 캡처 쪽에는 `(경로, mtime)` 캐시(`hash_cache`)를 옵션으로 추가해
   `FfmpegFinalRenderer._output_source_hash_cache`를 넘길 수 있게 했다.

   **다만 이 캐시도 렌더 하나를 못 넘는다** — `render_exact_preview_to_mp4`가
   `dataclasses.replace(self, ...)`로 proxy_renderer를 새로 만드는데,
   `replace()`가 `init=False` 필드의 `default_factory`를 다시 불러 이
   캐시도 원래 있던 `_stream_probe_cache`도 렌더마다 빈 채로 시작한다(기존부터
   있던 결함, 이번에 처음 확인). 렌더를 넘어서는 캐시가 필요하면 `replace()`
   대신 다른 구성 방법이 필요한데, 이번 범위 밖이라 손대지 않았다.

   최종 실측: **22.2초 → 8.1초(63% 단축)**, cpus는 2.0 그대로. 관련 pytest
   253 passed, 3 skipped, 전체 backend pytest 4081 passed, 56 skipped.
...
5. **2026-08-28, 새 갈래: 편집기 재생·컷 조작 UX 점검 시작, 화면 전수조사로 확장 중.**
   owner 지적("재생·컷 조작이 복잡함")으로 편집기 미리보기를 봤더니 `<video>`에
   브라우저 기본 `controls`가 켜진 채로 그 아래 커스텀 재생바(이전 프레임·재생·
   다음 프레임·반복·전체화면)가 또 있어서 재생바가 두 벌이었다 — 없앴다(커밋
   `18edd385`, 프런트엔드 테스트 1343 passed, 브라우저로 확인).

   **정정해 둔다:** 컷 도구(나누기·붙이기·빼기)가 "타임라인에서 장면을 먼저
   고르라"고 잠긴 채 시작하는 것은 결함이 아니었다 — 재생하거나 탐색만 해도
   재생헤드 위치의 장면이 자동 선택돼 바로 풀린다(`EditorWorkbench.tsx`의
   `seekPlayback` → `activeSegmentIdAt`). 최초 진입 직후 한 순간만 비어 있는
   정상 동작이라 손대지 않았다.

   owner가 이어서 **"편집기와 설정 페이지 내용을 일관성 있게"**를 요구했는데,
   이번 세션 안에서는 그 지시를 받은 기록이 없다(메모리·문서 검색해서 확인) —
   이전 세션 지시라면 어느 세션인지 다음 세션이 다시 물어야 한다.

   **전수조사 결과(용어 통일, 완료):** `/settings`부터 편집기·`/projects`·
   `/library`까지 훑어 2026-08-27 "미디어로 통일" 승인이 놓친 사용자 노출
   문구 8곳을 찾아 고쳤다(커밋 `33d7114b`, `c0e80af7`). 편집기 음악·효과음
   탭의 같은 글자는 뜻이 달라서(내 프로젝트 vs 라이브러리 검색) 일부러
   남겼다 — 처음에 지웠다가 테스트가 잡아서 되돌렸다.

   **코드리뷰(커밋 `7a3ce09a2..HEAD`, 8앵글) 결과, 2건 발견·1건 즉시 수정:**
   - 재생바 중복을 없애며 브라우저 기본 `controls`를 끄자 그 안에 있던
     **음소거 단추도 같이 사라졌다** — 미리보기 소리를 끌 방법이 통째로
     없어졌던 것을 발견해 `muted` state + 음소거 단추로 되살렸다(커밋
     `ae56d0b3`). 브라우저로 `<video>`의 `muted`가 실제로 토글되는 것까지
     확인.
   - `verify_output_sources()`의 이중 해시 제거(`7bde7b357`)가 "경과 시간
     0이라 방어 효과 없다"고 주장했는데, 다시 보니 캡처 루프가 여러 항목을
     순회하므로 큰 파일이 여럿이면 실제로 시간이 걸렸다 — 판단이 과장이었다.
     owner 지시로 고쳤다: 전체 재해시 대신 해시 직후 (크기, mtime)를 남겨
     뒀다가 루프 끝에 stat만 비교하는 값싼 재확인으로 보호를 되살렸다(커밋
     `1278c5eb`).

   **owner 지시로 추가 수정:** `_output_source_hash_cache`가 렌더 하나를 못
   넘던 구조적 한계(`dataclasses.replace()`가 `init=False` 캐시 필드를 매번
   비움)도 고쳤다 — `_replace_sharing_caches()`로 새 인스턴스가 같은 캐시
   딕셔너리를 이어받게 했다(같은 커밋 `1278c5eb`). 임시 계측으로 546MB
   소스가 **다른 요청에서도** 캐시 HIT로 넘어가는 것을 cache_id까지 확인
   했다(계측 코드는 확인 뒤 원복, 커밋 안 함).

   ~~**전수조사 중 새로 찾았지만 아직 손대지 않은 것**~~ — **2026-08-28
   owner 결정("①번으로 정리"), 구현 완료.** `/footage`의 `FootagePreview.tsx`도
   같은 패턴(기본 `controls` + 커스텀 재생바)이었다. 여기는 커스텀 쪽에
   재생/일시정지 대체가 아예 없어서, 편집기와 똑같이 기본 컨트롤부터 끄면
   음량 조절이 통째로 사라지는 회귀가 그대로 재현될 상황이었다 — 대체 단추
   (재생/일시정지·음소거)를 먼저 만들고 나서 기본 컨트롤을 껐다(커밋
   `278bd82d`). 브라우저로 음소거는 두 방향 다 확인했다. **재생/일시정지는
   끝까지 확인하지 못했다** — 이 테스트 환경의 합성 클릭이 브라우저
   자동재생 정책의 "신뢰된 사용자 조작"으로 안 잡혀서 재생 상태 전환을
   못 지켜봤다(이 세션 초반에 겪은 같은 환경 제약). 코드는 편집기
   미리보기에서 이미 검증된 것과 동일한 `togglePlayback` 패턴이라 동작은
   같을 것으로 판단하지만, 다음 세션이 실제 마우스로 한 번 눌러 확인하는
   게 안전하다.

   검토·내보내기 단계, 이야기(대본) 시작 화면은 훑어봤고 이번엔 문제를
   못 찾았다.

   **전체 backend pytest에서 걸린 것 하나:** `ProductShell.tsx`(자산→미디어
   문구 수정, `33d7114b`)가 `docs/oss/editor-ui-source-map.json`의
   `normalized_sha256` 핀을 깨뜨렸다 — 분할 실행에서는 안 보이고
   **전체 pytest에서만** 잡히는 자리(`test_editor_ui_source_provenance.py`,
   CLAUDE.md에 이미 적혀 있던 함정)라 이번에도 마지막 전체 실행에서
   처음 발견했다. 새 해시로 갱신했다(커밋 `8f5ca2af`). **ProductShell.tsx를
   또 고치면 이 핀도 같이 갱신해야 한다** — `Get-FileHash -Algorithm
   SHA256 -Path apps/web/src/app/ProductShell.tsx`로 재고, 파일 안의 두
   `normalized_sha256` 항목에 똑같이 넣는다.

   **최종 확인:** 전체 backend pytest `4081 passed, 56 skipped`, 프런트엔드
   1343 passed, 컨테이너 재빌드·push 전부 완료. 이 갈래는 여기서 닫는다.

## 6. 알려진 경계

- 배포는 이 저장소의 유일한 배포 경로인 `owner-ready.ps1 -Rebuild`로 끝났다 — 별도
  클라우드/원격 배포 파이프라인은 없다. 지금 컨테이너가 `origin/codex/videobox-container-compatibility`
  최신 커밋(`8f5ca2af`)까지 전부 반영한 상태로 떠 있다.
