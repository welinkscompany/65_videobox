...
1. **먼저** 프로젝트 관리를 어디에 둘지 정한다. 두 갈래가 있다.
   - 위 띠 스위처 안에 접어 넣기 — 지금 markup을 거의 그대로 옮길 수 있지만,
     관리 동작이 전부 **드롭다운을 연 뒤**로 밀린다. 시험 7개가 그만큼 달라진다.
   - `/projects` 화면으로 옮기기 — 관리는 원래 거기가 제자리라는 주장도 된다.
     띠는 고르는 것만 맡고, 이름·보관·삭제는 목록 화면에서 한다. 시험은 그 화면으로
     옮겨 다시 쓴다.
   **어느 쪽이든 owner에게 먼저 물어라.** 관리 동작이 한 겹 더 들어가는 변화다.
2. 그 다음에 `ProductShell`의 `<Sidebar>…</Sidebar>`(125~178줄)를 들어낸다.
