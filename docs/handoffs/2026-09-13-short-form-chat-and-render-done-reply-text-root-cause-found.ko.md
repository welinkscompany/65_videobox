...
1. `task_006f1523`부터 본다 — 경계 나누기가 완성본 바이트를 바꾸는 진짜
   원인을 찾고, owner와 상의해 고칠지 설계를 재검토할지 정한다. **이건
   완성본 렌더 정확성에 관한 것이라 blast radius가 크다.**
2. 완성본(마스터) 렌더 자원 오류(별도 task, 세그먼트 94개의
   `2026-09-12-ca6dd9ed` 프로젝트) — 여전히 안 풀림. 근본 원인(EAGAIN,
   pids_limit)은 이번 세션 조사 에이전트가 코드로 확인해 뒀다. 고치려면
   "입력 총량에 맞춰 스레드를 다시 나누기" 또는 "세그먼트 배치·concat 방식으로
   바꾸기" 중 하나를 owner와 상의해야 한다(조사 결과가 이 문서 초안에
   남아 있음 — 다음 세션이 이 인계를 이어받을 때 함께 전달할 것).
