...
1. `task_eccc77a3`부터 본다 — 완성본 렌더 자원 크래시. **세 방향 중 하나를
   owner에게 먼저 물어야 한다**(`pids_limit` 상향은 보안 하드닝 완화라
   승인 필요). 방향이 정해지면 구현하고, 반드시 같은 렌더를 10번 이상
   연속으로 돌려서 검증할 것 — 한두 번 성공은 증거가 아니다.
