...
## 6. 완성본 렌더 자원 크래시 — 원인 확정 + 고칠 방법도 실측 검증함

R1을 재는 김에 막혀 있던 완성본 렌더 자원 오류(`2026-09-12-ca6dd9ed`)의
진짜 원인도 확정했다. `-11`은 SIGSEGV가 아니라 ffmpeg의 EAGAIN이고,
`encoder_thread_limit()`(`ffmpeg_final_renderer.py`)이 컨테이너 CPU 몫만
보고 입력마다 스레드 2개를 배정하는데, 세그먼트/전환이 쌓여 입력이
많아지면 `입력 수 × 2`가 `compose.yaml`의 `pids_limit: 128`을 넘는다 —
이 함수의 옛 docstring이 이미 "더 많은 입력에서는 이 상한으로도 모자랄
수 있다, 그때는 pids_limit을 봐야 한다"고 예견해 뒀었다.

**실제로 컨테이너에서 확인했다**: `encoder_thread_limit()`을 임시로(커밋
안 하고) `return 1`로 고정해 재빌드하고 같은 프로젝트의 완성본을 다시
렌더했더니 **성공했다**(`final_render_job_024`, 494.837초, 15장면, 실제
mp4 파일까지 나옴). 예전엔 이 렌더가 시작하자마자 죽었었다. 실험 후
즉시 원상복구하고(git diff 깨끗함) 컨테이너도 정상 코드로 재빌드해 뒀다.

**아직 "진짜 수정"은 안 했다** — `return 1`은 모든 렌더의 속도를 절반으로
떨어뜨리는 임시 실험값이라 그대로 커밋하면 안 된다. 진짜 수정은 "이번
렌더의 총 입력 개수를 보고 필요할 때만 스레드를 낮추는" 동적 계산이고,
`task_591bfdff`에 정확한 구현 순서(어느 파일 몇 번째 줄, 몇 군데 호출부를
같이 고쳐야 하는지)까지 적어 큐에 올렸다.

## 7. 다음 세션이 먼저 할 일

1. `task_591bfdff`부터 본다 — 완성본 렌더 자원 크래시의 진짜 수정. 방향은
   이미 실측으로 검증됐고 구현 순서도 적어 뒀다 — owner 판단이 필요한
   설계 결정이 아니라 **구현만 남은 상태**다.
2. `task_006f1523`(R1 경계 나누기가 완성본 바이트를 바꾸는 진짜 원인) —
   owner가 급하지 않다고 확인했으므로 여유 있을 때 본다.
3. CapCut R2(blur 실물 확인) — 여전히 구조적으로 막혀 있음(앞 인계 참고).
   대표님이 사무실 데스크톱에서 직접 캡컷을 열어 눈으로 봐야 하는 항목이다.
4. 여유가 있으면 `yujin_local_conversation.py`를 쓰는 다른 대화 시나리오
   (자료 추천 카드가 뜬 상태에서의 대화 등)도 실물로 한 번 더 훑어서,
   이번 프롬프트 수정이 의도치 않게 다른 답변을 이상하게 만들지 않았는지
   확인하면 좋다 — 이번 세션은 렌더 + 색감 편집 둘만 확인했다.
5. 4절(내부 오류 문구 누출)의 프론트 수정도 여유가 있을 때 실물(브라우저)로
