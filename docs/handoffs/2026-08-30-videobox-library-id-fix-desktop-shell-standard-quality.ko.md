...
## 다음 우선순위 (이 네 번째 이어진 세션 종료 시점)

1. **캡컷 버튼 단위 벤치마킹 — 다음 화면 고르기.** 편집기 상단·좌우
   패널은 끝났다. 첫 화면·검토 화면 등 다른 화면으로 이어갈지, 아니면
   여기서 이 라운드를 닫을지 owner에게 먼저 확인.
2. **AI 대화 편집 Task 4의 마지막 조각.** 실제 로컬 LLM으로 candidate →
   proposal-preview MP4 → apply → undo/redo → 검토 승인 → final MP4까지
   끝까지 밟는 owned fixture QA. LM Studio가 떠 있는지부터 확인 필요.
3. **Tauri 데스크톱 셸 실제 빌드.** owner가 Windows Smart App Control을
   "허용 항목 지정" 방식으로 풀기로 했다 — 직접 처리 예정이라던 부분,
   완료 여부 확인 후 실제 빌드·검증 진행.

## 다음 세션 시작 프롬프트

```
CLAUDE.md와 docs/handoffs/2026-08-30-videobox-library-id-fix-desktop-shell-standard-quality.ko.md를
먼저 읽어. 이번 세션까지 job 상태 영속화, ComfyUI 취소 TOCTOU 수정,
AI 대화 편집 결함 2건, 캡컷 버튼 단위 벤치마킹 1~3단계(편집 동작 위치,
왼쪽 콘텐츠 탭 승격, 오른쪽 패널 상시 노출)까지 전부 끝났고 push까지
완료된 상태야. 다음 우선순위는 (1) 캡컷 버튼 단위 벤치마킹을 다른
화면으로 이어갈지 owner에게 확인, (2) AI 대화 편집 Task 4 마지막 조각
(로컬 LLM 종단 QA), (3) owner의 Smart App Control 처리 여부 확인 후
Tauri 데스크톱 셸 실제 빌드 순서로 진행해. 다른 지시가 없으면 이
순서대로 진행 여부를 먼저 물어봐.
```
