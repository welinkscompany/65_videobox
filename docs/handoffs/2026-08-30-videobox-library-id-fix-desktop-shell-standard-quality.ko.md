...
## 다음 우선순위 (이 세 번째 이어진 세션 종료 시점)

오늘 밤 지시받은 세 우선순위 중 둘(TOCTOU, 캡컷 캡처 복구)과 낮은 위험
후속(reply_text·자산 목록 프롬프트)까지 끝냈다. 남은 것:

1. **캡컷 버튼 단위 벤치마킹 착수.** 이제 참조 이미지 3장이 저장소에
   있다. 편집기 화면을 실제로 비교해 보니 CapCut 상단 도구줄(미디어·오디오·
   텍스트·스티커·편집효과·전환·캡션·필터·조정·템플릿·AI 아바타 — 전부
   "넣을 콘텐츠 종류" 탭)과 VideoBox 상단 도구줄(실행취소·다시실행·나누기·
   앞과 붙이기·빼기·다음 장면에도·미디어·세부 정보·내보내기 — 편집 동작과
   콘텐츠 탭이 섞여 있음)은 **버튼 크기·배치 차이가 아니라 정보 구조
   자체가 다르다.** 시작하기 전에 이 구조 차이를 그대로 옮길지, 아니면
   기존 정보 구조는 유지하고 크기·여백·아이콘 스타일만 맞출지 판단이
   필요하다 — 다음 세션(또는 이어지는 반복)이 이 판단부터 하고 시작할 것.
