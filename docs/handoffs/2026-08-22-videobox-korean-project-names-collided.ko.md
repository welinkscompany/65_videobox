**대체됨:** `docs/handoffs/2026-08-22-videobox-scene-transitions-and-the-frame-rate-trap.ko.md`
...
## 닫혔다 — PowerShell 껍데기 시험 50개는 부하 탓이었다 (2026-08-22)
...
결정적 증거 — `test_owner_ready_script.py::test_smoke_timeout_kills_the_child_tree`가
**전체와 같이 돌면 30초 제한에서 죽고, 단독으로는 3.81초에 통과한다.** 얼굴 LoRA
학습이 GPU를, 다른 에이전트가 CPU를 쓰는 **그 상태에서도** 단독이면 통과했다.
좁혀 가며 12개 → 1개 → 0개로 줄어든 것도 같은 그림이다.

이 시험들은 자식 프로세스를 띄우고 **벽시계 제한**을 건다. 기계가 붐비면 제한 안에
못 끝난다. `videobox-full-pytest-must-run-alone`이 말하던 바로 그것이고, 2026-08-18에
고정 sleep을 없앤 뒤로도 **이 네 파일은 여전히 부하에 약하다**(고정 sleep이 아니라
subprocess 제한이라 성격이 다르다).

**다음 사람에게:** 이 네 파일이 무더기로 깨지면 **먼저 기계가 붐비는지 보라.**
`git bisect`로 가기 전에 단독 실행부터 해라. 오늘 한 에이전트가 이걸 건너뛰고
"`.venv`가 없어서"라고 단정했다가 틀렸다 — venv를 넣으니 54개가 73개로 늘었다.
