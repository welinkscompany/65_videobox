...
- A2 verifier runtime/provenance: `requirements-dev.txt`만 `PyYAML==6.0.3`을 exact pin하며 canonical `.venv` 설치 버전과 일치한다. local installed metadata 기준 MIT, homepage `https://pyyaml.org/`, source `https://github.com/yaml/pyyaml`; external fetch와 runtime container dependency 추가는 없다. missing import는 generic·redacted failure다. Persistence contract: profile install persists in the videobox_hermes_oauth_state named volume at /opt/data; rerun uses --force idempotently; service cleanup only; the named volume is not deleted. gateway failure의 `compose stop`은 새로 시작한 service만 정리하며 full rollback/profile rollback이 아니다. focused GREEN **3 passed + 3 passed**, profile **52 passed, 1 skipped**, 핵심 두 파일 **102 passed, 1 skipped**, 아래 exact 8파일 command **225 passed, 1 skipped**다.
- A2 post-install state: RED **3건**으로 installer failure는 `/opt/data` named volume에 partial profile이 남았을 수 있다고, successful install 뒤 Hermes first-up/gateway failure는 profile이 persists한다고 구분했다. recovery는 exact one-off container 또는 새로 시작한 service만 정리하고 volume/profile을 삭제하지 않으며 `--force` safe rerun을 안내한다. secret·입력 path는 출력하지 않는다. focused **1 passed + 2 passed**, 핵심 두 파일 **103 passed, 1 skipped**, 아래 exact 8파일 command **226 passed, 1 skipped**다.
