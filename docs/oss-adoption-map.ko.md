...
> **대체됨:** OpenCut 재작성판이 저장소 출처 기록 셋(source map·NOTICES·verify 스크립트)에 AGPL로 틀리게
> 찍혀 있던 것을 2026-09-07에 바로잡았다(고정 커밋 `bab8af8`의 LICENSE는 MIT 본문, 업스트림 확인). 이 문서의
> "MIT" 판단이 맞았다.
...
있었다 — `gh api repos/<owner>/<repo>`로 직접 대조하지 않은 숫자는 근거로 쓰지 않는다.
