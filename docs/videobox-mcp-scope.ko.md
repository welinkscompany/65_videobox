...
> **선행 작업 하나:** 잡 행은 이미 공통이다(`GET /api/jobs`가 프로젝트를 넘어
> 목록을 준다, `projects.py:392`). 단건 조회 문(`GET /api/jobs/{job_id}`)을 여는
> 편이 대응표를 MCP에 두는 것보다 낫다 -- 화면도 같은 문을 쓸 수 있다.
> **이 문을 먼저 열지, 대응표로 갈지는 착수 전에 정한다.**
...
| `ingest_library_asset` | POST `/api/library/ingest` — **지금은 multipart 전용이라 경로를 못 받는다.** §5 참고 |
...
**① 자산 등록이 경로를 받는가 — 반은 맞다.**
프로젝트 자산 일곱 문은 JSON 본문에 `source_path`를 받는다(`models.py:380`).
**그러나 자료실은 multipart 전용이다**(`library_assets.py:143`) --
`LibraryIngestService.ingest`는 `Path | str | BinaryIO`를 다 받는데
(`library_ingest.py:53`) 문만 안 열려 있다. 경로로 자료실에 넣으려면 **그 문을
먼저 열어야 한다.** 이건 MCP가 아니라 API 쪽 작업이다.

**② 경로가 컨테이너에서 보이는지 판정할 수 있는가 — 도구는 있는데 방향이 반대다.**
`CapCutHostPathMap.maps()`가 정확히 그 판정을 한다(`host_paths.py:94`). 다만
**컨테이너→호스트** 방향이다(초안을 캡컷에 넘기려고 만든 것). MCP에 필요한 것은
**호스트→컨테이너**이고, 그건 아직 없다. 새로 만든다.

> **"없는 파일"과 "안 보이는 파일"은 다른 사실이다.** 둘을 같은 오류로 내면
> AK-System이 파일을 다시 만들며 헛돈다. 도구는 이 둘을 갈라서 답한다.
...
1. **잡 상태를 공통 문으로 열 것인가**(§2), 아니면 MCP가 열세 가지 대응표를 들 것인가.
2. **`list_project_assets`를 위해 문을 하나 열 것인가**, 타입별 셋을 합칠 것인가.
3. **자료실 경로 등록 문을 열 것인가**(§5-①). 안 열면 AK-System은 바이트를
   올려야 하고, 그건 그쪽이 피하려던 바로 그것이다.
   **참고: 그 능력은 이미 쓰이고 있다** -- 드롭 폴더 정리기·장면 영상·인포그래픽
   셋이 `LibraryIngestService.ingest`에 경로를 그대로 넘긴다(HTTP를 안 지난다).
   문만 없는 것이다.
4. **경로를 받는 등록 래퍼를 살릴 것인가 지울 것인가**(§3.2 상자). 2026-08-24
   문서와 정면으로 부딪힌다.
