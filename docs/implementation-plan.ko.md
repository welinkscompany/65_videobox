...
> 현재 worktree 기준 Task 22A–F release parity 기술 closeout은 `docs/superpowers/plans/2026-07-23-videobox-task22-release-parity.md`와 최신 status/handoff를 우선 적용한다. 모바일 원격 환경의 owner dogfood/Task 9 사람·CapCut 실증은 별도로 보존하고, 현재 승인된 implementation goal은 `docs/superpowers/plans/2026-07-26-videobox-hermes-yujin-master-plan.md`의 Hermes Yujin 20-task initiative다. Phase 0 P0-1/P0-2, Phase A A1–A4, Phase B B1–B5, Phase C C1–C4와 Phase D D1–D2를 완료했다. 현재 상태는 **17/20 (85.0%)**, 잔여 **15.0%**이며 Mem0 child는 **2/5 (40.0%)**다. D2는 별도 store 요청이 들어온 approved candidate만 격리된 Hermes-derived Mem0 adapter에 전달한다. provider add 응답 유실은 ambiguous reconcile로 수렴하고, durable claim/call-start/event/memory/delete marker와 request idempotency가 중복 저장·삭제 crash를 막는다. browser는 candidate handle과 고정 상태만 보며 provider ref·raw record·credential을 받지 않는다. 대화형 Hermes의 자동 Mem0 provider는 비활성이고 adapter는 별도 token/network에 격리되며, key가 없거나 adapter가 실패해도 기존 채팅·수동 편집 fallback과 자동 Apply 금지를 유지한다. 최신 integrated D2 **298 passed, 36 skipped**, core focused **168 passed**(disposable PostgreSQL 포함), compose/profile **83 passed, 1 skipped**, start/recovery **82 passed**, 실제 adapter image build와 network-none import smoke, 정적 profile/runtime verifier, `compileall`, `git diff --check`, 독립 spec/quality/gap/reverse review가 통과했다. 전체 Python/frontend/build는 D2에서 다시 실행하지 않았고 통과로 주장하지 않는다. 실제 Mem0/provider 호출, 브라우저 사람 E2E, 사용자 미디어·CapCut 사람 검증도 실행하지 않았다. 다음 작업은 **D3만**이다. 기존 durable candidate/store/delete API를 RightDock의 approve/list/delete UI에 연결하되 pending/rejected는 retrieval 0, explicit click 전 provider call 0, close/open 상태·scroll 보존을 유지한다. Task 9 사람/환경 acceptance 전까지 기존 공식 누적은 **9/22 (40.9%)**, 잔여 **59.1%**로 고정한다.
