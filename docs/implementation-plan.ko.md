...
> 현재 worktree 기준 Task 22A–F release parity 기술 closeout은 `docs/superpowers/plans/2026-07-23-videobox-task22-release-parity.md`와 최신 status/handoff를 우선 적용한다. 모바일 원격 환경의 owner dogfood/Task 9 사람·CapCut 실증은 별도로 보존하고, 현재 승인된 implementation goal은 `docs/superpowers/plans/2026-07-26-videobox-hermes-yujin-master-plan.md`의 Hermes Yujin 20-task initiative다. Phase 0 P0-1/P0-2, Phase A A1–A4, Phase B B1–B5와 Phase C C1–C4를 완료했다. 현재 상태는 **15/20 (75.0%)**, 잔여 **25.0%**이며 realtime-reliability child는 **4/4 (100.0%)**다. C4는 dashboard의 HTTP readiness와 실제 chat verification/degraded evidence를 분리하고, browser에는 read-only typed status만 제공하며, exact Yujin service 세 개만 관찰·재시작하는 bounded operator script와 실패 뒤 복구를 보장하는 static/live-gated drill을 추가했다. Hermes가 꺼져도 Director/editor 수동 편집은 유지되고 자동 Apply는 없다. C4 focused Python **218 passed**, focused frontend **5 files / 89 passed**, 전체 Python **2458 passed, 41 skipped**, 전체 frontend **51 files / 689 passed**, production build, profile/runtime/plan-state/zero-tools/provenance/UI-system verifier, default StaticOnly failure drill, `git diff --check`, 독립 spec 및 quality/gap/reverse review가 통과했다. 실제 Hermes/provider 호출, 구성된 Docker service-stop live drill, 브라우저 사람 E2E, 사용자 미디어·CapCut 사람 검증은 실행하지 않았고 통과로 간주하지 않는다. 다음 작업은 **D1만**이다. VideoBox에는 승인 workflow record만 두고, 명시적으로 승인된 편집 취향만 나중에 Hermes 소유 Mem0로 보낼 수 있는 typed candidate/policy DTO와 저장 경계를 TDD로 구현한다. D1은 외부 Mem0/provider를 호출하지 않는다. Task 9 사람/환경 acceptance 전까지 기존 공식 누적은 **9/22 (40.9%)**, 잔여 **59.1%**로 고정한다.
