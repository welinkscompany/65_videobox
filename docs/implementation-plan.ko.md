...
> 현재 worktree 기준 Task 22A–F release parity 기술 closeout은 `docs/superpowers/plans/2026-07-23-videobox-task22-release-parity.md`와 최신 status/handoff를 우선 적용한다. 모바일 원격 환경의 owner dogfood/Task 9 사람·CapCut 실증은 별도로 보존하고, 현재 승인된 implementation goal은 `docs/superpowers/plans/2026-07-26-videobox-hermes-yujin-master-plan.md`의 Hermes Yujin 20-task initiative다. Phase 0 P0-1/P0-2, Phase A A1–A4, Phase B B1–B5, Phase C C1–C4와 Phase D D1–D3를 완료했다. 현재 상태는 **18/20 (90.0%)**, 잔여 **10.0%**이며 Mem0 child는 **3/5 (60.0%)**다. D3 Editor RightDock는 현재 project+conversation에 이미 durable하게 존재하거나 테스트에서 seed한 public memory candidate/storage state를 관리하며 실제 대화→candidate production E2E를 주장하지 않는다. 한 번의 명시적 `승인하고 저장` 클릭에서 approve 성공 뒤에만 새 request ID로 store한다. approve 실패·stale은 store 0, reload의 approved/not-requested는 자동 저장 0, live claimed는 비클릭 처리 중이고 lease가 지난 claimed만 explicit retry로 add reclaim 또는 reconcile한다. candidate/action/error와 conversation scroll은 Route가 소유해 drawer close/open 뒤에도 유지되고 memory 장애가 유진 대화·수동 편집·단일 PreviewStage를 막지 않는다. focused 검증은 memory store/API backend **49 passed**, typed API memory **20 passed**, D3 frontend **4 files / 178 passed**(memory panel **3**, EditorWorkbenchRoute 전체 **114** 포함), disposable PostgreSQL current-conversation-before-limit **1 passed**와 expired-claim add/reconcile parity **1 passed**다. 독립 spec/quality/gap/reverse 재검토는 **C0/I0/M0 PASS**다. 전체 Python regression, 전체 frontend suite, production build, provenance, 실제 Mem0/provider call, 브라우저 사람 E2E, 사용자 미디어·CapCut 사람 검증은 D3에서 실행하지 않았고 통과로 주장하지 않는다. 다음 작업은 **D4만**이다. 분모 변경 없이 현재 RightDock의 명시적 `기억 후보 만들기` 하나만 현재 owned conversation의 완료 message ID와 typed policy-safe candidate를 기존 POST에 보내는 production producer로 구현하고, 자동 create/approve/store/provider call 0을 유지한다. 이어 approved+stored만 bounded retrieval로 creator context에 주입하고 pending/rejected/deleted/failed/unrelated는 retrieval 0, unavailable/empty/malformed/timeout은 기억 없이 대화·수동 편집 계속, live canary는 별도 명시 실행으로 유지한다. Task 9 사람/환경 acceptance 전까지 기존 공식 누적은 **9/22 (40.9%)**, 잔여 **59.1%**로 고정한다.
