...
> **2026-08-03 Task 23A closeout:** HEVC lazy browser-preview proxy를 TDD로 완료했다. 호환 영상은 기존 URL, 비호환 영상은 원본 불변 project-local H264 cache를 사용하고, durable claim/restart recovery/source SHA fence/Range/route epoch/latest-request/one-player/manual fallback을 검증했다. 실제 사용자 HEVC 샘플과 합성 verifier도 통과했다. Task 23 production 진행률은 **1/4 (25.0%)**, 잔여 **75.0%**다. current authoritative status는 `§319`, closeout handoff는 `docs/handoffs/2026-08-03-videobox-task23a-browser-preview-proxy-closeout.ko.md`이며 다음 goal은 **23B owner one-click 실행·진단**이다.
> **2026-08-03 authoritative update:** 사용자 지시로 기존 공식 누적 `9/22 (40.9%)`, 잔여 `59.1%`는 폐기했다. 과거 문서의 해당 수치는 당시 human-acceptance 동결 판단을 보존하는 historical record이며 현재 진행률 계산에 사용하지 않는다. 새 active goal은 **Task 23 owner-ready MVP polish**이고 `23A HEVC local browser-preview proxy → 23B owner one-click 실행·진단 → 23C 사용자 샘플 repeatable edit package → 23D Hermes readiness smoke` 순서다. 현재 production 진행률은 **0/4 (0.0%)**, 잔여 **100.0%**이며 planning은 완료 수에 포함하지 않는다. current written design은 `docs/superpowers/specs/2026-08-03-videobox-task23-owner-ready-mvp-polish-design.md`, authoritative 상태는 development status `§318`이다. 다음 gate는 written spec 사용자 검토이며 승인 뒤 detailed plan과 23A TDD 구현을 시작한다.
> **2026-07-31 authoritative update:** 사용자 원본 5개를 read-only로 점검하고 격리 API ingest·SHA-256·Range 206, Chromium H264/HEVC 재생, 기존 600초 final/SRT/CapCut output 역추적, 실제 CapCut Desktop project open, fresh 600초 smoke를 수행했다. 샘플 5개 중 4개인 HEVC source audition은 Chromium에서 화면을 표시하지 못할 수 있어 `PreviewStage`가 이를 감지하고 exact 편집본 미리보기로 안내하도록 TDD 보완했다. 전체 Python **2640 passed, 47 skipped**, 전체 frontend **52 files / 727 passed**, 자동 Chromium editor E2E **35/35**, production build와 static verifiers를 통과했다. 인증된 Hermes provider/live Mem0와 사람의 영상·음악·효과음 취향·청취·저작권·export 승인은 별도다. Hermes Yujin 기술 initiative는 **20/20 (100.0%)**이며 VideoBox 공식 누적은 Task 9 사람/환경 acceptance 기준 **9/22 (40.9%)**, 잔여 **59.1%**로 유지한다. 현재 다음 goal은 데스크톱 복귀 후 대표 결과물을 사람이 직접 보고 듣는 Task 9 acceptance이며, 인증 정보가 준비되면 Hermes provider/live Mem0 canary를 별도 수행한다. 상세 근거는 status `§317`과 `docs/handoffs/2026-07-31-videobox-owner-dogfood-release-audit-closeout.ko.md`를 우선 적용한다.
> 현재 worktree 기준 Task 22A–F release parity 기술 closeout은 `docs/superpowers/plans/2026-07-23-videobox-task22-release-parity.md`와 최신 status/handoff를 우선 적용한다. 모바일 원격 환경의 owner dogfood/Task 9 사람·CapCut 실증은 별도로 보존하고, `docs/superpowers/plans/2026-07-26-videobox-hermes-yujin-master-plan.md`의 Hermes Yujin 20-task initiative는 Phase 0, A, B, C, D와 F1까지 **20/20 (100.0%)**, 잔여 **0.0%**로 기술 closeout했다. F1 역방향 감사에서 memory source authority, Route post-create list supersession, SQLite duplicate-column migration concurrency와 API fixture recovery race를 RED로 재현해 보완했다. memory candidate source는 같은 project+conversation의 completed Hermes run exact user/assistant message만 허용하고 다른 상태·legacy는 candidate/audit 전 거부한다. post-create fresh list가 이전 initial list를 supersede하며 candidate/action/draft/scroll을 보존한다. migration은 exact duplicate-column race만 schema 재확인 뒤 성공으로 수렴한다. 최종 검증은 focused backend **920 passed, 1 skipped**, 전체 Python **2640 passed, 47 skipped**, frontend focused **4 files / 183 passed**, 전체 frontend **52 files / 725 passed**, 자동 Chromium editor E2E **35/35**, production build, provenance/UI-system, external-runtime/network/UI **3 files / 8 passed**, runtime/profile/plan static verifier, non-live memory smoke 외부 호출 `0`, disposable PostgreSQL 16 Yujin-memory **6 passed 및 exact container cleanup**, CycloneDX 1.6 SBOM **341 components / 348 dependencies**, 독립 spec/quality/gap/reverse **C0/I0/M0 PASS**, `git diff --check`다. 기존 Starlette warning, React act/jsdom/ErrorBoundary stderr, E2E color 경고, 500kB bundle warning, SBOM의 documented optional-WASM npm tree 출력은 비실패 출력이다. 전체 Docker 구조는 필수 process-local placeholder 환경값을 준 `docker compose config --quiet`로 확인한다. 실제 Hermes/Mem0 provider live, 브라우저 사람 E2E, 사용자 미디어·CapCut 사람 검증은 실행하지 않았고 통과로 주장하지 않는다. 기존 VideoBox 공식 누적은 Task 9 사람/환경 acceptance 전까지 **9/22 (40.9%)**, 잔여 **59.1%**로 고정한다. 다음 goal은 모바일 복귀 후 owner dogfood/Task 9 사람·환경 acceptance다.
> 개발 운영 상위 규칙은 저장소 루트 `CLAUDE.md`와 `docs/development-fast-path.ko.md`의 `## 10. 고정 운영 규정`을 프로젝트 전역 기본값으로 적용한다. 즉, 이 계획서를 실행할 때의 작업 우선순위, 선택적 TDD/서브에이전트/리뷰 사용, 표준 검증 경로, hot path 구분, 커밋/푸시, 진행률 보고, turn closeout 형식은 해당 규정을 따른다.
...
### 4.1 캡컷과의 관계 — owner 확정 (2026-08-21)

이 절이 없어서 그동안 **"캡컷에 있으니 넣자"가 근거가 되지 못했다.** owner가
2026-08-21에 범위를 이렇게 확정했다.

> "아예 디자인을 나는 캡컷과 거의 동일하게 하고 고급기능을 뺀 나머지 기능도
> 캡컷을 벤치마킹하고, 다만 우리는 ai 를 붙여서 쉽게 편집을 할수 있고,
> 그 자산이 본인이 찍은 비롤 + 본인 음성 등등 이라는거야."

| | 방침 |
|---|---|
| 생김새 | **캡컷과 거의 동일하게.** 껍데기 배치는 `decisions/2026-08-21-capcut-shell-layout.ko.md` |
| 기능 | **캡컷을 벤치마킹한다.** 고급 기능만 뺀다 |
| 빼는 것 | 멀티캠, 임의 키프레임, 전문 색보정, 고급 마스크 — 위 `제외` 목록 그대로 |

**따라서 "캡컷에 있다"는 이제 검토 근거가 된다.** 다만 자동 채택은 아니다 —
고급 기능 목록에 걸리는지 먼저 보고, 아니면 만들 대상으로 올린다.

#### 4.1.1 `pycapcut`의 효과 목록 — 조사 기록 (2026-08-21)

> **2026-08-22 후속:** owner가 이 목록을 보고 **전환부터** 만들기로 정했고, 그
> 작업은 끝났다 — `xfade` 여섯 개다(§4.1.2). 아래 1,137이라는 숫자는 여전히
> **구현 대상이 아니라 조사 기록**이다.

**이 절은 조사 기록이다. 구현 지시가 아니다.** 무엇을 만들지는 owner가 따로 정한다.
여기 적힌 숫자를 보고 전환·필터를 만들기 시작하지 마라.

`pycapcut==0.0.3`은 이미 의존성에 있다(`requirements-runtime.txt`, `requirements-container.txt`).
그 안에 캡컷의 효과 이름표가 열거형으로 통째로 들어 있다. 설치본에서 직접 세었다.

| 열거형 | 개수 | 그중 캡컷 유료(`is_vip`) |
|---|---|---|
| `TransitionType` (장면 전환) | 1,137 | **985** |
| `VideoSceneEffectType` (화면 효과) | 1,583 | **1,269** |
| `FilterType` (필터) | 454 | **342** |
| `VideoCharacterEffectType` (인물 효과) | 254 | — |
| `IntroType` · `OutroType` (등장·퇴장) | 251 · 219 | — |
| `TextIntro` · `TextOutro` · `TextLoopAnim` (글자) | 182 · 100 · 81 | — |
| `AudioSceneEffectType` (소리 효과) | 213 | — |
| `FontType` (글꼴) | 348 | — |
| `MaskType` (마스크) | 6 | — |

**결정적으로, 이건 효과가 아니라 이름표다.** 열거형 항목 하나를 열어 보면 들어 있는
것이 이게 전부다.

```
{'name': 'Cutout Flip', 'is_vip': False, 'resource_id': '7387771481670816257',
 'effect_id': '7387771481670816257', 'md5': '290a8f067f8039b1060df3d1e8d07ca0',
 'default_duration': 800000, 'is_overlap': True}
```

`resource_id`와 `md5`는 **캡컷이 자기 서버에서 받아 둔 자원을 찾는 열쇠**다. 픽셀도,
셰이더도, 필터 수식도 없다. **우리 ffmpeg 렌더러는 이것으로 아무것도 그릴 수 없다.**
그러므로 이 1,137개를 우리 완성본 MP4에 그대로 가져오는 길은 없다.
...
`False`인 152개 안에서 고르는 것이 안전하다.

**(b) 우리 편집기에는 무엇을 만들지 정하는 근거로만 쓴다.** 목록 자체를 가져오는 게
...
**§4의 `제외` 목록은 그대로다.** 고급 마스크가 여기 `MaskType` 6개로 들어 있다고
해서 범위가 열리지 않는다. 이 절은 범위를 넓히지 않는다.

#### 4.1.2 장면 전환 — 만든 것 (2026-08-22)

캡컷 왼쪽 패널의 없는 넷(스티커·효과·전환·필터) 가운데 **전환**을 만들었다.
세부와 함정은 `handoffs/2026-08-22-videobox-scene-transitions-and-the-frame-rate-trap.ko.md`.

**처음엔 여섯 개, 2026-08-23에 방향 짝(`wiperight`·`slidedown`) 추가로 여덟 개다.**
ffmpeg `xfade`가 직접 그려 주는 것 중 생김새 갈래가 겹치지 않게 골랐다(반대 방향은
겹치는 게 아니라 반대라는 이유로 짝만 추가, `transitions.py` 주석 참고).

| 값 | 화면 문구 | 갈래 |
|---|---|---|
| `fade` | 서서히 겹치기 | 겹침 |
| `fadeblack` | 검게 저물기 | 겹침 |
| `dissolve` | 흩어지며 넘기기 | 겹침 |
| `wipeleft` | 왼쪽으로 쓸어내기 | 쓸기 |
| `wiperight` | 오른쪽으로 쓸어내기 | 쓸기 |
| `slideup` | 위로 밀어올리기 | 밀기 |
| `slidedown` | 아래로 밀어내리기 | 밀기 |
| `circleopen` | 원으로 열기 | 모양 |

**전체 길이는 움직이지 않는다.** 이 저장소의 렌더러는 concat이 아니라 캔버스
오버레이 모델이라, 전환을 들어오는 클립의 첫 구간 안에만 얹고 앞 장면의 **남은
원본**을 빌려 쓴다. 전체 길이·자막 위치가 그대로다.

**2026-08-31에 남은 둘을 채웠다:**
- **화면에서 전환 길이 조절**: 속성 패널의 "앞 장면에서 넘어오기"에 초 단위
  칸이 생겼다(`InspectorControls.tsx`). 범위는 백엔드와 같은 0.1~2.0초.
- **CapCut 내보내기에 얹기**: `pycapcut_adapter.py`가 이제 전환을 이름으로
  대응해 붙인다(`fade→叠化`, `wipeleft→向左擦除` 등, `_CAPCUT_TRANSITION_TYPE_BY_KEY`
  참고). **이 대응은 이름으로만 골랐고 실제 캡컷 화면으로 확인하지 않았다** —
  특히 `slideup`/`slidedown`/`circleopen`은 확신이 낮다. pycapcut은 전환을
  "앞 조각"에 걸므로(우리 모델은 "들어오는 쪽") 어댑터가 방향을 뒤집고,
  먼저 놓인 조각에 나중에 전환을 붙이는 순서라 소재 등록도 직접 다시
  해야 했다(자동 등록은 `add_segment` 호출 시점의 값만 본다).

**2026-08-31에 유진 추천도 채웠다(owner: "너가 할수 있는거 먼저 진행해줘").**
`suggest_scene_transitions()`(`transitions.py`)는 신호 하나만 본다 — **앞 장면과
이 장면의 B-roll 자산이 다른가.** 다르면 `fade`를 추천하고, 이미 전환이 있거나
B-roll 신호가 없으면 아무 말도 하지 않는다. **대본 내용·움직임 방향·음악
분위기는 안 본다** — 그래서 `wipeleft`/`slideup` 같은 방향 있는 전환은 이
v1에서 절대 추천하지 않는다. 화면은 `GET .../transition-suggestions`로 받고,
"적용"은 owner가 쓰는 것과 **같은** `PATCH .../segments/{id}/transition`
엔드포인트를 `chosen_by: "yujin"`으로 부른다 — 새 적용 경로를 만들지 않았다.
기존 `DirectorCandidate` 추천 파이프라인(자산 배치)에 끼워 넣지 않고 이렇게
따로 둔 이유: 그 모델은 `asset_id`가 필수라 자산 없는 전환 추천을 넣으려면
그 파이프라인 전체를 흔들어야 했다.

### 4.2 차별점은 자산이 아니라 **고르는 일**이다 — owner 정정 (2026-08-21)

처음에 "본인이 찍은 B-roll과 본인 목소리를 쓴다"를 차별점으로 적었다가 owner가
바로잡았다.

> "캡컷도 본인 비롤 영상을 업로드 할수 있는데, 우리 차별성은 유진이가 대본에
> 있는 내용에서 영상, 음악, 효과음 등등을 추천해준다는거잖아."

**맞다. 자산을 가진 것은 차별점이 아니다.** 캡컷도 올린다. 다른 것은 이쪽이다 —
**대본을 읽고 장면마다 어떤 영상·음악·효과음을 쓸지 유진이 골라 준다.**

캡컷에서 그 일은 전부 사람이 한다. 장면마다 라이브러리를 뒤져서 고르고, 길이를
맞추고, 음량을 잡는다. 편집 시간의 대부분이 거기 있다.

**2026-08-22 owner가 한 겹 더 날카롭게 했다 — 이유가 취향이 아니라 계정이다.**

> "우리 장점은 누구나 만드는 영상이 아니라, 자신만의 비롤 영상으로 ai 영상이 아니라
> 본인만의 자산으로 만든 영상이라서 유튜브에 계정정지를 당할 사유가 없다는 거야"

**이건 "더 예쁘다"가 아니라 "안 잘린다"는 이야기다.** 유튜브는 생성물로 대량 제작한
채널을 제재하고, 캡컷 스톡·템플릿으로 만든 영상은 남들 것과 그림이 겹친다. 본인이
찍은 촬영본으로 만든 영상에는 그 사유가 없다.

**그래서 유진의 일은 "고르기"이되, 고르는 대상이 본인 자산이라는 점이 핵심이다.**
둘을 떼면 뜻이 무너진다 — 스톡에서 골라 주는 것이었으면 캡컷과 같아지고, 고르는
일이 없으면 그냥 폴더 업로드다. **고르기 + 본인 자산, 둘이 한 문장이다.**

이 문장은 owner가 외부 강의에서 이 제품을 소개할 때 쓸 말이기도 하다. 화면 배치를
캡컷과 똑같이 맞추는 이유도 여기에 붙는다 — 처음 본 사람이 설명 없이 쓸 수 있어야
한다(`docs/decisions/2026-08-21-capcut-shell-layout.ko.md`).

이 판단이 실무에 미치는 영향:

- **추천·색인·의미검색은 부수 기능이 아니라 제품의 중심이다.** `media_ranking.py`,
  `recommenders.py`, `library_*_indexer.py`가 그 자리다. 이것들이 사이드바 한 줄로만
  노출돼 있다면 그건 배치가 제품을 잘못 말하고 있는 것이다.
- **기능을 견줄 때 기준이 둘이다.** "캡컷에 있는가"와 "유진이 대신 해 줄 수 있는가".
  뒤쪽이 우리 쪽 값어치이므로, 같은 기능이라도 **사람이 고르게 만들면 절반만 만든 것**이다.
...
- closeout에는 계획서 준수 점검을 남길 것
- closeout 형식은 `docs/development-fast-path.ko.md` `§10.9`를 따를 것.
  다음 goal 프롬프트와 매 턴 진행률 보고는 2026-08-05에 제거했다.
  진행률은 `§10.8`에 따라 Task를 실제로 열거나 닫을 때만 보고한다.
