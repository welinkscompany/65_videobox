...
**여섯 개다.** ffmpeg `xfade`가 직접 그려 주는 것 중 생김새 갈래가 겹치지 않게 골랐다.
...
**길이는 움직이지 않는다.** 이 저장소의 렌더러는 concat이 아니라 캔버스 오버레이
모델이라, 전환을 들어오는 클립의 첫 구간 안에만 얹고 앞 장면의 **남은 원본**을
빌려 쓴다. 전체 길이·자막 위치가 그대로다.

**아직 아닌 것:** 유진 추천(값을 실을 자리 `chosen_by`는 준비됨), 화면에서 전환
길이 조절, CapCut 내보내기에 얹기.
