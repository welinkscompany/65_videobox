...
> 현재 worktree 기준 Task 22A–F release parity 기술 closeout은 `docs/superpowers/plans/2026-07-23-videobox-task22-release-parity.md`와 최신 status/handoff를 우선 적용한다. 모바일 원격 환경에서 owner dogfood/Task 9 사람·CapCut 실증은 별도로 보존하고, 현재 승인된 implementation goal은 `docs/superpowers/plans/2026-07-26-videobox-hermes-yujin-master-plan.md`의 Hermes Yujin 20-task initiative다. Phase 0의 P0-1/P0-2와 Phase A의 A1–A4 비라이브 기술 checkpoint를 완료했다. 현재 상태는 **6/20 (30.0%)**, 잔여 **70.0%**다. Phase A는 **4/4 (100.0%)**, runtime/chat child는 **6/6 (100.0%)**, 잔여 **0.0%**다. workspace↔gateway와 gateway↔Hermes는 서로 다른 internal network이며 Hermes만 provider egress를 가진다. 기술 fallback 테스트·frontend 전체·build·E2E·focused backend·static verifier는 통과했다. 반면 live provider canary와 실제 Hermes service-stop/manual environment 증거는 실행하지 않았고 통과로 간주하지 않는다. 이는 별도 환경이 구성되면 사용할 첫 useful checkpoint의 코드 완료이지 live 실증 완료가 아니다. 다음 작업은 **B1만**이다. Task 9 사람/환경 acceptance 전까지 기존 공식 누적은 **9/22 (40.9%)**, 잔여 **59.1%**로 고정한다.
