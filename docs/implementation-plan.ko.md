...
> 현재 worktree 기준 Task 22A–F release parity 기술 closeout은 `docs/superpowers/plans/2026-07-23-videobox-task22-release-parity.md`와 최신 status/handoff를 우선 적용한다. 모바일 원격 환경에서 owner dogfood/Task 9 사람·CapCut 실증은 별도로 보존하고, 현재 승인된 implementation goal은 `docs/superpowers/plans/2026-07-26-videobox-hermes-yujin-master-plan.md`의 Hermes Yujin 20-task initiative다. Phase 0의 P0-1/P0-2, A1의 격리 runtime topology/static startup 검증, A2의 versioned Yujin Soul/profile/skills package와 container-only install/static ownership 검증, A3의 zero-tool Hermes transport와 durable run/SSE boundary lifecycle correction 및 stateful output quarantine Critical correction까지 완료했다. 현재 상태는 **5/20 (25.0%)**, 잔여 **75.0%**다. Phase A는 **3/4**, runtime/chat child는 **5/6 (83.3%)**, 잔여 **16.7%**다. workspace↔gateway와 gateway↔Hermes는 서로 다른 internal network이며 Hermes만 provider egress를 가진다. 실제 서비스·live HTTP·OAuth·provider/chat/Mem0·실제 PostgreSQL 연결은 실행하지 않았다. 다음 작업은 **A4만**이다. Task 9 사람/환경 acceptance 전까지 기존 공식 누적은 **9/22 (40.9%)**, 잔여 **59.1%**로 고정한다.
