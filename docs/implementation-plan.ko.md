...
> 현재 worktree 기준 Task 22A–F release parity 기술 closeout은 `docs/superpowers/plans/2026-07-23-videobox-task22-release-parity.md`와 최신 status/handoff를 우선 적용한다. 모바일 원격 환경의 owner dogfood/Task 9 사람·CapCut 실증은 별도로 보존하고, 현재 승인된 implementation goal은 `docs/superpowers/plans/2026-07-26-videobox-hermes-yujin-master-plan.md`의 Hermes Yujin 20-task initiative다. Phase 0 P0-1/P0-2, Phase A A1–A4, Phase B B1–B5와 Phase C C1을 완료했다. 현재 상태는 **12/20 (60.0%)**, 잔여 **40.0%**이며 realtime-reliability child는 **1/4 (25.0%)**다. C1은 기존 run/message model을 확장해 public SSE event와 cursor를 DB replay truth로 저장한다. begin/draft/terminal은 user·assistant·proposal과 원자적으로 묶이며, process restart 뒤 completed suffix 또는 startup `interrupted`를 provider 재호출 없이 복구한다. 반복 GET, strict `Last-Event-ID`, scope 404, malformed 400, future 409, pruned 410을 지원하고 disconnect는 provider run을 취소하지 않는다. draft/cancel과 terminal list/status 경쟁조건, terminal 저장 실패의 가짜 event 금지, 30일·최신 128개 retention, SQLite/PostgreSQL legacy backfill을 TDD로 닫았다. 실제 PostgreSQL에서 cursor/terminal concurrent winner와 legacy replay를 검증했다. C2 frontend reconnect/cancel/retry, C3 capability, C4 dashboard, Mem0, 자동 apply, source copy, OpenCut runtime, provider/API 확대는 열지 않았다. C1 focused **92 passed**, 전체 Python **2168 passed, 25 skipped**, 실제 PostgreSQL+compatibility **31 passed**, 전체 frontend **50 files / 657 passed**, production build, profile/runtime/plan-state/zero-tools/provenance/UI-system verifier, `py_compile`, `git diff --check`, 독립 spec/quality/gap/reverse review가 통과했다. 실제 provider, 사용자 미디어·CapCut 사람 검증은 실행하지 않았고 통과로 간주하지 않는다. 다음 작업은 **C2만**이다. Task 9 사람/환경 acceptance 전까지 기존 공식 누적은 **9/22 (40.9%)**, 잔여 **59.1%**로 고정한다.
