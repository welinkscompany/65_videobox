...
> 현재 worktree 기준 Task 22A–F release parity 기술 closeout은 `docs/superpowers/plans/2026-07-23-videobox-task22-release-parity.md`와 최신 status/handoff를 우선 적용한다. 모바일 원격 환경의 owner dogfood/Task 9 사람·CapCut 실증은 별도로 보존하고, 현재 승인된 implementation goal은 `docs/superpowers/plans/2026-07-26-videobox-hermes-yujin-master-plan.md`의 Hermes Yujin 20-task initiative다. Phase 0 P0-1/P0-2, Phase A A1–A4와 Phase B B1–B5를 완료했다. 현재 상태는 **11/20 (55.0%)**, 잔여 **45.0%**이며 Phase B와 creator-tools child는 **5/5 (100.0%)**다. B5는 실제 RightDock→typed proposal→명시적 단일 Apply→current-revision session/manifest refresh→기존 output route를 UI/E2E와 real local store/API non-live smoke로 역방향 검증했다. 추천 표시·radio 선택 전 mutation은 0이며, Inspector close/open 뒤 conversation/candidate/draft/scroll/player 상태와 단일 PreviewStage 소유권을 유지한다. Hermes unavailable·위조 후보·output pending/running/failed/unavailable에서도 editor/output mutation은 0이고 manual editor fallback이 남는다. 재개 코드리뷰에서 segment caption style playback, non-image candidate terminal attestation, TTS exact asset revision/file SHA, exact-clone Live root binding을 보완했다. Live smoke는 secret-gated nonce HMAC으로 server canonical root를 첫 POST 전에 묶으며 다른 절대 root clone이면 POST 0으로 차단한다. 실제 PostgreSQL 16에서 변조 TTS의 전체 session rollback도 증명했다. 미지원 effect, 자동 apply, source copy, OpenCut runtime, provider/API 확대, Mem0, SaaS는 열지 않았다. 기존 closeout의 전체 frontend **50 files / 657 passed**, editor E2E **9 passed**, 전체 Python **2139 passed, 21 skipped**에 더해 2026-07-30 재검증 focused Python **83 passed**, frontend **3 files / 127 passed**, production build, PostgreSQL store **16 passed**, targeted TTS rollback **1 passed**, non-live creator smoke와 독립 spec/quality/gap/reverse review가 통과했다. 실제 `-Live`, live provider, 전체 Docker topology, 사용자 미디어·CapCut 사람 검증은 실행하지 않았고 통과로 간주하지 않는다. 다음 작업은 **C1만**이며 durable run/event cursor와 final/interrupted conversation 복구를 current route/session/revision 경계와 manual fallback을 유지한 채 TDD로 구현한다. Task 9 사람/환경 acceptance 전까지 기존 공식 누적은 **9/22 (40.9%)**, 잔여 **59.1%**로 고정한다.
