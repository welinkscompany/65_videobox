...
> 현재 worktree 기준 Task 22A–F release parity 기술 closeout은 `docs/superpowers/plans/2026-07-23-videobox-task22-release-parity.md`와 최신 status/handoff를 우선 적용한다. 모바일 원격 환경의 owner dogfood/Task 9 사람·CapCut 실증은 별도로 보존하고, 현재 승인된 implementation goal은 `docs/superpowers/plans/2026-07-26-videobox-hermes-yujin-master-plan.md`의 Hermes Yujin 20-task initiative다. Phase 0 P0-1/P0-2, Phase A A1–A4, Phase B B1–B5와 Phase C C1–C3를 완료했다. 현재 상태는 **14/20 (70.0%)**, 잔여 **30.0%**이며 realtime-reliability child는 **3/4 (75.0%)**다. C3는 Gateway 전용 Ed25519 private key와 API 전용 public key, 정확히 한 번만 사용할 수 있는 read/publish capability, durable issue/consume/replay/revoke ledger와 redacted audit, current session·asset truth 재검증, 안전한 rotation/startup 검증을 기존 reserve→attach→stream 경로에 추가했다. read admission/preparation 실패는 provider/attach 전에 막고, provider stream 뒤 publish terminal 실패는 proposal/editor mutation 0과 manual fallback으로 닫는다. C3 focused suite **468 passed, 33 skipped**, 실제 PostgreSQL 16 **38 passed, skip 0**, 전체 Python **2367 passed, 41 skipped**, 전체 frontend **50 files / 668 passed**, production build, profile/runtime/plan-state/zero-tools/provenance/UI-system verifier, `git diff --check`, 독립 spec 및 quality/gap/reverse review가 통과했다. 실제 Hermes/provider 호출, 브라우저 사람 E2E, 사용자 미디어·CapCut 사람 검증은 실행하지 않았고 통과로 간주하지 않는다. 다음 작업은 **C4만**이다. dashboard health/restart/fallback 상태가 HTTP readiness와 실제 chat verification을 구분하고, named Yujin service만 안전하게 재시작하며, 장애 drill 뒤에도 수동 편집을 유지하도록 구현한다. Task 9 사람/환경 acceptance 전까지 기존 공식 누적은 **9/22 (40.9%)**, 잔여 **59.1%**로 고정한다.
