...
> 현재 worktree 기준 Task 22A–F release parity 기술 closeout은 `docs/superpowers/plans/2026-07-23-videobox-task22-release-parity.md`와 최신 status/handoff를 우선 적용한다. 모바일 원격 환경의 owner dogfood/Task 9 사람·CapCut 실증은 별도로 보존하고, 현재 승인된 implementation goal은 `docs/superpowers/plans/2026-07-26-videobox-hermes-yujin-master-plan.md`의 Hermes Yujin 20-task initiative다. Phase 0 P0-1/P0-2, Phase A A1–A4와 Phase B B1–B4를 완료했다. 현재 상태는 **10/20 (50.0%)**, 잔여 **50.0%**이며 Phase B와 creator-tools child는 **4/5 (80.0%)**, 잔여 **20.0%**다. B4는 실제 backend가 지원하는 자막 문구/완전한 스타일, 청취 승인 TTS, explanation/image/table 오버레이만 explicit single Apply에 연결하고, `timeline_gaps`만 backend-attested read-only finding으로 노출한다. strict gateway DTO와 terminal transaction이 TTS/image/gap/current-session truth를 다시 검증하며 generalized B3+B4 mode도 B3 media만 preview/materialize한다. 미지원 effect, 자동 apply, source copy, OpenCut runtime, provider/API 확대, Mem0, SaaS는 열지 않았다. controller focused Python **373 passed, 1 skipped**, 전체 frontend **50 files / 648 passed**, production build, profile/runtime/plan-state/zero-tools/provenance/UI-system verifier와 독립 spec/quality/gap/reverse review가 통과했다. 전체 Python, editor E2E, live provider/Docker/PostgreSQL, 사용자 미디어·CapCut 사람 검증은 실행하지 않았고 통과로 간주하지 않는다. 다음 작업은 **B5만**이며 explicit Apply 전 mutation 0, one-player preview, output reverse smoke, state preservation, Hermes unavailable manual fallback을 non-live real-store smoke와 UI/E2E로 증명해 Phase B를 closeout한다. Task 9 사람/환경 acceptance 전까지 기존 공식 누적은 **9/22 (40.9%)**, 잔여 **59.1%**로 고정한다.
