...
> 현재 worktree 기준 Task 22A–F release parity 기술 closeout은 `docs/superpowers/plans/2026-07-23-videobox-task22-release-parity.md`와 최신 status/handoff를 우선 적용한다. 모바일 원격 환경의 owner dogfood/Task 9 사람·CapCut 실증은 별도로 보존하고, 현재 승인된 implementation goal은 `docs/superpowers/plans/2026-07-26-videobox-hermes-yujin-master-plan.md`의 Hermes Yujin 20-task initiative다. Phase 0 P0-1/P0-2, Phase A A1–A4, Phase B B1–B5, Phase C C1–C4와 Phase D D1을 완료했다. 현재 상태는 **16/20 (80.0%)**, 잔여 **20.0%**이며 Mem0 child는 **1/5 (20.0%)**다. D1은 VideoBox에 `creator` 범위의 memory candidate와 명시적 approve/reject workflow만 추가했다. source message 소유권·순서, canonical idempotency, 정책과 durable store 이중 guard, 원자적 body-free audit, 동일 시각에도 뒤집히지 않는 monotonic `event_order`, SQLite/PostgreSQL parity를 고정했다. Gateway/Hermes/Mem0/provider/network/editor mutation은 0이고 manual fallback과 자동 Apply 금지를 유지한다. D1 focused **96 passed**, 관련 Director/API/PostgreSQL 회귀 **39 passed, 34 skipped**, disposable PostgreSQL 16 D1 workflow **1 passed**, `py_compile`, `git diff --check`, 독립 spec 및 quality/gap/reverse review가 통과했다. 전체 Python/frontend/build는 D1에서 다시 실행하지 않았고 통과로 주장하지 않는다. 실제 Mem0/provider 호출, 브라우저 사람 E2E, 사용자 미디어·CapCut 사람 검증도 실행하지 않았다. 다음 작업은 **D2만**이다. 승인된 candidate만 Hermes 소유 Mem0 adapter로 전달하되 자격증명과 raw provider record를 browser/VideoBox store에 노출하지 않고 local/test external call 0을 유지한다. Task 9 사람/환경 acceptance 전까지 기존 공식 누적은 **9/22 (40.9%)**, 잔여 **59.1%**로 고정한다.
