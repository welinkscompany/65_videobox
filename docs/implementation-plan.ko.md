...
> 현재 worktree 기준 Task 22A–F release parity 기술 closeout은 `docs/superpowers/plans/2026-07-23-videobox-task22-release-parity.md`와 최신 status/handoff를 우선 적용한다. 모바일 원격 환경의 owner dogfood/Task 9 사람·CapCut 실증은 별도로 보존하고, 현재 승인된 implementation goal은 `docs/superpowers/plans/2026-07-26-videobox-hermes-yujin-master-plan.md`의 Hermes Yujin 20-task initiative다. Phase 0 P0-1/P0-2, Phase A A1–A4, Phase B B1–B5와 Phase C C1/C2를 완료했다. 현재 상태는 **13/20 (65.0%)**, 잔여 **35.0%**이며 realtime-reliability child는 **2/4 (50.0%)**다. C2는 최초 SSE open을 포함한 `100/250/500ms` bounded reconnect, strict durable cursor 중복 제거, route-owned cancel, blocked/interrupted explicit retry를 기존 run/message store 위에 추가했다. RightDock close는 실행을 유지하고 route 이동·unmount만 정확히 한 번 취소한다. reconnect가 소진돼도 실행 소유권과 명시적 취소를 보존하며 새 Hermes/legacy Director 시작만 막고 수동 편집은 유지한다. prompt acceptance 전 정확한 ticket 만료만 한 번 재발급하고 acceptance 뒤 provider 자동 재시도는 하지 않는다. SQLite/PostgreSQL의 `retry_of_run_id`와 durable `message_order` migration으로 linked retry와 동일 timestamp user/assistant 순서를 보존한다. C2 focused Python **194 passed**, focused frontend **2 files / 137 passed**, 전체 Python **2192 passed, 29 skipped**, 실제 PostgreSQL **23 passed**, 전체 frontend **50 files / 668 passed**, production build, profile/runtime/plan-state/zero-tools/provenance/UI-system verifier, `py_compile`, `git diff --check`, 독립 spec 및 quality/gap/reverse review가 통과했다. 실제 Hermes/provider 호출, 브라우저 사람 E2E, 사용자 미디어·CapCut 사람 검증은 실행하지 않았고 통과로 간주하지 않는다. 다음 작업은 **C3만**이다. 기존 attach/stream topology와 durable ledger를 재사용하는 source-grounded capability 계획 보정부터 시작하고, 임의 gateway→API callback, apply/render/export/filesystem capability는 추가하지 않는다. Task 9 사람/환경 acceptance 전까지 기존 공식 누적은 **9/22 (40.9%)**, 잔여 **59.1%**로 고정한다.
