...
> 현재 worktree 기준 Task 22A–F release parity 기술 closeout은 `docs/superpowers/plans/2026-07-23-videobox-task22-release-parity.md`와 최신 status/handoff를 우선 적용한다. 모바일 원격 환경의 owner dogfood/Task 9 사람·CapCut 실증은 별도로 보존하고, 현재 승인된 implementation goal은 `docs/superpowers/plans/2026-07-26-videobox-hermes-yujin-master-plan.md`의 Hermes Yujin 20-task initiative다. Phase 0 P0-1/P0-2, Phase A A1–A4, Phase B B1–B5, Phase C C1–C4와 Phase D D1–D4를 완료했다. D4는 현재 RightDock의 명시적 `기억 후보 만들기` 한 경로만 현재 owned conversation의 완료 durable message ID 1–8개와 typed safe candidate를 기존 POST로 보내며 자동 create/approve/store/provider write/apply는 `0`이다. 새 owned durable dispatch만 approved+stored 기억을 1회·750ms·top 5로 검색하고 exact local private mapping을 재검증해 ID 없는 advisory preference 최대 5개·총 1,400자로 주입하며, 48KiB 초과 시 memory를 먼저 제거한다. unsafe/replay/timeout/outage/empty는 검색 `0` 또는 `memories=()`로 수렴해 대화·수동 편집·단일 PreviewStage를 유지한다. 현재 상태는 **19/20 (95.0%)**, 잔여 **5.0%**, Mem0 child **4/5 (80.0%)**다. 검증은 backend memory **149 passed**, 직접 영향 creator-context/Gateway/Hermes/API **188 passed**, frontend focused **3 files / 173 passed**, 전체 frontend **52 files / 724 passed**, disposable PostgreSQL 16 retrieval-row SQLite parity **1 passed 및 exact container cleanup 확인**, production frontend build, non-live smoke `network_calls=0 provider_calls=0`, plan verifier, `git diff --check`, 독립 spec/quality/gap/reverse **C0/I0/M0 PASS**다. 전체 Python regression, provenance, 실제 Mem0/provider live add/search/delete, 브라우저 사람 E2E, 사용자 미디어·CapCut 사람 검증은 D4에서 실행하지 않았고 통과로 주장하지 않는다. 다음 작업은 **F1 최종 통합 closeout만**이며, Task 9 사람/환경 acceptance 전까지 기존 공식 누적은 **9/22 (40.9%)**, 잔여 **59.1%**로 고정한다.
