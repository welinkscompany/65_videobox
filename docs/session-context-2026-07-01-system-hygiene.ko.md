...
- 현재 worktree 기준 backend full regression은 `305 passed`다
...
- backend full regression은 현재 direct 실행 기준으로 `305 passed`까지 다시 확인됐다
...
- current-focused helper 재검증 기준으로 backend output-gating slice는 현재 `11 passed`다
