...
- 현재 worktree 기준 backend full regression은 `313 passed`다
...
- backend full regression은 현재 direct 실행 기준으로 `313 passed`까지 다시 확인됐다
...
- 이후 fresh `pytest -q` 전체 회귀를 다시 캡처해 `313 passed in 724.93s`를 확인했고, 최신 full-backend baseline을 그 수치로 갱신했다
...
- current-focused helper 재검증 기준으로 backend output-gating slice는 현재 `17 passed`다
