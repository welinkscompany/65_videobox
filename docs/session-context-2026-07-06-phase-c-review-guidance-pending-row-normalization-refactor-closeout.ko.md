# 2026-07-06 Phase C review guidance pending row normalization refactor closeout

## 이번 턴에서 한 일

- `packages/core-engine/src/videobox_core_engine/review_guidance.py` 안에서 pending recommendation prompt row 정규화 코드를 helper 1개로 공통화했습니다.
...
- review guidance prompt는 blocking pending recommendation 판별과 prompt row canonicalization이 같은 파일 안에서 따로 흩어져 있어, 나중에 trim/lower/default 규칙이 한쪽만 바뀌면 blocked guidance prompt surface 기준이 다시 어긋날 수 있었습니다.

## 변경 범위

- 제품 동작 변경 없음
- review guidance prompt 내부 helper 공통화만 수행
...
- review/output prompt normalization 중복 중 남은 후보가 더 있는지 다시 좁힙니다.
