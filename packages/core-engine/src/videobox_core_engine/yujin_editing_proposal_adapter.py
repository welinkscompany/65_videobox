...
class YujinEditingContext:
    ...
    #: 지금 화면 맞춤이 걸린 장면과 그 값(`fit`/`crop`/`blur`). 색감·전환·사진
    #: 움직임과 **같은 이유**로 준다 -- 목록과 지금 걸린 값은 한 쌍이고, 지금 값을
    #: 빼면 "원래대로 돌려줘"가 막힌다. 2026-09-12에 대표님이 "좌우가 잘렸다"를
    #: 말로 고칠 수 있게 하면서 같이 들어왔다.
    fits_by_segment: tuple[tuple[str, str], ...] = ()
