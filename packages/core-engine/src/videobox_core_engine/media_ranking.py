...
def rank_candidates(segment: dict[str, Any], assets: list[dict[str, Any]], preferences: dict[str, list[str]] | None = None, weights: dict[str, float] | None = None) -> list[DirectorCandidate]:
    ...
    for asset in assets:
        ...
        controls = dict(asset.get("controls") or {})
        if media_type in {"bgm", "sfx"}:
            controls.setdefault("gain_db", 0.0)
        candidate = DirectorCandidate(candidate_id=f"candidate:{asset_id}", visible_reference_code=f"P01-{letter}-00", media_type=media_type, asset_id=asset_id, library_asset_id=asset.get("library_asset_id"), reason_chips=tuple(sorted(target_words & tags)) or ("metadata",), scores=scores, availability=str(asset.get("availability", "available")), review_status=str(asset.get("review_status", "approved")), preview_uri=asset.get("preview_uri"), controls=controls, expected_content_sha256=asset.get("content_sha256"), media_revision=str(asset.get("media_revision") or f"unversioned:{asset_id}"), canonical_metadata=metadata, license_policy="unknown_user_owned" if user_owned_unknown else "verified", warning_provenance=warning_provenance)
