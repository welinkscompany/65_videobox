...
def _image_overlay_catalogue(context: YujinEditingContext) -> str:
    ...
    return (
        ...
        # **여기도 Task 4(2026-09-11, 이 계획)에서 고친 자리다.** 예전 문구는
        # "안 물어본 칸을 채우면 안 된다"까지만 말했지, 채우지 않았을 때
        # 그 칸이 어떻게 되는지(지금 값 유지)는 말하지 않았다. 안내문이
        # 이걸 말 안 하면 유진은 "말 안 하면 지워질까 봐" 넷을 매번 다
        # 채워 보내려 하거나, 반대로 지워지는 줄 모르고 안 채워서 대표님이
        # 겪은 결함을 그대로 반복한다 -- 목록과 안내문은 한 쌍이라는
        # 교훈(전환·색감·자산 목록)이 여기도 그대로 걸린다.
        "넷은 **말한 것만 싣는다** -- 말 안 한 칸은 지금 걸린 값을 그대로 두고 지우지 않는다"
        "(예: 크기만 말하면 자리·움직임은 그대로다). 채워 넣으면 그 칸이 실제로 바뀐다 -- "
        "바꿀 뜻이 없는 칸까지 채우면 의도치 않게 값이 바뀌니 정말 바꿀 칸만 채워라. "
        "프리셋 하나만 콕 집어 안 고름으로 되돌리는 말은 "
        "따로 없다 -- 자리를 통째로 다시 정하고 싶으면 얹은 것을 뺐다가(remove_image_overlay) "
        "새로 얹어라. "
...
def _approved_asset_catalogue(context: YujinEditingContext) -> str:
    ...
    return (
        ...
        # 얹을 수 있는 것을 **보이는 것 전부**로 넓혔다(2026-09-10). 예전에는
        # 사진뿐이었는데, 그 근거("영상은 한 장으로 읽힌다")가 사실이 아니었다.
        # 소리만 있는 자산은 화면에 그릴 것이 없어 그대로 못 얹는다.
        "`set_image_overlay`로 화면 위에 얹을 수 있는 것은 **보이는 자산**(사진·영상)뿐이다 -- 소리(음악·효과음)는 못 얹는다."
    )
...
def _short_form_catalogue(context: YujinEditingContext) -> str:
    ...
    **reply_text 지시가 따로 필요한 이유(2026-09-12 실물 확인).** 실제 화면
    채팅으로 "숏폼 다시 만들어줘"를 시켜 보니, operations는 정확히
    `remake_short_form`을 골랐는데(변형본이 실제로 갱신됨) reply_text는 "숏폼"을
    숏폼 **영상**이 아니라 숏폼 **대본**(내레이션 원고)으로 읽어 55초짜리 대본
    전체를 답으로 써 버렸다 -- 기능은 맞게 동작했지만 답이 딴소리라 대표님이
    혼란스러울 뻔했다. 그래서 이 셋만은 reply_text가 무엇을 말해야 하는지까지
    못박는다 -- 대본을 쓰지 말고, 영상 장면을 고르는 중이라고만 짧게 말한다.

    **render_short_form은 더 심한 사례가 실물에서 나왔다(2026-09-13).**
    operations는 정확히 render_short_form을 골라 렌더가 실제로 성공했는데
    (완성본 mp4까지 나옴), reply_text는 "저는 실제로 파일을 내보내거나
    렌더링을 직접 실행하지는 않지만..."이라며 **실제로 한 일을 안 했다고
    부정하고**, 있지도 않은 "숏폼 생성 요청" 단추를 누르라고 안내했다 --
    작은 로컬 모델이 "나는 실행 못 하는 텍스트 도우미"라는 일반적인 습성으로
    빠진 것으로 보인다. 그래서 이 항목만 "실제로 이미 시작됐다"를 강하게
    반복해 못박는다.
    """
    if context.has_short_form_variant:
        return (
            ...
            "'다시'와 같은 뜻이다. **여기서 '숏폼'은 세로 짧은 영상이지 내레이션 "
            "대본이 아니다** -- reply_text에 대본이나 자막 문구를 새로 지어 쓰지 "
            "말고 '숏폼 장면을 다시 골라 볼게요' 정도로 짧게만 답한다. "
            ...
            "뽑아 달라는 요청에는 render_short_form을 쓴다. **이 의도를 골라 "
            "보내는 순간 서버가 실제로 렌더를 이미 시작한다 -- 버튼을 누르라고 "
            "안내하거나, 직접 실행하지 않는다고 답하면 그건 거짓말이다(실제로는 "
            "이미 실행되고 있다).** 렌더는 분 단위로 걸려 이 응답 안에서 "
            "끝나지는 않지만, 시작 자체는 이미 됐다. reply_text는 반드시 "
            "'숏폼을 만들고 있어요. 출력 화면에서 확인해 주세요.'라는 한 문장만 "
            "쓴다 -- 대본·자막·썸네일 문구를 덧붙이거나, 몇 초 걸린다거나 "
            "다 됐다고 지어 말하지 않는다."
        )
    return (
