...
#: 사진에게는 사진이라고 물어야 한다(2026-09-06 실측). 같은 말로 물었더니 모델이
#: `"이 영상은 낮 시간대 일본의 한 도시 거리..."`로 요약했다. 유진이 장면마다
#: 자산을 고를 때 대조하는 것이 이 문장이라, 대본에 "사진"이 나올 때 겹칠 수가
#: 없었다. 창작자가 읽어도 어느 쪽인지 헷갈린다.
STILL_VISION_ANALYSIS_PROMPT = (
    "이 사진을 분석해라. 사진 한 장이다 -- 움직임이나 장면 전환을 지어내지 마라. "
    "모든 값과 요약을 한국어로만 써라. 영어 단어를 쓰지 마라."
)
