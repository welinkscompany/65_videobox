...
class MediaAnalysisService:
    ...
    def __init__(self, *, store: Any, media_probe: Any, vision_provider: Any, clock: Callable[[], datetime] | None = None, extractor_version: str = "v1") -> None:
    ...
    def enqueue_analysis(self, *, project_id: str, asset_id: str, profile: AnalysisProfile | None = None) -> dict[str, Any]:
        ...
        return self.store.create_media_analysis(project_id=project_id, asset_id=asset_id, idempotency_key=f"{source_sha}:{key}", cache_key=key)
    ...
    def _dispatch_once(self, *, project_id: str, analysis_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            response = self.vision_provider.analyze_images(VisionAnalysisRequest(model_name="local", prompt="Analyze local media", images=images, response_schema=FIXED_VISION_RESPONSE_SCHEMA))
            if self._cancelled(project_id, analysis_id):
                return None
