...
- VideoBox는 **creator-complete MP4-first 경량 편집기**다. 컷, 자막, B-roll, 음악, 효과음, 연결된 가로·세로 변형,
  검토와 MP4 출력을 VideoBox 안에서 끝낸다.
- **캡컷을 벤치마킹한다** (owner 확정 2026-08-21, §4.1). 생김새는 거의 동일하게,
  기능은 아래 고급 항목만 빼고. **"캡컷에 있다"는 검토 근거가 된다** — 자동 채택은 아니다.
- **차별점은 자산이 아니라 고르는 일이다** (§4.2). 캡컷도 본인 영상을 올린다.
  다른 것은 **대본을 읽고 장면마다 뭘 쓸지 유진이 골라 준다**는 것이다.
  추천·색인·의미검색은 부수 기능이 아니라 제품의 중심이다.
