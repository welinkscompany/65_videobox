...
| **최신 세션 인계** | `docs/handoffs/2026-08-16-videobox-real-use-defects-closed.ko.md` |
| 그 앞 인계 (배경·경위) | `docs/handoffs/2026-08-14-videobox-claude-resume-handoff.ko.md` |
...
- 전문 색보정(advanced grading), 고급 마스크(advanced masks), 임의 키프레임(arbitrary keyframes),
  멀티캠(multicam), 고급 모션그래픽과 실시간 멀티트랙 UI는 범위 밖이다.
- 이 경계를 넘는 요구가 오면 먼저 계획서와의 충돌을 사용자에게 알리고 결정을 받는다.

**범위 확장 승인됨 (2026-08-16).** owner가 자율 창작 루프
(`추천 → 선택 → 대본 → 확정 → 제작 → 업로드 → 분석 → 추천`)를 최종 목표로 확정하고
범위 확장을 명시 승인했다. 승인 기록:
...
- **넓어진 것은 편집 앞뒤다** — 대본·제목 제안, 유튜브 업로드, 성과 분석이 범위 안으로 들어왔다.
- **위 편집 경계는 그대로다** — 전문 색보정·고급 마스크·임의 키프레임·멀티캠은 여전히 밖이다.
- **사람이 남는 자리 세 곳을 없애지 않는다**: 제목 `선택`, 대본 `확정`, 업로드 `누르기`.
  owner가 직접 지정한 게이트다.
...
2026-08-07에 이 정의로 다시 점검했더니 "완료"로 닫혀 있던 것들이 이렇게 나왔다.
...
하루에 이 패턴을 여덟 번 찾았다. **부품을 만드는 것과 제품을 만드는 것은 다르다.**

이 격차는 재보면 계속 나온다. 2026-08-07에 `api.ts` 메서드 144개 중 37개가 화면에서
한 번도 호출되지 않았고, 2026-08-09에 다시 재니 147개 중 **31개**가 여전히 화면 코드
어디에도 이름조차 없었다. 그 사이 여러 개를 화면에 붙였는데도 그렇다. 숫자 자체보다
**새 백엔드를 만들 때마다 부르는 화면을 같이 만들지 않으면 이 목록이 늘어난다**는 것이
요점이다.
...
`artifacts/` 정리 (owner 승인, 2026-08-09). 무조건 보존이 아니라 **다시 만들 수
있는가**로 판단한다. 아래 둘 중 하나면 지워도 된다.
...
- 외부 게시·업로드 — **유튜브 업로드와 텔레그램 연결은 2026-08-16에 승인됐다**
  (`docs/decisions/2026-08-16-autonomous-creator-loop-scope-expansion.ko.md`).
  **다만 개별 업로드마다 owner가 누르는 게이트는 없애지 않는다.** 그 밖의 외부 게시는
  여전히 이 조항 아래 있다.
