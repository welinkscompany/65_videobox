...
이 문서에 모델 이름을 적지 않는다 — 적는 순간 갈라진다(2026-09-11에 실제로
여섯 곳이 갈라져 있었다).

- **바꾸는 법은 한 명령이다**: `scripts/set-local-model.ps1 <모델id>`. 여섯 곳을
  한 번에 바꾸고, **LM Studio에 그 모델이 안 올라와 있으면 거부한다**(`-Force`로만
...
- **갈라짐 울타리**: `tests/test_local_model_name_is_one_value.py`가 compose 기본값·
  유진 프로필·코드 기본값이 서로 같은지 본다. 값을 안 박았으므로 모델을 바꿔도
  이 시험은 안 고쳐도 된다.
- **한 기계에 모델이 여럿 있을 수 있다.** owner가 다른 프로젝트에서 옛 모델을
  함께 쓰는 기간이 있다(2026-09-11). 그래서 "안 쓰니까 지워도 된다"고 적지 마라.
- **영상 분석도 설정을 따른다**(2026-09-11 수정). 예전에는 LM Studio에 **먼저
  올라온** 비전 모델을 집어서, 다른 프로젝트가 옛 모델을 올려 두면 설정과 무관하게
  그것이 쓰였다. 같은 함정을 다시 파지 마라.

## 4. 완료의 정의 — 이 저장소가 가장 비싸게 배운 것

**완료: owner가 화면에서 그 기능을 실제로 쓸 수 있는가.** 백엔드가 도는 것은 완료가 아니다.
테스트 통과만으로 완료를 주장하지 않는다 — 단위 테스트는 배관을 검증할 뿐이다. 부품을
만드는 것과 제품을 만드는 것은 다르다.

**API 단건 확인은 화면 확인을 대체하지 못한다.** 화면은 보통 여러 API를 함께 부르고,
그중 하나만 실패해도 사용자에게는 아무것도 보이지 않는다. `curl`이나 스크립트로 엔드포인트
하나를 통과시킨 것을 화면 검증으로 쓰지 않는다 — owner가 밟을 경로를 브라우저에서 그대로 밟는다.

이 정의로 다시 잴 때마다 격차가 나왔다 — 구체적 사고 기록은
`docs/development-fast-path.ko.md` §10.5.1.
...
**뜻: 기본은 자체 호스팅(`VIDEOBOX_MEM0_MODE=local`)이라 밖으로 안 나간다. `MEM0_API_KEY`를
채우면 그때만 승인 저장한 기억 문구(280자 이내)와 검색 질의가 외부로 나간다.** 원본
영상·자산·대본·프로젝트 식별자는 안 나가고, 스키마가 `extra="forbid"`라 그 밖의 것을
실을 수도 없다. 나머지는 전부 로컬이다.

**로컬이 원본, Mem0는 검색만.** 게이트웨이 응답 중 로컬 기록과 정확히 일치하는 것만
채택한다(`yujin_memory_service.py:207`) — 외부가 기억을 주입할 수 없다. 이 대조는
제거하지 않는다.

**조회 폴백이 있다(2026-08-31).** 게이트웨이가 없으면 로컬 승인 기억을 저장 순서로 돌려준다(`:194`).
