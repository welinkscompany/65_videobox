...
| **최신 세션 인계** | `docs/handoffs/2026-08-10-videobox-session-close-owner-audio-and-store-split.ko.md` |
...
- MVP 제외: 풀 자체 편집기, 실시간 멀티트랙 UI, 고급 모션그래픽, 색보정, 오디오 믹싱,
  자유 키프레임, 완전 자동 최종본 보장, 결제·멀티유저.
- 편집기에 기능을 넣을지는 §8.4의 한 가지 질문으로 판단한다: **CapCut으로 넘겨도 되는 일인가.**
  넘겨도 되면 넣지 않는다. 색보정·오디오 믹싱·키프레임·모션그래픽은 계속 제외다.
