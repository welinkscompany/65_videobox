...
**이 승인이 무엇을 뜻하는지 분명히 한다 — 유진이 기억하는 내용, 즉 owner의 영상 기획과
대화 내용이 외부 서비스로 나간다.** VideoBox의 다른 모든 것은 여전히 로컬이며, 이
승인은 Mem0 기억 경로 하나에만 적용된다. 다른 외부 전송의 근거가 아니다.

**함께 감수하는 것: 기억에 폴백이 없다.** 게이트웨이가 없으면 유진의 기억 조회는 빈
결과를 돌려준다(`yujin_memory_service.py:127`). 로컬 기억으로 되돌아가지 않는다.
Mem0를 켜면 기억이 외부 의존이 되고, 외부가 죽으면 대화는 되지만 과거 기억은 못 꺼낸다.
폴백을 넣는 것은 별도 작업이다.
