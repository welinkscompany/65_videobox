...
2026-08-05부로 개발 주체를 Claude Code로 전환했다. 이 저장소에는 이제 `AGENTS.md`가 없다 —
아무 코드·문서·스크립트도 그 파일을 참조하지 않는 것을 확인한 뒤 완전히 삭제했다.
...
`§10.2.3`은 코드리뷰·갭검증·역방향검증을 고정 의무가 아니라 리스크 기반으로 하라고 한다.
아래는 그 판단의 기본값이다. 실제로 2026-08-05에 갭 검증이 죽은 코드를 잡았다.
projection에 필드를 추가하고 필터 분기까지 만들었는데 **UI가 호출하지 않아 동작하지 않는 코드**였고,
이는 이 프로젝트가 고치려는 "만들어놓고 연결 안 함" 패턴 그 자체였다.
...
실제 음성 인식이 한 번도 실행되지 않았다. 단위 테스트는 배관을 검증할 뿐이다.

**완료의 정의: owner가 화면에서 그 기능을 실제로 쓸 수 있는가.**

백엔드가 동작하는 것은 완료가 아니다. 2026-08-07에 이 정의로 다시 점검했더니
"완료"로 닫혀 있던 것들이 이렇게 나왔다.

- `Task 17`(숏폼)은 백엔드 기준 완료였지만 **세로를 고를 화면이 없어** owner는 숏폼을
  만들 수 없었다
- `F-9`(가로 기본값)는 해결됨이었지만 **owner가 쓰는 경로는 안 고쳐져** 롱폼까지 전부
  세로로 렌더되고 있었다
- 검토 승인 API는 있었지만 **부르는 화면이 없어** 내보내기가 통째로 막혀 있었다

하루에 이 패턴을 여덟 번 찾았다. `api.ts`의 메서드 144개 중 37개가 화면에서 한 번도
호출되지 않는다. **부품을 만드는 것과 제품을 만드는 것은 다르다.**

**API 단건 확인은 화면 확인을 대체하지 못한다.**

화면은 보통 여러 API를 함께 부르고, **그중 하나만 실패해도 사용자에게는 아무것도 보이지
않는다.** 2026-08-07에 승인 API가 202로 통과하는 것을 확인하고 "내보내기가 뚫렸다"고
판단했으나, 실제로 브라우저에서 열어보니 같은 화면이 다른 호출의 500 때문에 여전히
"불러오지 못했어요"만 띄우고 있었다.

`curl`이나 스크립트로 엔드포인트 하나를 통과시킨 것을 화면 검증으로 쓰지 않는다.
**owner가 밟을 경로를 브라우저에서 그대로 밟는다.** 그 화면이 부르는 모든 호출을 함께
확인한다.
...
**이 승인이 무엇을 뜻하는지 분명히 한다 — 유진이 승인 저장한 기억 문구(280자 이내)와
검색 질의가 외부 서비스로 나간다.** 원본 영상·자산·대본·프로젝트 식별자는 나가지 않고,
스키마가 `extra="forbid"`라 그 밖의 것을 실을 수도 없다. VideoBox의 다른 모든 것은
여전히 로컬이며, 이 승인은 Mem0 기억 경로 하나에만 적용된다.

**로컬이 원본이고 Mem0는 검색만 맡는다.** 게이트웨이가 돌려준 항목 중 로컬 기록과
정확히 일치하는 것만 채택하므로(`yujin_memory_service.py:181`) 외부가 기억을 주입할 수
없다. 이 대조를 제거하지 않는다.

**함께 감수하는 것: 조회 폴백이 없다.** 게이트웨이가 없으면 기억 조회가 빈 결과를
돌려준다(`:127`). 기억이 사라지는 게 아니라 **꺼내 줄 검색이 없는 것**이다. 결과적으로
유진은 과거 기억을 참조하지 못한다. 폴백을 넣는 것은 별도 작업이고, 의미 검색 없이
무엇을 돌려줄지 품질 판단이 따로 필요하다.
