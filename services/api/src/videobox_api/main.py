...
def create_app(*, projects_root: Path | None = None) -> FastAPI:
    ...
    @app.post(
        "/api/projects/{project_id}/assets/narration-audio",
    ...
    @app.post(
        "/api/projects/{project_id}/assets/script-document",
        status_code=status.HTTP_201_CREATED,
    )
    ...
    @app.post(
        "/api/projects/{project_id}/jobs/transcription",
    ...
    @app.post(
        "/api/projects/{project_id}/jobs/segment-analysis",
        status_code=status.HTTP_202_ACCEPTED,
    )
    def start_segment_analysis(
        project_id: str,
        payload: StartSegmentAnalysisRequest,
    ) -> StartJobResponse:
        try:
    ...
    def get_segment_analysis_job(project_id: str, job_id: str) -> SegmentAnalysisJobResponse:
        try:
            ...
            raise _http_error(exc) from exc
        return SegmentAnalysisJobResponse(**result)
