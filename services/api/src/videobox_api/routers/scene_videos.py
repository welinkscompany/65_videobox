...
def build_scene_videos_router(store: LocalProjectStore) -> APIRouter:
    ...
    def start_scene_video(
        ...
        job_id = uuid4().hex
        with _jobs_lock:
            _jobs[job_id] = {"project_id": project_id, "status": "processing", "result": None, "error_detail": None}
        background_tasks.add_task(_run_job, service, project_id, job_id, payload)
...
def _run_job(service: Any, project_id: str, job_id: str, payload: SceneVideoCreateRequest) -> None:
    ...
    try:
        ...
        with _jobs_lock:
            _jobs[job_id] = {"project_id": project_id, "status": "failed", "result": None, "error_detail": _detail(exc)}
    except Exception as exc:  # noqa: BLE001 -- 백그라운드 작업이라 여기서 반드시 잡아야 폴링이 영원히 "처리 중"으로 남지 않는다
        with _jobs_lock:
            _jobs[job_id] = {"project_id": project_id, "status": "failed", "result": None, "error_detail": str(exc)}
